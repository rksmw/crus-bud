-- Arreglar evento "Pilgrim's Bounty"/"Generosidad del Peregrino" ( http://wotlk.openwow.com/event=41 )
...
(32823, 46598, 1, 0), -- Bountiful Table (Ride Vehicle Hardcoded)
(32830, 46598, 1, 0), -- Food Holder (Ride Vehicle Hardcoded)
(32840, 46598, 1, 0); -- Plate Holder (Ride Vehicle Hardcoded)
...
DELETE FROM `achievement_criteria_data` WHERE `criteria_id` IN (11201,11200,11202,11198,11199,11203,11204,11205,11206,11207);
...
-- Temp vehicle fix
UPDATE `creature_template` SET `speed_run` = 0, `VehicleId` = '321' WHERE `entry` IN (34823,34812,34824,34822,34819); -- Temp Fix (not blizz)
...
-- Tiempo de respawn de los pavos
UPDATE `creature` SET `spawntimesecs` = 5 WHERE `id` = 32820;
