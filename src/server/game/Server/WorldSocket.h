...
class LoginInfo
{
    ...
    protected:
        LoginInfo(std::string account, uint32 clientBuild, uint32 clientSeed, uint8 digest[], uint32 unk2, uint32 unk3, uint64 unk4, uint32 unk5, uint32 unk6, uint32 unk7, WorldPacket& data)
        : Account(account), ClientBuild(clientBuild), ClientSeed(clientSeed), Unk2(unk2), Unk3(unk3), Unk4(unk4), Unk5(unk5), Unk6(unk6), Unk7(unk7), Data(data)
        {
            memcpy(Digest, digest, sizeof(uint8) * 20);
        }
...
class WorldSocket : public WorldHandler
{
    ...
    private:
        ...
        void HandleAuthSessionCallback(PreparedQueryResult result, LoginInfo* loginInfo);

        QueryCallback<PreparedQueryResult, LoginInfo*, true> _loginCallback;
