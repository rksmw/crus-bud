...
enum BattlefieldQueueResponses
{
    BF_QUEUE_RESPONSE_FAILED,
    BF_QUEUE_RESPONSE_SUCCESS,
    BF_QUEUE_RESPONSE_FULL
};
...
class WorldSession
{
    ...
    public:                                                 // opcodes handlers
        ...
        // Battlefield
        void SendBfInvitePlayerToBattle(uint32 battleId, uint32 zoneId, uint32 time);
        void HandleBfBattleInviteResponse(WorldPacket &recvData);
        void SendBfBattleInviteResponse(uint32 battleId);
        void HandleBfQueueRequest(WorldPacket &recvData);
        void SendBfInvitePlayerToQueue(uint32 battleId);
        void HandleBfQueueInviteResponse(WorldPacket &recvData);
        void SendBfQueueInviteResponse(uint32 battleId, uint32 zoneId, BattlefieldQueueResponses response);
        void HandleBfExitRequest(WorldPacket &recvData);
        void SendBfLeavePending(uint32 battleId);
        void SendBfLeaveMessage(uint32 battleId, BattlefieldLeaveReason reason = BF_LEAVE_REASON_EXITED);
        void HandleBfManagerAdvanceState(WorldPacket &recvData);
        void HandleBfManagerTransitionTime(WorldPacket &recvData);
