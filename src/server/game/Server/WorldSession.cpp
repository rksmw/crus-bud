...
void WorldSession::LogoutPlayer(bool save)
{
    ...
    if (_player)
    {
        ...
        else if (_player->GetMapId() == 631)
        {
            // Exploit fix para Blood-Queen Lana'thel y Lich King
            InstanceScript* instance = _player->GetInstanceScript();
            if (instance && (instance->GetBossState(8) == IN_PROGRESS || instance->GetBossState(12) == IN_PROGRESS))
                _player->RepopAtGraveyard();
        }
