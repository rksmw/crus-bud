...
void IpCountryMgr::Load()
{
    ...
    {
        sLog->outDebug(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 ipcountries. DB table `ipcountries` is empty!");
        return;
    }

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u ipcountries in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
