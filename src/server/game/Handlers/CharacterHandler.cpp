...
void WorldSession::HandleChangePlayerNameOpcodeCallBack(PreparedQueryResult result, std::string const& newName)
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_RENAME_LOG);
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_RACECHANGE_LOG);

    stmt->setUInt32(0, lowGuid);
    stmt->setUInt8(1, oldRace);
    stmt->setUInt8(2, race);
