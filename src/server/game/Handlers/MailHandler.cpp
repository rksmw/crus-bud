...
void WorldSession::HandleSendMail(WorldPacket& recvData)
{
    ...
    if (!accountBound && player->GetSession()->GetAccountId() != receiverAccountId && player->GetTeam() != receiverTeam && !HasPermission(RBAC_PERM_TWO_SIDE_INTERACTION_MAIL))
    {
    ...
    if (items_count > 0 || money > 0)
    {
            ...
            for (uint8 i = 0; i < items_count; ++i)
            {
                ...
                if (log)
                {
                    if (sWorld->getBoolConfig(CONFIG_LOG_COMMANDS_TO_DB))
                        sLog->outTradeDB("Mail Send", GetAccountId(), item->GetEntry(), item->GetCount(), 0, receiverAccountId);
                    else
        ...
        {
            if (sWorld->getBoolConfig(CONFIG_LOG_COMMANDS_TO_DB))
                sLog->outTradeDB("Mail Send", GetAccountId(), 0, 0, money, receiverAccountId);
            else
    ...
    if (msg == EQUIP_ERR_OK)
    {
        ...
        if (m->COD > 0)                                     //if there is COD, take COD money from player and send them to sender by mail
        {
            ...
            if (HasPermission(RBAC_PERM_LOG_GM_TRADE))
            {
                ...
                if (sWorld->getBoolConfig(CONFIG_LOG_COMMANDS_TO_DB))
                    sLog->outTradeDB("Mail Send Cod", GetAccountId(), 0, 0, m->COD, sender_accId);
                else
