...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    {
        //Custom - LANG_UNIVERSAL in Dalaran PvP Area.
        if (sWorld->getBoolConfig(CONFIG_DALARAN_UNIVERSAL_LANG) && sender->GetAreaId() == 4570)
            lang = LANG_UNIVERSAL;
            ...
            else
            {
                switch (type)
                {
                    ...
                    case CHAT_MSG_OFFICER:
                        ...
                        break;
                    case CHAT_MSG_SAY:
                        if (Battleground* bg = sender->GetBattleground())
                            if (sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP) && ((sender->InArena() && bg->GetArenaType() == ARENA_TYPE_3v3_SOLO) || sender->isUsingLfg() || sender->inRandomLfgDungeon()))
                                lang = LANG_UNIVERSAL;
                        break;
                }
            }
