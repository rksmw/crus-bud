...
void WorldSession::HandleGossipHelloOpcode(WorldPacket& recvData)
{
    ...
    if (unit->IsSpiritGuide())
    {
        ...
        Battlefield* bf = _player->GetBattlefield();
        if (bf)
        {
            bf->AddPlayerToResurrectQueue(unit->GetGUID(), _player->GetGUID());
            sBattlefieldMgr->SendAreaSpiritHealerQueryOpcode(_player, bf, unit->GetGUID());
        }
    }
...
void WorldSession::HandleAreaSpiritHealerQueryOpcode(WorldPacket & recvData)
{
    ...
    if (bf)
        sBattlefieldMgr->SendAreaSpiritHealerQueryOpcode(_player, bf, guid);
}
