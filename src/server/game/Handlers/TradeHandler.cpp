...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    if (his_trade->IsAccepted())
    {
        ...
        if (HasPermission(RBAC_PERM_LOG_GM_TRADE))
        {
            ...
            {
                if (sWorld->getBoolConfig(CONFIG_LOG_COMMANDS_TO_DB))
                    sLog->outTradeDB("Trade", _player->GetSession()->GetAccountId(), 0, 0, my_trade->GetMoney(), trader->GetSession()->GetAccountId());
                else
            ...
            {
                if (sWorld->getBoolConfig(CONFIG_LOG_COMMANDS_TO_DB))
                    sLog->outTradeDB("Trade", trader->GetSession()->GetAccountId(), 0, 0, his_trade->GetMoney(), _player->GetSession()->GetAccountId());
                else
