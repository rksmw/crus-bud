...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvData)
{
    ...
    // Tele back up in case of falling under texture, for RoV and DS
    uint32 Orientacion = GetPlayer()->GetOrientation();
    // Dalaran Sewers
    if (_player->GetMapId() == 617)
    {
        if (movementInfo.pos.GetPositionZ() < 3)
            _player->TeleportTo(_player->GetMapId(), 1291, 790, 7.12f, Orientacion, 0);
    }
    // Ruins of Lordaeron
    if (_player->GetMapId() == 572)
    {
        if (movementInfo.pos.GetPositionZ() < 31.2)
            _player->TeleportTo(_player->GetMapId(), 1286, 1668, 40, Orientacion, 0);
    }
    // Ruins of Nagrand
    if (_player->GetMapId() == 559)
    {
        if (movementInfo.pos.GetPositionZ() < 7)
            _player->TeleportTo(_player->GetMapId(), 4054, 2925, 14.5f, Orientacion, 0);
    }
    // Blade's Edge
    if (_player->GetMapId() == 562)
    {
        if (movementInfo.pos.GetPositionZ() < -3)
            _player->TeleportTo(_player->GetMapId(), 6241, 262, 1.5f, Orientacion, 0);
    }
