...
void LoadSkillPerfectItemTable()
{
    ...
    if (!result)
    {
        TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 spell perfection definitions. DB table `skill_perfect_item_template` is empty.");
        return;
    }
    ...
    do /* fetch data and run sanity checks */
    {
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Skill perfection data for spell %u has non-existent spell id in `skill_perfect_item_template`!", spellId);
            continue;
        }
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Skill perfection data for spell %u has non-existent required specialization spell id %u in `skill_perfect_item_template`!", spellId, requiredSpecialization);
            continue;
        }
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Skill perfection data for spell %u has impossibly low proc chance in `skill_perfect_item_template`!", spellId);
            continue;
        }
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_SQL, "Skill perfection data for spell %u references non-existent perfect item id %u in `skill_perfect_item_template`!", spellId, perfectItemType);
            continue;
        }
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u spell perfection definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
