    ...
    if (result)
    {
        do
        {
            ...
            if (CreatureData const* data = sObjectMgr->GetCreatureData(lowGUID))
                ...
                sLog->outError(LOG_FILTER_EVENT, "No se pudo obtener Creature GUID: %u", lowGUID);
        } while (result->NextRow());
    }
    else
        sLog->outError(LOG_FILTER_EVENT, "No se pudieron obtener Creatures en el evento %u", GetEventId());
}
    ...
    if (result)
    {
        do
        {
            ...
            if (GameObjectData const* data = sObjectMgr->GetGOData(lowGUID))
                ...
                sLog->outError(LOG_FILTER_EVENT, "No se pudo obtener el GObject GUID: %u", lowGUID);
        } while (result->NextRow());
    }
    else
        sLog->outError(LOG_FILTER_EVENT, "No se pudieron obtener GObjects en el evento %u", GetEventId());
}
