...
void EventMgr::Load()
{
    ...
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 events. DB table `event` is empty!");
        return;
    }

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u events in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
