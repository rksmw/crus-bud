...
void TargetedMovementGeneratorMedium<T, D>::_setTargetLocation(T* owner, bool updateDestination)
{
    ...
    // Toda unidad que este siguiendo puede forzar el destino
...
bool TargetedMovementGeneratorMedium<T, D>::DoUpdate(T* owner, uint32 time_diff)
{
    ...
    if (i_recheckDistance.Passed())
    {
        ...
        // Si es una Pet y esta en Arena o Battleground es ultra sensible a la reaccion de movimiento del target
        ...
        if (!owner->GetMap()->IsBattlegroundOrArena())
            ...
            allowed_dist += CONTACT_DISTANCE; // Distancia minima de recalculo

        // Si es un movimiento de combate sumamos el maximo rango melee
        ...
        /** No funciona con npcs grandes
        // El NPC no puede estar sobre el target
        if (!targetMoved && owner->movespline->Finalized() && owner->GetTypeId() == TYPEID_UNIT && i_target->GetTypeId() == TYPEID_PLAYER)
        {
            float x, y, z;
            i_target->GetPosition(x, y, z);
            targetMoved = owner->IsWithinDist3d(x, y, z, i_target->GetObjectSize());
        }
        */

        // Si el actual punto o el siguiente no es OffMesh
    ...
    */
    ...
    if (owner->movespline->Finalized())
    {
        ...
        // Limpiamos el path (por los Offmesh)
        if (i_path)
            i_path->Clear();
    }
    else
...
{
    // Si es una pet, actualizamos su orientacion
    if (owner->IsPet())
