...
PathGenerator::PathGenerator(const Unit* owner) :
    ...
    _navMeshQuery(NULL), _antiLoop(false), _complementPath(NULL)
{
...
bool PathGenerator::CalculatePath(float destX, float destY, float destZ, bool forceDest, float startX, float startY, float startZ)
{
    Clear(); // Evitamos taldeos por reutilizar la data
    ...
    if (!_navMesh || !_navMeshQuery || _sourceUnit->HasUnitState(UNIT_STATE_IGNORE_PATHFINDING) ||
        (_sourceUnit->ToCreature() && _sourceUnit->ToCreature()->IsIgnorePathfinding()) ||
...
void PathGenerator::BuildPolyPath(G3D::Vector3 const& startPos, G3D::Vector3 const& endPos)
{
    ...
    if (startPoly == INVALID_POLYREF || endPoly == INVALID_POLYREF)
    {
        ...
        _type = (path || waterPath) ? PATHFIND_NORMAL | PATHFIND_NOT_USING_PATH : PATHFIND_NOPATH;
        return;
    }
    ...
    if (_polyLength)
    {
        for (; pathStartIndex < _polyLength; ++pathStartIndex)
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_MAPS, "Invalid poly ref in BuildPolyPath. _polyLength: %u, pathStartIndex: %u,"
...
void PathGenerator::BuildPointPath(const float *startPoint, const float *endPoint)
{
    ...
    else
    {
        ...
        if (_sourceUnit->GetMapId() == 649) // Trial of Crusader
    ...
    if (!_antiLoop && !_forceDestination && (_type & PATHFIND_INCOMPLETE))
    {
        // Si se encuentran en tiles diferentes
        ...
        {
            // Calculamos el angulo entre el tile final y el actual
            ...
            // Almacenamos la posicion final actual, como nuestro nuevo punto de partida
            Position startPos;
            startPos.Relocate(GetActualEndPosition().x, GetActualEndPosition().y, GetActualEndPosition().z);

            // Movemos nuestro nuevo punto de partida en 1 yard
            _sourceUnit->MovePositionToFirstCollision(startPos, 1.0f, angle - _sourceUnit->GetOrientation());

            // Almacenamos la nueva posicion del punto de partida
            float startX, startY, startZ;
            startX = startPos.GetPositionX();
            startY = startPos.GetPositionY();
            startZ = startPos.GetPositionZ();

            // Creamos un nuevo PathGenerator, para continuar el path entre los tiles
            if (!_complementPath)
                _complementPath = new PathGenerator(_sourceUnit);
            // Activamos una variable para que no se ejecute de forma infinita
            _complementPath->SetAntiLoop();
            // Definimos limite de path, considerando la cantidad de points actuales
            _complementPath->SetPathLengthLimit((_pointPathLimit - pointCount) * SMOOTH_PATH_STEP_SIZE);
            // Calculamos el resto del path
            ...
            if (result && (_complementPath->GetPathType() & PATHFIND_NORMAL))
            {
                ...
                // i es el indice del path original
                // j es el indice del path complementario
                uint32 i = pointCount, j = 1; // Comienza de uno porque el primero punto es igual al ultimo

                pointCount += pathPoints.size() - 1; // Tiene un punto menos, pues no consideramos el primero
                _pathPoints.resize(pointCount);

                // Completamos el path con los nuevos puntos
                ...
                // Igualamos el destino final actual
                                     ...
                                     float* smoothPath, int* smoothPathSize, uint32 maxSmoothPathSize)
{
    *smoothPathSize = 0;
    ...
    // Limpiamos el contenedor de OffMeshPoints
    ...
    while (npolys && nsmoothPath < maxSmoothPathSize)
    {
        ...
        else if (offMeshConnection && InRangeYZX(iterPos, steerPos, SMOOTH_PATH_SLOP, 1.0f))
        {
            ...
            if (dtStatusSucceed(_navMesh->getOffMeshConnectionPolyEndPoints(prevRef, polyRef, startPos, endPos)))
            {
                if (nsmoothPath < maxSmoothPathSize)
                {
                    // Insertamos el OffMeshPoint (START)
                    _offMeshIdPoints.insert(nsmoothPath);
                    dtVcopy(&smoothPath[nsmoothPath*VERTEX_SIZE], startPos);
                    nsmoothPath++;
                }
                // Insertamos el OffMeshPoint (END)
                _offMeshIdPoints.insert(nsmoothPath);
