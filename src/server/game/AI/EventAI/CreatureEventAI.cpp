...
void CreatureEventAI::DoScriptText(int32 textEntry, WorldObject* source, Unit* target)
{
    ...
    {
        case CHAT_TYPE_SAY:
            source->MonsterSay(textEntry, (*i).second.Language, target ? target : 0);
            break;
        case CHAT_TYPE_YELL:
            source->MonsterYell(textEntry, (*i).second.Language, target ? target : 0);
            break;
        case CHAT_TYPE_TEXT_EMOTE:
            source->MonsterTextEmote(textEntry, target ? target : 0);
            break;
        case CHAT_TYPE_BOSS_EMOTE:
            source->MonsterTextEmote(textEntry, target ? target : 0, true);
            break;        
    }
}
