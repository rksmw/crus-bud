...
struct JoinTargetSelector : public std::unary_function<Unit*, bool>
{
    public:
        JoinTargetSelector(PREDICATE1 const& predicate1, PREDICATE2 const& predicate2) : _predicate1(predicate1), _predicate2(predicate2) { }
        bool operator()(Unit const* target) const
        {
            return _predicate1(target) && _predicate2(target);
        }

    private:
        PREDICATE1 const& _predicate1;
        PREDICATE2 const& _predicate2;
};
