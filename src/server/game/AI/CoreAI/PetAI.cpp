...
void PetAI::UpdateAI(uint32 diff)
{
    ...
    if (me->GetVictim() && me->GetVictim()->IsAlive())
    {
        ...
        {
            if (!me->GetOwner()->IsDynCombat() && me->GetCharmInfo()->IsCommandAttack() || (me->GetCharmInfo()->IsAtStay() && me->IsWithinMeleeRange(me->GetVictim())))
                me->GetOwner()->SetDynCombat(true);
            else if (!me->GetCharmInfo()->IsCommandAttack() && me->GetOwner()->IsDynCombat())
                me->GetOwner()->SetDynCombat(false);

            if (me->HasUnitState(UNIT_STATE_FLEEING | UNIT_STATE_ROOT | UNIT_STATE_STUNNED | UNIT_STATE_DISTRACTED | UNIT_STATE_CONFUSED) && me->GetOwner()->IsDynCombat())
                me->GetOwner()->SetDynCombat(false);
        }
    ...
    if (!me->HasUnitState(UNIT_STATE_CASTING))
    {
        ...
        for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i)
        {
            ...
            if (spellInfo->IsPositive() && me->GetVictim())
            {
...
bool PetAI::CanAttack(Unit* target)
{
    ...
    // La pet no debe atacar a jugadores que se encuentren en una trampa
    if (target->HasAura(60210)      // Freezing Arrow Effect
        || target->HasAura(31932))  // Freezing Trap Effect
        return me->GetCharmInfo()->IsCommandAttack();
