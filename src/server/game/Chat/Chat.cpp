    ...
    for (uint32 i = 0; table[i].Name != NULL; ++i)
    {
        ...
        if ((table[i].Handler)(this, table[i].Name[0] != '\0' ? text : oldtext))
        {
            ...
            if (!AccountMgr::IsPlayerAccount(m_session->GetSecurity()))
            {
                ...
                {
                    if (sel_guid == p->GetGUID())
                        sel_guid = 0;
                    std::string command;
                    size_t pos = fullcmd.find(table[i].Name[0] != '\0' ? text : oldtext);
                    if (pos != std::string::npos && pos < fullcmd.size())
                        command = fullcmd.substr(1, pos-1);
                    else
                        command = fullcmd.substr(1, fullcmd.size());
                    std::string args = fullcmd.substr(command.size(), fullcmd.size());

                    bool HasTarget = p->GetSelectedUnit() && sel_guid != 0;
                    sLog->outCommandDB(command.c_str(), args.c_str(), m_session->GetAccountId(), p->GetGUIDLow(), p->GetPositionX(),
                        p->GetPositionY(), p->GetPositionZ(), p->GetMapId(), HasTarget ? (uint32)p->GetSelectedUnit()->GetTypeId() : TYPEID_PLAYER, HasTarget ? p->GetSelectedUnit()->GetName().c_str() : "", HasTarget ? p->GetSelectedUnit()->GetGUIDLow() : 0);
                }
                else
...
bool CliHandler::isAvailable(ChatCommand const& cmd) const
{
    ...
    return HasPermission(cmd.Permission) && cmd.AllowConsole;
}
