...
class WorldObject : public Object, public WorldLocation
{
    ...
    public:
        ...
        void MovePosition(Position &pos, float dist, float angle, bool ignoreZLOS = false) const;
        void GetNearPosition(Position &pos, float dist, float angle);
        void MovePositionToFirstCollision(Position &pos, float dist, float angle, bool ignoreZLOS = false) const;
        void GetFirstCollisionPosition(Position &pos, float dist, float angle, bool ignoreZLOS = false) const;
         ...
         */
        typedef std::map<uint64, bool> EventVisible;
        EventVisible _EventVisibilityPlayers;
        // Para todo jugador que no se encuentre sera invisible, siempre que haya al menos una visibilidad determinada
        // La visibilidad se hereda para todo el grupo, si es que lo hay
        void SetEventVisibleFor(Player* player, bool x);
        void DelEventVisibleFor(Player* player);
        void DelEventVisibleFor(uint64 playerGuid);
        bool IsEventVisibleFor(const WorldObject* obj) const;

        bool IsBuildEventVisibleFor(const WorldObject* obj) const;
