...
bool WorldObject::IsWithinLOSInMap(const WorldObject* obj) const
{
    ...
    // Hack para Ice Tomb de ICC - Sindragosa Encounter
...
bool WorldObject::IsWithinLOS(float ox, float oy, float oz) const
{
    ...
    if (GetTypeId() == TYPEID_UNIT && (entry == 36980 || entry == 38321 || entry == 38320 || entry == 38322))
        return true;
...
void WorldObject::SetEventVisibleFor(Player* player, bool x)
{
    ASSERT(player || !ToPlayer());
    ...
    if (group)
    {
        for (GroupReference* groupRef = group->GetFirstMember(); groupRef != NULL; groupRef = groupRef->next())
            if (Player* member = groupRef->GetSource())
                _EventVisibilityPlayers[member->GetGUID()] = x;
    }
    else
        _EventVisibilityPlayers[player->GetGUID()] = x;
}

void WorldObject::DelEventVisibleFor(Player* player)
{
    ASSERT(player || !ToPlayer());
    ...
    if (group)
    {
        for (GroupReference* groupRef = group->GetFirstMember(); groupRef != NULL; groupRef = groupRef->next())
            if (Player* member = groupRef->GetSource())
                DelEventVisibleFor(member->GetGUID());
    }
    else
        DelEventVisibleFor(player->GetGUID());
}

void WorldObject::DelEventVisibleFor(uint64 playerGuid)
{
    _EventVisibilityPlayers.erase(playerGuid);
}

bool WorldObject::IsEventVisibleFor(const WorldObject* obj) const
{
    bool isVisible = true;              // Valor predeterminado para cuando no hay visibiliad determinada
    bool forPlayer = obj->ToPlayer();

    for (EventVisible::const_iterator itr = _EventVisibilityPlayers.begin(); itr != _EventVisibilityPlayers.end(); itr++)
    {
        // Cuando quiere ver un Jugador
        if (forPlayer)
        {
            // Si encontramos el jugador, retornarmos la visibiliadad para este
            if (obj->ToPlayer()->GetGUID() == (*itr).first)
                return (*itr).second;
        }
        // Cuando quiere ver a una Unidad o Objeto
        else
        {
            for (EventVisible::const_iterator itr2 = obj->_EventVisibilityPlayers.begin(); itr2 != obj->_EventVisibilityPlayers.end(); itr2++)
            {
                // Si tiene al menos un jugador en comun, se pueden ver entre ellos, siempre que los dos coincidan en la visiblidad
                if ((*itr).first == (*itr2).first)
                    return (*itr).second == (*itr2).second;
            }
        }

        // Mientras no lo encontremos sera invisible para este, si tiene al menos una visibilidad definida "true"
        if ((*itr).second)
            isVisible = false;
    }
...
bool WorldObject::IsBuildEventVisibleFor(const WorldObject* obj) const
{
    if (obj->ToPlayer() && ToPlayer())
        return true;

    Event* event = NULL;

    const Creature* creature = obj->ToCreature();
    if (creature && creature->GetDBTableGUIDLow() == creature->GetGUIDLow())
        event = sEventMgr->GetEventByCreatureGUID(creature->GetGUID(), true);

    const GameObject* gobject = obj->ToGameObject();
    if (gobject && gobject->GetDBTableGUIDLow() == gobject->GetGUIDLow())
        event = sEventMgr->GetEventByGameObjectGUID(gobject->GetGUID(), true);

    if (!event)
        return true;

    creature = ToCreature();
    gobject = ToGameObject();

    return (creature && creature->GetDBTableGUIDLow() == creature->GetGUIDLow() && event->IsEventCreature(creature->GetGUID())) ||
        (gobject && gobject->GetDBTableGUIDLow() == gobject->GetGUIDLow() && event->IsEventGameObject(gobject->GetGUID())) ||
        (ToPlayer() && event->IsConstructor(GetGUID()));
}
...
bool WorldObject::CanSeeOrDetect(WorldObject const* obj, bool ignoreStealth, bool distanceCheck, bool checkAlert) const
{
    ...
    // Solo seran visibles si los dos pueden verse
    if (!IsEventVisibleFor(obj) || !obj->IsEventVisibleFor(this))
        return false;

    // Si pertenece a un evento construido y no es visible
    if (!IsBuildEventVisibleFor(obj) || !obj->IsBuildEventVisibleFor(this))
        return false;
    ...
    {
        if (thisGO->GetGoType() == GAMEOBJECT_TYPE_TRAP && ToPlayer() && thisGO->GetOwner() && thisGO->GetOwner()->getClass() == CLASS_HUNTER)
        {
            float DetectRange = 5.5f; // Default distance Melee attack dist. + Collision (1.5f)
...
bool WorldObject::CanDetectStealthOf(WorldObject const* obj, bool checkAlert) const
{
    ...
    if (unit)
        combatReach = unit->GetCombatReach() + 1.5f;


    if (unit && obj && GetTypeId() == TYPEID_PLAYER && ToPlayer() && ToPlayer()->IsSpectator())
        return false;
    ...
    for (uint32 i = 0; i < TOTAL_STEALTH_TYPES; ++i)
    {
        ...
        float visibilityRange = float(detectionValue) * 0.35f + combatReach;
...
TempSummon* Map::SummonCreature(uint32 entry, Position const& pos, SummonPropertiesEntry const* properties /*= NULL*/, uint32 duration /*= 0*/, Unit* summoner /*= NULL*/, uint32 spellId /*= 0*/, uint32 vehId /*= 0*/)
{
    ...
    // Definimos visibilidad de evento basado en herencia
    if (summoner)
        for (std::map<uint64, bool>::const_iterator itr = summoner->_EventVisibilityPlayers.begin(); itr != summoner->_EventVisibilityPlayers.end(); itr++)
            if (Player* player = ObjectAccessor::FindPlayer((*itr).first))
                summon->SetEventVisibleFor(player, (*itr).second);
...
void WorldObject::SetZoneScript()
{
    if (Map* map = FindMap())
    {
        ...
        {
            if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldByZoneId(GetZoneId()))
                m_zoneScript = bf;
            else
...
GameObject* WorldObject::SummonGameObject(uint32 entry, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime)
{
    ...
    for (EventVisible::const_iterator itr = _EventVisibilityPlayers.begin(); itr != _EventVisibilityPlayers.end(); itr++)
        if (Player* player = ObjectAccessor::FindPlayer((*itr).first))
            go->SetEventVisibleFor(player, (*itr).second);
    ...
    class NearUsedPosDo
    {
        public:
            ...
            void operator()(Creature* c) const
            {
                ...
                if (!c->IsAlive() || c->HasUnitState(UNIT_STATE_ROOT | UNIT_STATE_  | UNIT_STATE_DISTRACTED) ||
...
void WorldObject::GetFirstCollisionPosition(Position &pos, float dist, float angle, bool ignoreZLOS) const
{
    GetPosition(&pos);
    MovePositionToFirstCollision(pos, dist, angle, ignoreZLOS);
}
...
void WorldObject::MovePosition(Position &pos, float dist, float angle, bool ignoreZLOS) const
{
    ...
    uint8 parts = dist >= 10.0f ? 10 : (dist <= 2.0f ? 2 : uint8(dist));
    float step = dist / parts;
    float stepX = step * std::cos(angle);
    float stepY = step * std::sin(angle);
    bool isPrecise = step <= 1.0f;

    // Retornamos las coordenadas al comienzo
    destx = pos.m_positionX;
    desty = pos.m_positionY;
    destz = pos.m_positionZ;

    TC_LOG_DEBUG(LOG_FILTER_MAPS, "\n++ MovePositionToFirstCollision ++\n");

    // Revisamos si es posible alcanzar el punto de collision en Z
    for (uint8 j = 0; j < parts; ++j)
    {
        float post_destx = destx + stepX;
        float post_desty = desty + stepY;
        float post_ground = GetMap()->GetHeight(GetPhaseMask(), post_destx, post_desty, MAX_HEIGHT, true);
        float post_floor = GetMap()->GetHeight(GetPhaseMask(), post_destx, post_desty, pos.m_positionZ, true);
        float post_destz = fabs(post_ground - pos.m_positionZ) <= fabs(post_floor - pos.m_positionZ) ? post_ground : post_floor;
        float dx = post_destx - destx;
        float dy = post_desty - desty;
        float catXY = sqrt(dx*dx + dy*dy);
        float catZ = fabs(destz - post_destz);
        float post_angle = std::atan(catZ/catXY);

        TC_LOG_DEBUG(LOG_FILTER_MAPS, "destx: %f", destx);
        TC_LOG_DEBUG(LOG_FILTER_MAPS, "desty: %f", desty);
        TC_LOG_DEBUG(LOG_FILTER_MAPS, "destz: %f", destz);
        TC_LOG_DEBUG(LOG_FILTER_MAPS, "post_destx: %f", post_destx);
        TC_LOG_DEBUG(LOG_FILTER_MAPS, "post_desty: %f", post_desty);
        TC_LOG_DEBUG(LOG_FILTER_MAPS, "post_destz: %f", post_destz);
        TC_LOG_DEBUG(LOG_FILTER_MAPS, "catXY: %f", catXY);
        TC_LOG_DEBUG(LOG_FILTER_MAPS, "catZ: %f", catZ);
        TC_LOG_DEBUG(LOG_FILTER_MAPS, "post_angle: %f\n", post_angle);

        // Si la altura tiene un angulo menor o igual a 60 grados
        if (post_angle <= M_PI/3)
        {
            destx = post_destx;
            desty = post_desty;
            destz = post_destz;
        }
        // llegamos al destino Z maximo
        else
        {
            if (isPrecise)
                break;
            ...
            {
                float temp_dist = sqrt((post_destx - destx)*(post_destx - destx) + (post_desty - desty)*(post_desty - desty));
                parts = temp_dist >= 10.0f ? 10 : (temp_dist <= 2.0f ? 2 : uint8(temp_dist));
                step = temp_dist / parts;
    ...
    if (GetMapId() == 618 && pos.m_positionZ < 29.0f)
        pos.m_positionZ = 28.5f;
...
void WorldObject::MovePositionToFirstCollision(Position &pos, float dist, float angle, bool ignoreZLOS) const
{
    angle += GetOrientation();
    float destx, desty, destz, ground, floor;
    ...
    // Realizaremos este proceso posterior al LOS
    /*
    ground = GetMap()->GetHeight(GetPhaseMask(), destx, desty, MAX_HEIGHT, true);
    floor = GetMap()->GetHeight(GetPhaseMask(), destx, desty, pos.m_positionZ, true);
    destz = fabs(ground - pos.m_positionZ) <= fabs(floor - pos.m_positionZ) ? ground : floor;
    */
    ...
    // El coef de tolerancia y orientacion ha sido alterada                                                              (ya tiene +2.0f)                     (2D)
    bool col = VMAP::VMapFactory::createOrGetVMapManager()->getObjectHitPos(GetMapId(), pos.m_positionX, pos.m_positionY, pos.m_positionZ, destx, desty, pos.m_positionZ, destx, desty, destz, -CONTACT_DISTANCE);
    ...
    // El coef de tolerancia y orientacion ha sido alterada                          (ya tiene +2.0f)                     (2D)
    col = GetMap()->getObjectHitPos(GetPhaseMask(), pos.m_positionX, pos.m_positionY, pos.m_positionZ, destx, desty, pos.m_positionZ, destx, desty, destz, -CONTACT_DISTANCE);
    ...
    // Ahora que hemos procesado el LOS, calcularemos si la altura es correcta
    ground = GetMap()->GetHeight(GetPhaseMask(), destx, desty, MAX_HEIGHT, true);
    floor = GetMap()->GetHeight(GetPhaseMask(), destx, desty, destz, true);
    destz = fabs(ground - destz) <= fabs(floor - destz) ? ground : floor;
