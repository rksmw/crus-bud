        ...
        if (Player *player = ToPlayer())
        {
            if (player->HaveSpectators() && slot < MAX_AURAS)
            {
                ...
                if (Unit* caster = aura->GetCaster())
                    casterID = (caster->ToPlayer()) ? caster->GetGUID() : 0;

                msg.SetGuid(player->GetGUID());
...
uint32 Unit::DealDamage(Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const* spellProto, bool durabilityLoss)
{
    ...
    if (spellProto && victim && this->GetTypeId() == TYPEID_PLAYER && victim->GetTypeId() == TYPEID_PLAYER && victim->HasAuraType(SPELL_AURA_MOD_STEALTH) && ((damage > 0 && damagetype == DOT)
        || (damagetype != DOT && damagetype != NODAMAGE && !(spellProto->AttributesCu & SPELL_ATTR0_CU_CANT_BREAK_STEALTH)) || ((spellProto->AttributesCu & SPELL_ATTR0_CU_CANT_BREAK_STEALTH) && damage > 0)
        || (!(spellProto->AttributesCu & SPELL_ATTR0_CU_CANT_BREAK_STEALTH) && spellProto->IsAffectingArea())))
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage* damageInfo, int32 damage, SpellInfo const* spellInfo, WeaponAttackType attackType, bool crit)
{
    ...
    if (spellInfo->AttributesEx4 & SPELL_ATTR4_FIXED_DAMAGE)
    {
        ...
        // Calculo de Resistencias Absorvidas
        if (damage > 0)
        {
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32 *absorb, uint32 *resist, SpellInfo const* spellInfo)
{
    ...
    if ((schoolMask & SPELL_SCHOOL_MASK_NORMAL) == 0 && (!spellInfo || (spellInfo->AttributesEx4 & SPELL_ATTR4_IGNORE_RESISTANCES) == 0))
    {
        ...
        {
            if (IsPet() || IsGuardian() && GetOwner()->GetTypeId() == TYPEID_PLAYER)
                if (Unit* owner = GetOwner())
                    victimResistance -= float(abs(owner->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_TARGET_RESISTANCE, schoolMask)) + owner->ToPlayer()->GetSpellPenetrationItemMod());
        }
        else
        {
            if (Player* player = ToPlayer())
                victimResistance -= float(abs(player->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_TARGET_RESISTANCE, schoolMask)) + player->GetSpellPenetrationItemMod());
        }
...
void Unit::AttackerStateUpdate (Unit* victim, WeaponAttackType attType, bool extra)
{
    ...
    if ((GetTypeId() == TYPEID_PLAYER) && (attType == BASE_ATTACK || attType == OFF_ATTACK) && !IsWithinLOSInMap(victim))
        return;
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if (!victim->HasInArc(M_PI, this))
    {
        ...
        {
            if (spell->AttributesCu & SPELL_ATTR0_CU_REQ_CASTER_BEHIND_TARGET || spell->Id == 19263)
                canParry = false;
        }
    }
    // Spells with these attributes can't be dodged parried or blocked even if the player has turned since the ability was used (Shred, Backstab, etc)
        ...
        spell->AttributesEx & SPELL_ATTR1_UNK27 && 
        spell->AttributesEx2 & SPELL_ATTR2_UNK20 && 
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spell)
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER && HitChance > 500) // Prevent Bug on Spells (50-100% MissChance) && Fixed PVP HitChance
        CantMiss = true;

    if (rand < tmp && victim->HasAura(31224))
        return SPELL_MISS_RESIST;
    else if (rand < tmp && !CantMiss)
        return SPELL_MISS_MISS;
    ...
    // Roll chance
    if (rand < tmp)
        return SPELL_MISS_RESIST; // Should not return RESIST (It's not binary resistances)
...
{
    // Si la unidad esta en el agua solo es accesible por criaturas que naden
    if (IsInWater())
        return c->CanSwim();
    // Si la unidad esta en el aire solo es accesible por criaturas que vuelen o lo alcancen fisicamente
    else if (IsFlying())
        return c->CanFly() || c->GetDistance(this) <= c->GetObjectSize() + CONTACT_DISTANCE;
    // Si la criatura vuela puede acceder a toda unidad
    else if (c->CanFly())
        return true;
    // Si la criatura camina puede acceder...
    else if (c->CanWalk())
    {
        // Si esta lo suficientemente cerca, evitamos generar carga de mas
        if (c->GetDistance(this) <= c->GetObjectSize() + CONTACT_DISTANCE)
            return true;

        // Si puede crear una ruta de MMAPS
        else if (MMAP::MMapFactory::IsPathfindingEnabled(GetMapId()) && !c->IsIgnorePathfinding())
        {
        ...
        // Si no tiene MMAPS activado
        else
            return true;
    }
    // No es accesible por nada
    else
        return false;
}
...
void Unit::RemoveOwnedAura(AuraMap::iterator &i, AuraRemoveMode removeMode)
{
    ...
    if (this && aura && aura->GetId())
    {
        if (Player* tempPlayer = this->ToPlayer())
        {
            if (tempPlayer->HaveSpectators() && !tempPlayer->IsSpectator() && !tempPlayer->IsGameMaster())
            {
                SpectatorAddonMsg msg;
                msg.SetGuid(tempPlayer->GetGUID());
                msg.RemoveAura(aura->GetCasterGUID(), aura->GetId(),
                    aura->GetSpellInfo()->IsPositive(), aura->GetSpellInfo()->Dispel,
                    aura->GetDuration(), aura->GetMaxDuration(),
                    aura->GetStackAmount(), true);
                tempPlayer->SendSpectatorAddonMsgToBG(msg);
            }
        }
    }
...
void Unit::RemoveAura(AuraApplication * aurApp, AuraRemoveMode mode)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
        if (ToPlayer()->HaveSpectators())
        {
            SpectatorAddonMsg msg;
            msg.SetPlayer(ToPlayer()->GetName());
            msg.RemoveAura(aurApp->GetBase()->GetCasterGUID(), aurApp->GetBase()->GetId(),
                aurApp->GetBase()->GetSpellInfo()->IsPositive(), aurApp->GetBase()->GetSpellInfo()->Dispel,
                aurApp->GetBase()->GetDuration(), aurApp->GetBase()->GetMaxDuration(),
    ...
    switch (dummySpell->SpellFamilyName)
    {
            ...
            if (dummySpell->Id == 53651)
            {
                ...
                if (GetTypeId() != TYPEID_PLAYER)
                {
                    ...
                    if (!beaconTarget || beaconTarget == this || !(beaconTarget->GetAura(53563, victim->GetGUID())))
                        return false;
                    float minHealval = (float)beaconTarget->GetMaxNegativeAuraModifier(SPELL_AURA_MOD_HEALING_PCT);
                    float TakenHealMod = 1.0f;

                    if (minHealval)
                        AddPct(TakenHealMod, minHealval);

                    basepoints0 = int32(damage * TakenHealMod);
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (dummySpell->Id == 49028)
            {
                ...
                Unit* owner = pPet->GetOwner();
                if (!owner || !owner->GetVictim() || !damage)
                    return false;

                if (procSpell) // Spell damage
                {
                    uint32 procDmg = damage / 2;
                    pPet->SendSpellNonMeleeDamageLog(owner->GetVictim(), procSpell->Id, procDmg, procSpell->GetSchoolMask(), 0, 0, false, 0, false);
                    pPet->DealDamage(owner->GetVictim(), procDmg, NULL, SPELL_DIRECT_DAMAGE, procSpell->GetSchoolMask(), procSpell, true);
                ...
                {
                    CalcDamageInfo damageInfo;
                    CalculateMeleeDamage(owner->GetVictim(), 0, &damageInfo, BASE_ATTACK);
                    damageInfo.attacker = pPet;
                    damageInfo.damage = damageInfo.damage / 2;
                    // Envia log de daño al cliente
                    pPet->DealDamageMods(owner->GetVictim(), damageInfo.damage, &damageInfo.absorb);
...
bool Unit::HandleAuraProc(Unit* victim, uint32 damage, Aura* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown, bool * handled)
{
    ...
    // Power Shield vs AOE
    if (!handled)
        if (dummySpell->HasAura(SPELL_AURA_MOD_STEALTH) || dummySpell->HasAura(SPELL_AURA_MOD_INVISIBILITY))
            if (procEx & PROC_EX_ABSORB && damage <= 0)
                *handled = true;
    return false;
}
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (trigger_spell_id)
    {
        ...
        case 54748:
        {
            ...
            if (!(procSpell->GetAllEffectsMechanicMask() & ((1 << MECHANIC_INTERRUPT) | (1 << MECHANIC_SILENCE))) || this == victim)
                return false;
            if (this->HasAura(54748)) // Burning Determination - [Mage]
            {
                this->RemoveAura(54748);
                return false;
            }
            break;
        }
...
void Unit::RecalculatePetsScalingDamageDonePct()
{
    for (Unit::ControlList::iterator itr = m_Controlled.begin(); itr != m_Controlled.end();)
    {
        ...
        if (unit->HasUnitTypeMask(UNIT_MASK_GUARDIAN))
            ((Guardian*)unit)->RecalculatePetScalingDamageDonePct();
    }
...
uint32 Unit::SpellDamageBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack) const
{
    ...
    if (IsSummon() && IsCharmedOwnedByPlayerOrPlayer())
        DoneAdvertisedBenefit += ((TempSummon*)this)->GetBonusDamage();
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack) const
{
    ...
    AuraEffectList const& mDmgTaken = GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN);
    for (AuraEffectList::const_iterator i = mDmgTaken.begin(); i != mDmgTaken.end(); ++i)
    {
        AuraEffect* aura = *i;
        if (!aura || !aura->GetAmount() || !(aura->GetMiscValue() & spellProto->SchoolMask))
            continue;

        // They were adding physical type spells "magic" (bleeding), so increased damage.
        if ((aura->GetMiscValue() == 1) && (spellProto->GetEffectMechanic(3) > 0))
        {
            // No physical adds "magic" with mechanics on the effects
        }
        else
            TakenTotalMod *= 1 + aura->GetBaseAmount() / 100.0f;
    }
    ...
    if (uint32 mechanicMask = spellProto->GetAllEffectsMechanicMask())
    {
        ...
        if (dmgBonusDisease != 0)
            AddPct(TakenTotalMod, dmgBonusDisease);            // apply bonus
    }
...
bool Unit::IsImmunedToSpellEffect(SpellInfo const* spellInfo, uint32 index) const
{
    ...
    // Anti-magic Shell; Inmune a magical aura effects, poisons, diseases
    if (HasAura(48707, EFFECT_0))
    {
        // Deben aplicar su efecto
        switch (spellInfo->Id)
        {
            case 7922:  // Warrior - Charge (Stun effect)
            case 20253: // Warrior - Intercept (Stun effect)
            case 45334: // Druid - Feral Charge Effect (Root)
            case 53148: // Hunter (Pet) - Charge (Root effect)
            case 70447: // ICC Volatile Ooze (Putricide) - Volatile Ooze Adhesive (10N)
            case 72836: // ICC Volatile Ooze (Putricide) - Volatile Ooze Adhesive (25N)
            case 72837: // ICC Volatile Ooze (Putricide) - Volatile Ooze Adhesive (10H)
            case 72838: // ICC Volatile Ooze (Putricide) - Volatile Ooze Adhesive (25H)
                return false;
                break;
        }
        ...
        switch ((*i)->GetSpellInfo()->SpellIconID)
        {
            ...
            case 2109:
                if ((*i)->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL)
                {
                    if (GetTypeId() != TYPEID_PLAYER)
                        continue;
                    float mod = ToPlayer()->GetRatingBonusValue(CR_CRIT_TAKEN_MELEE) * (-10.0f);
                    AddPct(TakenTotalMod, std::max(mod, float((*i)->GetAmount())));
                }
                break;
        }
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    if ((!bySpell || !(bySpell->AttributesEx6 & SPELL_ATTR6_CAN_TARGET_INVISIBLE)) && (obj ? !obj->CanSeeOrDetect(target, bySpell && bySpell->IsAffectingArea()) : !CanSeeOrDetect(target, (bySpell && bySpell->IsAffectingArea())) || (target->GetTypeId() == TYPEID_PLAYER && target->HasStealthAura() && target->IsInCombat() && IsInCombatWith(target))))
        return false;
...
bool Unit::IsVisible(ServerSideVisibilityType type) const
{
    return (m_serverSideVisibility.GetValue(type) > SEC_PLAYER) ? false : true;
}

void Unit::SetVisible(bool x, ServerSideVisibilityType type)
{
    if (type == SERVERSIDE_VISIBILITY_GM)
        m_serverSideVisibility.SetValue(type, x ? SEC_PLAYER : SEC_GAMEMASTER);
    else
    {
        m_serverSideVisibility.SetValue(type, x ? GHOST_VISIBILITY_ALIVE | GHOST_VISIBILITY_GHOST : GHOST_VISIBILITY_GHOST);
        m_serverSideVisibilityDetect.SetValue(type, x ? GHOST_VISIBILITY_ALIVE | GHOST_VISIBILITY_GHOST : GHOST_VISIBILITY_GHOST);
    }
...
void Unit::UpdateSpeed(UnitMoveType mtype, bool forced)
{
    ...
    // Guild Leveling System (Bonus: Mount Speed (Rank 1 - 2)  BonusId: 6 - 10)
    uint8 BonusMountSpeed = 0;
    if (GetTypeId() == TYPEID_PLAYER)
    {
        Player* player = ToPlayer();
    ...
    switch (mtype)
    {
        ...
        case MOVE_RUN:
        {
            ...
            {
                main_speed_mod = GetMaxPositiveAuraModifier(SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED) + BonusMountSpeed;
        ...
        case MOVE_FLIGHT:
        {
            ...
            {
                main_speed_mod = GetMaxPositiveAuraModifier(SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED) + BonusMountSpeed;
...
void Unit::SetDynCombat(bool apply)
{
    _isDynCombat = apply ? true : false;
}

bool Unit::IsDynCombat()
{
    return _isDynCombat;
}
                ...
                if (Player* player = owner->ToPlayer())
                {
                    if (player->HaveSpectators())
                    {
                        if (!player->IsSpectator() && !player->IsGameMaster())
                        {
                            SpectatorAddonMsg msg;
                            msg.SetGuid(player->GetGUID());
                            if (const CreatureTemplate* cTemp = pet->GetCreatureTemplate())
                                msg.SetPetType(cTemp->family);
                            msg.SetPetCurrentHP(pet->IsAlive() ? (uint16)pet->GetHealthPct() : 0);
                            player->SendSpectatorAddonMsgToBG(msg);
                        }
                    }
                }
...
void Unit::SetMaxHealth(uint32 val)
{
    ...
    if (Player* player = ToPlayer())
    {
        if (ToPlayer()->HaveSpectators())
        {
            if (!ToPlayer()->IsSpectator() && !ToPlayer()->IsGameMaster())
            {
                SpectatorAddonMsg msg;
                msg.SetGuid(ToPlayer()->GetGUID());
                msg.SetMaxHP(val);
                ToPlayer()->SendSpectatorAddonMsgToBG(msg);
            }
        }
...
void Unit::UpdateCharmAI()
{
    ...
    {
        if (IsCharmed() && !HasAura(30019)) // if is charmed and dont have aura 30019 (Control Piece - Chess event - Karazhan)
        {
            i_disabledAI = i_AI;
...
void Unit::ProcDamageAndSpellFor(bool isVictim, Unit* target, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellInfo const* procSpell, uint32 damage, SpellInfo const* procAura)
{
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        if (procExtra & PROC_EX_ABSORB && isVictim)
            if (damage || spellProto->Id == 53736 || spellProto->Id == 31801 || spellProto->Id == 53742 || spellProto->Id == 31803 || spellProto->Effects[EFFECT_0].TriggerSpell || spellProto->Effects[EFFECT_1].TriggerSpell || spellProto->Effects[EFFECT_2].TriggerSpell)
                active = true;
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (this->IsPet() && this->ToPet()->GetOwner()->GetTypeId() == TYPEID_PLAYER)
        Player* player = this->ToPet()->GetOwner();
    ...
    if (player && this != victim)
    {
        ...
        if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldByZoneId(player->GetZoneId()))
            bf->HandlePlayerKill(player, victim);
    }
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    if (charmer->GetTypeId() == TYPEID_PLAYER && (aurApp && aurApp->GetBase() && aurApp->GetBase()->GetId() != 30019))
    {
    ...
    {
        InitCharmInfo();
        if (aurApp && aurApp->GetBase() && aurApp->GetBase()->GetId() == 30019)
...
void Unit::ApplyResilience(Unit const* victim, float* crit, int32* damage, bool isCrit, CombatRating type) const
{
    ...
    switch (type)
    {
        case CR_CRIT_TAKEN_MELEE:
            ...
            if (source && damage)
            {
                if (source->getClass() == CLASS_WARRIOR) // Custom
                {
                    if (isCrit)
...
float Unit::MeleeSpellMissChance(const Unit* victim, WeaponAttackType attType, int32 skillDiff, uint32 spellId) const
{
    ...
    if (victim->GetTypeId() == TYPEID_PLAYER)
        missChance += diff > 0 ? diff * 0.02f : diff * 0.01f;
    else
...
void Unit::KnockbackFrom(float x, float y, float speedXY, float speedZ)
{
    ...
    {
        // Bladestorm
        if (player->HasAura(46924) || player->HasAura(51690))
           return;
...
void Unit::_ExitVehicle(Position const* exitPosition)
{
    ...
    vehicle->ApplyAllImmunities(); // Y ahora devolvemos el STATE_ROOT de algunos
