...
enum UnitState
{
    ...
    //UNIT_STATE_TIMED_EVADE     = 0x00800000,
...
class Unit : public WorldObject
{
    public:
        ...
        // player or player's pet resilience (-1%)
        uint32 GetMeleeWarriorCritDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_MELEE, 2.68f, 40.6f, damage); } // Warrior Nerferd =D
        uint32 GetMeleeCritDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_MELEE, 2.3f, 34.3f, damage); }
        uint32 GetRangedCritDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_RANGED, 2.4f, 34.3f, damage); }
        uint32 GetSpellCritDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_SPELL, 2.4f, 34.3f, damage); }

        // player or player's pet resilience (-1%), cap 100%
        uint32 GetMeleeWarriorDamageReduction(uint32 damage) const { return GetCombatRatingDamageReduction(CR_CRIT_TAKEN_MELEE, 2.34f, 100.0f, damage); }
        ...
        // Visibility system
        bool IsVisible(ServerSideVisibilityType type = SERVERSIDE_VISIBILITY_GM) const;
        void SetVisible(bool x, ServerSideVisibilityType type = SERVERSIDE_VISIBILITY_GM);
