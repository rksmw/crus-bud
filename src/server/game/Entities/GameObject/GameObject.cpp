            ...
            switch (GetGoType())
            {
                case GAMEOBJECT_TYPE_TRAP:
                {
                    ...
                    if (goInfo->trap.type == 2)
                        m_cooldownTime = time(NULL) + 50;   // Hardcoded tooltip value
                ...
                if (goInfo->type == GAMEOBJECT_TYPE_TRAP)
                {
                    ...
                    if (ok)
                    {
                        ...
                        if (goInfo->trap.spellId)
                        {
                            ...
                            {
                                ok->ToPlayer()->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
                                ok->ToPlayer()->RemoveAurasByType(SPELL_AURA_MOD_INVISIBILITY);
                            }
                        ...
                        if (goInfo->trap.type == 1)
                            SetLootState(GO_JUST_DEACTIVATED);
                        // Makes taking casts shadow sight, causing you to lose stealth and invisibility &
                        // If this did not mounted or dismounted kicks mistake.
                        switch (goInfo->trap.spellId)
                        {
                            //Power-Ups Battleground / Arenas
                            case 23451: //Speed Buff
                            case 23493: //Food Buff
                            case 23505: //Berserk Buff
                            case 34709: //Shadow sight arena spell
                                if (!ok->IsMounted())
                                    ok->CastSpell(ok, goInfo->trap.spellId);
                                break;
                            default:
                                break;
                        }
...
bool GameObject::ActivateToQuest(Player* target) const
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_CHEST:
        {
            ...
            {
                /// @todo fix this hack
                // buscar battlegroundAV para algunos objetos que sólo se activan después de gots mineros capturados por el propio equipo
                if (GetEntry() == BG_AV_OBJECTID_MINE_N || GetEntry() == BG_AV_OBJECTID_MINE_S)
                    if (Battleground* bg = target->GetBattleground())
                        if (bg->GetTypeID(true) == BATTLEGROUND_AV && !(((BattlegroundAV*)bg)->CanActivateGO(GetEntry(), target->GetTeam())))
                            return false;
                return true;
            }
            break;
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
        {
            ...
            if (user->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if (Battlefield* bf = player->GetBattlefield())
                    bf->EventPlayerUsedGO(player, this);
    ...
    switch (state)
    {
        ...
        case GO_DESTRUCTIBLE_DAMAGED:
        {
            ...
            if (eventInvoker)
            {
                ...
                if (Battlefield* bf = eventInvoker->GetBattlefield())
                    bf->EventPlayerDamagedGO(eventInvoker, this, m_goInfo->building.damagedEvent);
            }
        ...
        case GO_DESTRUCTIBLE_DESTROYED:
        {
            ...
            if (eventInvoker)
            {
                ...
                if (Battlefield* bf = eventInvoker->GetBattlefield())
                    bf->EventPlayerDamagedGO(eventInvoker, this, m_goInfo->building.destroyedEvent);
            }
