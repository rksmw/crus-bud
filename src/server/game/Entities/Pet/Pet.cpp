...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current)
{
    ...
    else
    {
        ...
        uint32 savedfocus = fields[12].GetUInt32();
        uint32 savedenergy = fields[13].GetUInt32();
        ...
        else
        {
            ...
            SetPower(POWER_FOCUS, savedfocus > GetMaxPower(POWER_FOCUS) ? GetMaxPower(POWER_FOCUS) : savedfocus);
            SetPower(POWER_ENERGY, savedenergy > GetMaxPower(POWER_ENERGY) ? GetMaxPower(POWER_ENERGY) : savedenergy);
        }
    }
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    switch (petType)
    {
        ...
        default:
        {
            switch (GetEntry())
            {
                case 37994: // Mage Water Elemental (pet glyph of eternal water)
                case 510:   // Mage Water Elemental
                {
                    if (Player *owner = m_owner->ToPlayer())
                    {
                        SetCreateHealth(uint32(owner->GetMaxHealth() / 2));
                        SetCreateMana(uint32(owner->GetMaxPower(POWER_MANA) / 2));
                    }
                    SetModifierValue(UNIT_MOD_ARMOR, BASE_VALUE, float(m_owner->GetArmor()) * 0.35f);  //  Bonus Armor (35% of player armor)
                    SetBonusDamage(int32(GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_FROST) * 0.4f));
                    break;
                }
                ...
                case 19668: // Shadowfiend
                {
                    ...
                    {
                        SetCreateHealth(uint32(owner->GetMaxHealth() / 2));
                        SetCreateMana(uint32(owner->GetMaxPower(POWER_MANA) / 1.5));
                    }
                ...
                {
                    if (Player *owner = m_owner->ToPlayer()) // get 100% of owner player's haste
                    {
                        ...
                        //SetCreateHealth(uint32(owner->GetMaxHealth() * 0.8)); // hp must be 0.8x of DK hp - dont remove comment some multiplicator already scale gargyole health
                    }
                    float attackPower = 0.0f;
                    attackPower += int32(GetOwner()->GetTotalAttackPowerValue(BASE_ATTACK) * 0.2f);
                    attackPower += int32(GetOwner()->GetTotalAuraModifier(SPELL_AURA_MOD_ATTACK_POWER));
                    SetBonusDamage(attackPower);
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel - (petlevel / 4)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel + (petlevel / 4)));
                    if (!HasAura(61697))       // Death Knight Pet Scaling 03
                        AddAura(61697, this);
                ...
                case 27893: // Dancing Rune Weapon
                {
                    ...
                    if (!HasAura(51906))   // Death Knight Rune Weapon Scaling 02
                        AddAura(51906, this);
                    break;
                }
                case 26125:  // Risen Ghoul
                {
                    if (!HasAura(54566))       // Death Knight Pet Scaling 01
                        AddAura(54566, this);
                    if (!HasAura(51996))       // Death Knight Pet Scaling 02
                        AddAura(51996, this);
                    if (!HasAura(61697))       // Death Knight Pet Scaling 03
                        AddAura(61697, this);
                    if (!HasAura(62137))       // Avoidance
