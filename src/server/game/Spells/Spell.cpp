    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        switch (m_spellInfo->Id)
        {
            ...
            case 30449: // Spell Steal
                if (target->GetTypeId() == TYPEID_PLAYER && target->HasAura(8178))
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 51328:
            ...
            if (!(m_targets.GetUnitTarget() && m_targets.GetUnitTarget()->GetDisplayId() == m_targets.GetUnitTarget()->GetNativeDisplayId() &&
                (((m_targets.GetUnitTarget()->GetEntry() == 26125 || m_targets.GetUnitTarget()->GetEntry() == 27829) && m_targets.GetUnitTarget()->GetOwnerGUID() == m_caster->GetGUID())
...
void Spell::SelectImplicitCasterDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    switch (targetType.GetTarget())
    {
        ...
        case TARGET_DEST_CASTER_FISHING:
        {
             ...
             ZLiquidStatus status = m_caster->GetMap()->getLiquidStatus(x, y, z, MAP_ALL_LIQUIDS, &liquidStatus);
             float liquidLevel = liquidStatus.level;
             if (liquidLevel > z + 0.75f)
             {
    ...
    switch (targetType.GetTarget())
    {
        ...
        case TARGET_DEST_CASTER_FRONT_RIGHT:
            m_caster->GetFirstCollisionPosition(pos, dist + 1.0f, angle, true);
            break;
        default:
...
void Spell::AddGOTarget(GameObject* go, uint32 effectMask)
{
    ...
    if (m_spellInfo->Speed > 0.0f)
    {
        ...
        if (dist < 5.0f)
            dist = 5.0f;
        target.timeDelay = uint64(floor(dist / m_spellInfo->Speed * 1150.0f));
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if (unit && unit->ToPlayer())
        if (Aura* aura = unit->GetAura(26888))
            if (uint32(aura->GetDuration()) < 9840)
    ...
    {
            if ((m_spellInfo->AttributesCu & SPELL_ATTR0_CU_CANT_FADED) && missInfo == SPELL_MISS_NONE) // Can't break vanish when missed
            {      
    ...
    else if (!(m_spellInfo->AttributesCu & SPELL_ATTR0_CU_CANT_FADED))
                    return;
    ...
    else if (missInfo == SPELL_MISS_REFLECT)                // In case spell reflect from target, do all effect on caster (if hit)
    {
        if (target->reflectResult == SPELL_MISS_NONE)       // If reflected spell hit caster -> do all effect on him
        {
            ...
            m_caster->ProcDamageAndSpell(unitTarget, PROC_FLAG_NONE, PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_NEG, PROC_EX_REFLECT, 1, BASE_ATTACK, m_spellInfo); // See here
    ...
    else if (m_damage > 0)
    {
        ...
        if (missInfo == SPELL_MISS_NONE && HitTarget)
        {
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    if (m_caster != unit)
    {
        ...
        if (m_caster->_IsValidAttackTarget(unit, m_spellInfo))
        {
            ...
            if (!((m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_NORMAL) || (m_spellInfo->SchoolMask & SPELL_SCHOOL_MASK_HOLY)) && !m_spellInfo->IsPositive() && !m_caster->IsTotem())
            {
                float resistChance = unit->GetResistance(SpellSchoolMask(m_spellInfo->SchoolMask));
                resistChance += float(unit->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_TARGET_RESISTANCE, m_spellInfo->SchoolMask));
                ...
                if (resistChance && m_spellInfo->AttributesCu & SPELL_ATTR0_CU_CAN_RESIST)
                {
                    if ((m_caster->IsPet() || m_caster->IsGuardian()) && m_caster->GetOwner()->GetTypeId() == TYPEID_PLAYER)
                        resistChance -= float(m_caster->GetOwner()->GetSpellPenetration(SpellSchoolMask(m_spellInfo->SchoolMask)));

                    if (m_caster->GetTypeId() == TYPEID_PLAYER) // Creatures dont have Spell Penetration
                        resistChance -= float(m_caster->GetSpellPenetration(SpellSchoolMask(m_spellInfo->SchoolMask)));

                    resistChance = float(resistChance * 10.0f); // Resist Chance Formular 130 Resist -> 18%
    ...
    //        {
    //            if ((m_caster->IsPet() || m_caster->IsGuardian()) && m_caster->GetOwner()->GetTypeId() == TYPEID_PLAYER)
    //                resistChance -= float(m_caster->GetOwner()->ToPlayer()->GetSpellPenetrationItemMod() + m_caster->GetOwner()->ToPlayer()->GetSpellPenetration(SpellSchoolMask(m_spellInfo->SchoolMask)) * 2.5f);

    //            if (m_caster->GetTypeId() == TYPEID_PLAYER) // Creatures dont have Spell Penetration
    //                resistChance -= float(m_caster->ToPlayer()->GetSpellPenetrationItemMod() + m_caster->ToPlayer()->GetSpellPenetration(SpellSchoolMask(m_spellInfo->SchoolMask)) * 2.5f);

    //            if (resistChance > 59)
    //            {
    //                resistChance = float((resistChance / 128.0f) * 1000.0f); // Resist Chance Formular 130 Resist -> 18%

    //                if (resistChance > 10000) // Resist can't be higher than 100%
    //                    return SPELL_MISS_RESIST;

    //                if (resistChance && resistChance > irand(1,10000))
    //                    return SPELL_MISS_RESIST;
    //            }
    //        }
    //    }
...
void Spell::cast(bool skipCheck)
{
    ...
    if (((m_spellInfo->Speed > 0.0f || GetCCDelay(m_spellInfo) > 0) && !m_spellInfo->IsChanneled()) || m_spellInfo->AttributesEx4 & SPELL_ATTR4_UNK4)
    {
...
void Spell::handle_immediate()
{
    ...
    if (m_spellState != SPELL_STATE_CASTING)
    {
        ...
        {
            if (Player* tempPlayer = m_caster->ToPlayer())
            {
                if (tempPlayer->HaveSpectators() && !tempPlayer->IsSpectator() && !tempPlayer->IsGameMaster() && m_spellInfo->CastTimeEntry && m_spellInfo->CastTimeEntry->CastTime && !m_spellInfo->IsChanneled())
                {
                    SpectatorAddonMsg msg;
                    msg.SetGuid(tempPlayer->GetGUID());
                    msg.CastSpell(m_spellInfo->Id, SPECTATOR_ADDON_SPELL_OK);
...
uint64 Spell::handle_delayed(uint64 t_offset)
{
    ...
    if (next_time == 0)
    {
        ...
        if (m_caster && m_spellInfo && m_spellInfo->IsChanneled())
        {
            if (Player* tempPlayer = m_caster->ToPlayer())
            {
...
void Spell::finish(bool ok)
{
        ...
        if (m_caster && GetSpellInfo())
        {
            if (Player* tempPlayer = m_caster->ToPlayer())
            {
                if (tempPlayer->HaveSpectators() && !tempPlayer->IsSpectator() && !tempPlayer->IsGameMaster())
                {
                    SpectatorAddonMsg msg;
                    msg.SetGuid(tempPlayer->GetGUID());
                    msg.CastSpell(GetSpellInfo()->Id, SPECTATOR_ADDON_SPELL_INTERRUPTED);
                    tempPlayer->SendSpectatorAddonMsgToBG(msg);
                }
            }
        }
    ...
    if (IsAutoActionResetSpell())
    {
        ...
        {
            m_caster->resetAttackTimer(BASE_ATTACK, true);
            if (m_caster->haveOffhandWeapon())
                m_caster->resetAttackTimer(OFF_ATTACK, true);
            m_caster->resetAttackTimer(RANGED_ATTACK, true);
        }
    }
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (m_spellInfo->Id != 42292 && m_spellInfo->Id != 65547)
    {
    ...
    // Can't cast DeathGrip while jump / fleeing and can't target NOT IN LOS targets
    if (m_spellInfo->Id == 49576 && (m_caster->IsFalling() || (m_targets.GetUnitTarget() && !m_caster->IsWithinLOSInMap(m_targets.GetUnitTarget()))))
        return SPELL_FAILED_BAD_TARGETS;

    // Hand of Reckoning don't cause damage to players or
    if (m_spellInfo->Id == 67485 && (m_targets.GetUnitTarget()->GetTypeId() == TYPEID_PLAYER || (m_targets.GetUnitTarget()->GetVictim() && m_targets.GetUnitTarget()->GetVictim()->GetGUID() == m_caster->GetGUID())))
        return SPELL_FAILED_DONT_REPORT;
    ...
    if (SpellDestination const* target = m_targets.GetDst())
    {
        float targetx = target->_position.GetPositionX();
        float targety = target->_position.GetPositionY();
        float targetz = target->_position.GetPositionZ();
    ...
    // Fix pet Dynamic Combat
    if (Unit* _target = m_targets.GetUnitTarget())
        if (_target != m_caster && _target->GetTypeId() == TYPEID_PLAYER && _target->ToPlayer()->IsDynCombat() && (_target->getClass() == CLASS_WARLOCK || _target->getClass() == CLASS_DEATH_KNIGHT) && m_spellInfo->Id == 51724)
            return SPELL_FAILED_AFFECTING_COMBAT;
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_CHARGE:
            {
                ...
                if (GetSpellInfo()->NeedsExplicitUnitTarget())
                {
                    ...
                    Position pos;
                    // INNECESARIO, sobreescrito por GetPosition en GetFirstCollision
                    ...
                    *  I know, it's hard hackfix, replace OFFMESH con.
                    */
                    if (GetSpellInfo()->Id == 54216) // Master's call Charging Pet Range wrong
                        m_preGeneratedPath.SetPathLengthLimit(m_spellInfo->GetMaxRange(true) * 20.00f); // 20Y
                    if (m_caster->GetMapId() == 562 && m_caster->GetPositionZ() > 9.0f)
                        m_preGeneratedPath.SetPathLengthLimit(m_spellInfo->GetMaxRange(true) * 20.00f); // 20Y
            ...
            case SPELL_EFFECT_SUMMON:
            {
                ...
                switch (SummonProperties->Category)
                {
                    case SUMMON_CATEGORY_PET:
                        if (m_caster->GetPetGUID() && m_spellInfo->Id != 1122)
                            return SPELL_FAILED_ALREADY_HAVE_SUMMON;
            ...
            {
                if (m_caster->GetPetGUID() && m_spellInfo->Id != 1122)                  //let warlock do a replacement summon
                {
            ...
            case SPELL_EFFECT_TALENT_SPEC_SELECT:
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    {
                        plr->MonsterWhisper("BattleCore ERROR: No puedes cambiar talentos mientras estas en lista de espera para 1v1 o 3v3", plr);
                        return SPELL_FAILED_DONT_REPORT;
                    }
        ...
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            ...
            case SPELL_AURA_AOE_CHARM:
            {
                ...
                if (m_spellInfo->Effects[i].ApplyAuraName == SPELL_AURA_MOD_CHARM
                    || m_spellInfo->Effects[i].ApplyAuraName == SPELL_AURA_MOD_POSSESS || m_spellInfo->Id != 1122)
                {
            ...
            case SPELL_AURA_MOD_INCREASE_MOUNTED_FLIGHT_SPEED:
            {
                ...
                {
                    Battlefield* bf = m_originalCaster->ToPlayer()->GetBattlefield();
                    if (AreaTableEntry const* pArea = GetAreaEntryByAreaID(m_originalCaster->GetAreaId()))
                    {
                        if ((pArea->flags & AREA_FLAG_NO_FLY_ZONE) || (bf && bf->IsBattleTime()))
...
uint32 Spell::GetCCDelay(SpellInfo const* spell)
{
    ...
    case SPELLFAMILY_MAGE:
        if (spell->SpellIconID == 1548)             // Dragon's Breath.
            return 63;
        if (spell->Id == 33395)                     // Water Elemental Freeze.
            return 63;
    ...
    case SPELLFAMILY_WARLOCK:
        if (spell->SpellIconID == 88)           // Death Coil.
            return 63;
        if (spell->SpellIconID == 77)           // Spell Lock.
            return 63;
        if (spell->Id == 22703)                 // Summon Inferno Stun.
            return 63;
    ...
    case SPELLFAMILY_DRUID:
        if (spell->SpellIconID == 495)          // Pounce.
            return 63;
    ...
    case SPELLFAMILY_GENERIC:
        ...
        if (spell->Id == 6358)      // Seduction.
            return 63;
        if (spell->Id == 8326 || spell->Id == 20584) // Ghost - These spells are not positive and are delayed with the hack below
            return 0;
        /*if (spell->Id == 42292)   // pvp trinket
            return 0;*/
        break;
    }
    return 0;
