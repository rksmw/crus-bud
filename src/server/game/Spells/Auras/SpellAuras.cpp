    ...
    for (std::map<Unit*, uint8>::iterator itr = targets.begin(); itr!= targets.end();)
    {
        ...
        if (addUnit)
        {
            ...
            else // (GetType() == UNIT_AURA_TYPE)
            {
                ...
                // Prevents exploit dual stack auras.
                else if (caster)
                {
                    for (Unit::AuraApplicationMap::iterator iter = itr->first->GetAppliedAuras().begin(); iter != itr->first->GetAppliedAuras().end(); ++iter)
                    {
...
void Aura::UnregisterSingleTarget()
{
    ...
    if (!caster) //ASSERT(caster);
    {
        TC_LOG_ERROR(LOG_FILTER_GENERAL, "Preventing crash at Aura::UnregisterSingleTarget() [Guid: %u SpellId: %u]", GetCasterGUID() ? GetCasterGUID() : 0, GetId());
        return;
    }
...
void Aura::HandleAuraSpecificMods(AuraApplication const* aurApp, Unit* caster, bool apply, bool onReapply)
{
    ...
    if (apply)
    {
        if ((m_spellInfo->AttributesCu & SPELL_ATTR0_CU_DONT_BREAK_STEALTH) && target->IsControlledByPlayer())
