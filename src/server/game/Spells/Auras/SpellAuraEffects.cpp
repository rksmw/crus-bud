        ...
        case SPELL_AURA_PERIODIC_DUMMY:
            switch (GetSpellInfo()->SpellFamilyName)
            {
                case SPELLFAMILY_GENERIC:
                    switch (GetId())
                    {
                        case 30019: // Control Piece (Chess event - Karazhan)
                        {
                            ...
                            if (!caster->GetCharm())
                            {
                                ...
                                caster->RemoveAurasDueToSpell(30532);
                            }
                            return;
                        }
                        // Drink
            ...
            switch (GetId())
            {
                ...
                case 52916: // Honor Among Thieves
                    if (target->GetTypeId() == TYPEID_PLAYER)
                        ...
                        {
                            float visibilityRange = target->GetMap()->GetVisibilityRange();
                            std::list<Unit*> spellTargets;
                            Trinity::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target, visibilityRange);
                            Trinity::UnitListSearcher<Trinity::AnyUnfriendlyUnitInObjectRangeCheck> searcher(target, spellTargets, u_check);
                            target->VisitNearbyObject(visibilityRange, searcher);
                            for (UnitList::iterator iter = spellTargets.begin(); iter != spellTargets.end(); ++iter)
                                if ((*iter)->HasAura(48672) || (*iter)->HasAura(48676))
                                    target->CastSpell((*iter), 51699, true);
                        }
                        break;
                case 71563:
                    ...
                    switch (GetId())
                    {
                        case 2584: // Waiting to Resurrect
                            ...
                            if (target->GetTypeId() == TYPEID_PLAYER)
                            {
                                ...
                                if (Battlefield* bf = target->ToPlayer()->GetBattlefield())
                ...
                case 42994: // Ram - Gallop
                {
                    ...
                    if (GetBase()->GetDuration() <= 892000)
                    {
                            ...
                            caster->ToPlayer()->GetQuestStatus(11409) == QUEST_STATUS_INCOMPLETE)
                        {
                            ...
                            {
                            case 42992:
                                caster->ToPlayer()->KilledMonsterCredit(24263, 0);
                                break;
                            case 42993:
                                caster->ToPlayer()->KilledMonsterCredit(24264, 0);
                                break;
                            case 42994:
                                caster->ToPlayer()->KilledMonsterCredit(24265, 0);
                                break;
                            }
                        }
                    }
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        damage = std::max(GetAmount() + GetBonusAmount(), 0); // if isAreaAura == true, GetBonusAmount == 0. 
    ...
    // Fix avoidance on pets (90% damage reduction)
    if (damage > 0 && target && GetCaster())
    {
