...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* /*target*/) const
{
    ...
    if (caster)
    {
        ...
        if (!_spellInfo->IsPassive()) // no estoy seguro si es bueno para todos los casos, fix de Magic Absorption
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        switch (Effects[i].ApplyAuraName)
        {
            ...
            case SPELL_AURA_FLY:
            {
                if (player)
                {
                    if (zone_id != 4395) // Everything except Dalaran
                    {
                        Battlefield* bf = player->GetBattlefield();
                        AreaTableEntry const* pArea = GetAreaEntryByAreaID(area_id);

                        if (!pArea || (pArea->flags & AREA_FLAG_NO_FLY_ZONE) || (bf && bf->IsBattleTime()))
                            return SPELL_FAILED_INCORRECT_AREA;
                    }
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
    ...
    //Chequeo para ver si el source puede ver al target, spells como blood boil o fan of knives
   ...
   //Chequeo para spells como fire nova.
   if ((Effects[0].TargetA.GetTarget() == TARGET_DEST_DEST) && (Effects[0].TargetB.GetTarget() == TARGET_UNIT_DEST_AREA_ENEMY))
       if (!caster->IsWithinLOSInMap(target))
           return SPELL_FAILED_LINE_OF_SIGHT;

   //Chequeo para ver si el source puede ver al target, spells como algunos totem y whirlwind
...
bool SpellInfo::CheckTargetCreatureType(Unit const* target) const
{
    ...
    // if target is magnet (currently grounding totem needs this) the skip check
    if (target->IsMagnet())
        return true;
...
uint32 SpellInfo::CalcCastTime(Spell* spell /*= NULL*/) const
{
    ...
    if (spell && castTime > 10 && spell->GetCaster()->GetTypeId() == TYPEID_PLAYER)
        if (Battleground *bg = spell->GetCaster()->ToPlayer()->GetBattleground())
            if (bg->isArena() && bg->GetStartDelayTime() > 100)
                return 10;

    if (Attributes & SPELL_ATTR0_REQ_AMMO && (!IsAutoRepeatRangedSpell()))
        castTime += 500;

    // Throw Love Rocket castTime hack
    if (spell && spell->GetSpellInfo()->Id == 45153)
        castTime = -1;
