...
void Spell::EffectSchoolDMG(SpellEffIndex effIndex)
{
    ...
    if (unitTarget && unitTarget->IsAlive())
    {
        ...
        if (m_spellInfo->Id == 47480) // Thrash (Risen Ally - DK)
        {
            Aura* Frenzy = m_caster->GetAura(62218);
            if (!Frenzy)
                return;

            uint32 HasCharges = Frenzy->GetCharges();
            damage = int32((0.22 * m_caster->GetOwner()->GetTotalAttackPowerValue(BASE_ATTACK)) * HasCharges);
        }
            ...
            case SPELLFAMILY_GENERIC:
            {
                ...
                switch (m_spellInfo->Id)                     // better way to check unknown
                {
                    ...
                    case 51963:
                    {
                        ...
                        damage = (m_caster->getLevel() - 55) * 4 + 75;
                        damage += m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.4;
                        break;
                    }
                    ...
                    if (Player* player = m_caster->ToPlayer())
                    {
                        ...
                        if (uint32 combo = player->GetComboPoints())
                        {
                            ...
                            if (AuraEffect const* aurEff = unitTarget->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_ROGUE, 0x00010000, 0, 0, m_caster->GetGUID()))
                            {
                                ...
                                bool needConsume = !m_caster->HasAura(64199); // Fix Glyph of Envenom 
...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARLOCK: 
            switch (m_spellInfo->Id)
            {
                case 19028: // Soul Link - Alert see here
                {
    ...
    switch (m_spellInfo->Id)
    {
        case 32312: // Move 1 (Karazhan - Chess event AI short distance move)
        case 37144: // Move   (Karazhan - Chess event player knight move)
        case 37146: // Move   (Karazhan - Chess event player pawn move)
        case 37148: // Move   (Karazhan - Chess event player queen move)
        case 37151: // Move   (Karazhan - Chess event player rook move)
        case 37152: // Move   (Karazhan - Chess event player bishop move)
        case 37153: // Move   (Karazhan - Chess event player king move)
        case 37388: // Move 2 (Karazhan - Chess event AI long distance move)
        {
        ...
        case 30012: // Move (Karazhan - Chess event)
        {
        ...
        && effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH_TARGET)
    {
        ...
        switch (triggered_spell_id)
        {
            ...
            case 18461:
            {
                ...
                // Don't leave combat if you are in combat with a boss
            ...
            case 35729:
            {
                ...
                for (Unit::AuraApplicationMap::iterator iter = Auras.begin(); iter != Auras.end();)
                {
                    ...
                    if (((spell->DmgClass == SPELL_DAMAGE_CLASS_MAGIC && spell->GetSchoolMask() != SPELL_SCHOOL_MASK_NORMAL) // only affect magic spells
                        ...
                        // We ignore Unbound Plague
                        && spell->Id != 72855 && spell->Id != 72856)
                    {
...
void Spell::EffectJumpDest(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->Id == 49575) // Death Grip
        m_caster->GetMotionMaster()->MoveJump(x, y, z, 64.0f, 3.0f);
    else
    {
        float speedXY, speedZ;
        CalculateJumpSpeeds(effIndex, m_caster->GetExactDist2d(x, y), speedXY, speedZ);
...
void Spell::CalculateJumpSpeeds(uint8 i, float dist, float & speedXY, float & speedZ)
{
    if (m_spellInfo->Effects[i].MiscValue)
        speedZ = float(m_spellInfo->Effects[i].MiscValue)/2.5f;
    else if (m_spellInfo->Effects[i].MiscValueB)
        speedZ = float(m_spellInfo->Effects[i].MiscValueB)/30;
    else
        speedZ = 4.0f;
    ...
    if (m_spellInfo->Effects[i].ValueMultiplier)
        multip = m_spellInfo->Effects[i].ValueMultiplier;
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    if (!m_targets.HasDst())
    {
        if (m_spellInfo->Id == 30532) // In Game (Karazhan - Chess event)
        {
            if (unitTarget->GetMapId() == 532 && unitTarget->GetTypeId() == TYPEID_PLAYER) // Karazhan
            {
                unitTarget->ToPlayer()->TeleportTo(532, -11107.200f, -1842.880f, 229.707f, 5.378f, TELE_TO_NOT_LEAVE_COMBAT | TELE_TO_SPELL);
                return;
            }
        }
...
void Spell::EffectHeal(SpellEffIndex /*effIndex*/)
{
    ...
    if (unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
        ...
        //// Heal de lifebloom - explosion aumentara spellpower coef por cada stack de lifebloom - periodico
        //else if (m_spellInfo->Id == 33778)
        //    addhealth = caster->SpellHealingBonusDone(unitTarget, m_spellInfo, addhealth, HEAL, m_spellValue->EffectBasePoints[1]);
...
void Spell::EffectHealthLeech(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->Id == 20004 && m_caster->GetTypeId() == TYPEID_PLAYER)
        m_caster->ToPlayer()->AddSpellCooldown(20004, 0, time(NULL) + 8);
}
...
void Spell::EffectDispel(SpellEffIndex effIndex)
{
    ...
    for (int32 count = 0; count < damage && !dispel_list.empty();)
    {
        ...
        if (!chance)
        {
            ...
            // When resist resisted, return info
...
void Spell::EffectSummonPet(SpellEffIndex effIndex)
{
    ...
    if (OldSummon)
    {
        ...
        Unit::AuraApplicationMap& appliedAuras = OldSummon->GetAppliedAuras();
...
void Spell::EffectWeaponDmg(SpellEffIndex effIndex)
{
    ...
    // Melee attack always break stealth
...
void Spell::EffectScriptEffect(SpellEffIndex effIndex)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            switch (m_spellInfo->Id)
            {
                case 37142: // Karazhan - Chess NPC Action: Melee Attack: Conjured Water Elemental (Alliance)
                case 37143: // Karazhan - Chess NPC Action: Melee Attack: Human Charger (Alliance)
                case 37147: // Karazhan - Chess NPC Action: Melee Attack: Human Cleric (Alliance)
                case 37149: // Karazhan - Chess NPC Action: Melee Attack: Human Conjurer (Alliance)
                case 37150: // Karazhan - Chess NPC Action: Melee Attack: King Llane (Alliance)
                case 32227: // Karazhan - Chess NPC Action: Melee Attack: Human Footman (Alliance)
                ...
                case 32228: // Karazhan - Chess NPC Action: Melee Attack: Orc Grunt
                {
                    if (!unitTarget)
                        break;
...
void Spell::EffectDismissPet(SpellEffIndex effIndex)
{
    ...
    // 61669 13159 49071 Aspect Stacking
    if (pet->GetOwner()->getClass() == CLASS_HUNTER)
    {
        pet->RemoveAurasDueToSpell(61669);
        pet->RemoveAurasDueToSpell(13159);
        pet->RemoveAurasDueToSpell(49071);
    }
...
void Spell::EffectLeap(SpellEffIndex /*effIndex*/)
{
    ...
    Position destFinal;
    // Distancia de movimiento maxima permitida
    float dist = unitTarget->GetDistance(destTarget->GetPositionX(), destTarget->GetPositionY(), destTarget->GetPositionZ());
    // Procesamos el destino por collision
    unitTarget->GetFirstCollisionPosition(destFinal, dist, 0.0f, true); // En linea recta ignore zLOS

    // Si la distancia final es mayor que la maxima permitida
    if (unitTarget->GetDistance(destFinal.GetPositionX(), destFinal.GetPositionY(), destFinal.GetPositionZ()) > dist)
    {
        // Procesamos el destino para la collision final, aceptable para este caso
        unitTarget->GetFirstCollisionPosition(destFinal, dist, 0.0f, false); // Con zLOS (Asi dejamos en el borde del puente)
        // destFinal jamas conseguira superar el valor de distancia de dist, asique no lo comprobamos
    }
    else
    {
        // Si se intenta subir en terminos de la coordenada Z y se puede crear una ruta de MMAPS
        if (destFinal.GetPositionZ() >= unitTarget->GetPositionZ() + 1.0f /*(1.0f de tolerancia)*/ && MMAP::MMapFactory::IsPathfindingEnabled(unitTarget->GetMapId()) && (!unitTarget->ToCreature() || !unitTarget->ToCreature()->IsIgnorePathfinding()))
        {
            PathGenerator path(unitTarget);
            if (!path.CalculatePath(destFinal.GetPositionX(), destFinal.GetPositionY(), destFinal.GetPositionZ()) || (path.GetPathType() & (PATHFIND_NOPATH | PATHFIND_INCOMPLETE)))
            {
                // Procesamos el destino para la collision final, aceptable para este caso
                unitTarget->GetFirstCollisionPosition(destFinal, dist, 0.0f, false); // Con zLOS (Asi dejamos en el borde de una muralla, que tenia una superficie arriba)
                // destFinal jamas conseguira superar el valor de distancia de dist, asique no lo comprobamos
            }
        }
    }

    unitTarget->NearTeleportTo(destFinal.GetPositionX(), destFinal.GetPositionY(), destFinal.GetPositionZ(), unitTarget->GetOrientation(), unitTarget == m_caster);
}
...
void Spell::EffectChargeDest(SpellEffIndex /*effIndex*/)
{
    ...
    if (m_targets.HasDst())
    {
        ...
        // Cambiamos el sentido de obtencion de la posicion con el objeto de obtener una "mas" correcta
        //float dist = m_caster->GetDistance(pos);
        //m_caster->GetFirstCollisionPosition(pos, dist, angle);
        m_caster->MovePositionToFirstCollision(pos, CONTACT_DISTANCE, angle);
