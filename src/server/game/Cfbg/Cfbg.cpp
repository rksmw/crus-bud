            ...
            if (Player* pPlayer = ObjectAccessor::FindPlayer(itr->first))
            {
                if (GetDistance2d(pPlayer->GetPositionX(), pPlayer->GetPositionY()) <= distance)
                {
                    ...
                    if (GetTeam() == pPlayer->GetTeam())
                        ChatHandler::BuildChatPacket(data, (ChatMsg)msgtype, LANG_UNIVERSAL, this, pPlayer, message);
...
void Player::MorphFit(bool value)
{
    if (!IsPlayingNative() && value)
    {
        if (GetOTeam() == HORDE)
        {
            ...
            SetDisplayId(displayids[rand]);
            SetNativeDisplayId(displayids[rand]);
        }
        else
        {
...
bool BattlegroundQueue::FillXPlayersToBG(BattlegroundBracketId bracket_id, Battleground* bg, bool start)
{
    ...
    for (GroupsQueueType::const_iterator itr = m_QueuedGroups[bracket_id][BG_QUEUE_MIXED][BG_QUEUE_GUILD_FALSE].begin(); itr != m_QueuedGroups[bracket_id][BG_QUEUE_MIXED][BG_QUEUE_GUILD_FALSE].end(); ++itr)
