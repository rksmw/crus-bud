...
namespace Trinity
{
    ...
    struct PlayerDistWorker
    {
        ...
        {
            for (PlayerMapType::iterator itr=m.begin(); itr != m.end(); ++itr)
                if (itr->GetSource()->InSamePhase(i_searcher) && itr->GetSource()->IsWithinDist(i_searcher, i_dist) && i_searcher->IsEventVisibleFor(itr->GetSource()))
                    i_do(itr->GetSource());
        }
    ...
    */
    ...
    class NearestCreatureEntryWithLiveStateInObjectRangeCheck
    {
        public:
            ...
            bool operator()(Creature* u)
            {
                if (u->GetEntry() == i_entry && u->IsAlive() == i_alive && i_obj.IsWithinDistInMap(u, i_range) && i_obj.IsEventVisibleFor(u))
                {
                    i_range = i_obj.GetDistance(u);         // use found unit range as new range limit for next check
                    return true;
                }
