...
bool Guild::Create(Player* pLeader, std::string const& name)
{
    ...
    GuildLevelingSystemLoadLevelInfo();
...
// Guild Leveling System
void Guild::GuildLevelingSystemLoadLevelInfo()
{
    PreparedStatement* stmt;
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUILD_LEVELING_LEVEL_INFO);
    stmt->setUInt32(0, m_id);
    PreparedQueryResult result = CharacterDatabase.Query(stmt);

    if (result)
    {
        m_current_guild_leveling_system_xp = (*result)[0].GetUInt32();
        m_guild_system_leveling_level = (*result)[1].GetUInt8();
    }
    else
    {
        m_current_guild_leveling_system_xp = 0;
        m_guild_system_leveling_level = 0;
    }

    if (m_guild_system_leveling_level < GUILD_LEVELING_MAX_LEVEL)
    {
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUILD_LEVELING_XP_FOR_NEXT_LEVEL);
        stmt->setUInt32(0, m_guild_system_leveling_level);
        PreparedQueryResult result = CharacterDatabase.Query(stmt);

        if (result)
            m_xp_next_level = (*result)[0].GetUInt32();
        else
            m_xp_next_level = 0;
    }
}

bool Guild::HasLevelBonusGuildLeveling(uint16 GuildLevelingSystemBonus)
{
    uint8 req_level = sWorld->GetReqGuildLevelForBonus(GuildLevelingSystemBonus);
    return req_level > 0 && m_guild_system_leveling_level >= req_level;
}

void Guild::GuildLevelingGiveXp(uint32 value)
{
    if (m_guild_system_leveling_level > GUILD_LEVELING_MAX_LEVEL)
        return;

    if ((m_current_guild_leveling_system_xp + value) >= m_xp_next_level)
    {
        m_current_guild_leveling_system_xp = (m_current_guild_leveling_system_xp + value) - m_xp_next_level;
        SetLevelGuildLeveling(m_guild_system_leveling_level + 1, false);
    }
    else
        m_current_guild_leveling_system_xp += value;

    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_LEVELING_XP);
    stmt->setUInt32(0, m_current_guild_leveling_system_xp);
    stmt->setUInt32(1, m_id);
    CharacterDatabase.Execute(stmt);
}

void Guild::SetLevelGuildLeveling(uint8 level, bool byCommand)
{
    if (level > GUILD_LEVELING_MAX_LEVEL)
        return;

    m_guild_system_leveling_level = level;
    ...
    sWorld->SendWorldText(LANG_GUILD_LEVELING_SERVER_LEVEL_UP, m_name.c_str(), level);

    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILD_LEVELING_LEVEL);
