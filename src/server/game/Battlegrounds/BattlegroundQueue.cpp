...
#include "Player.h"
#include "World.h"
#include "../../scripts/Custom/Battleground/battleground_soloqueue.h"
...
BattlegroundQueue::BattlegroundQueue()
{
    ...
    // This is the Arena Queue Announce
    teamID = 0;
}
...
// TODO: Funcion inestable, revisar...
bool BattlegroundQueue::CheckIPExistsInQueue(std::string const& remote_addr, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated, bool isPremade)
{
    ...
    // TODO: BG_QUEUE_GUILD_TRUE.... y BG_QUEUE_GUILD_FALSE?
    for (GroupsQueueType::iterator queue_itr = m_QueuedGroups[bracketId][index][BG_QUEUE_GUILD_TRUE].begin(); queue_itr != m_QueuedGroups[bracketId][index][BG_QUEUE_GUILD_TRUE].end(); ++queue_itr)
...
GroupQueueInfo* BattlegroundQueue::AddGroup(Player* leader, Group* grp, BattlegroundTypeId BgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 arenateamid, uint32 guildId)
{
    ...
    if (sWorld->getBoolConfig(BATTLEGROUND_CROSSFACTION_ENABLED) && ginfo->ArenaType == 0)
    ...
    if (isRated && sWorld->getBoolConfig(CONFIG_ARENA_QUEUE_ANNOUNCER_ENABLE))
    {
        ...
        if (Team)
            ...
            {
                if ((ginfo->ArenaType == ARENA_TEAM_2v2 && ginfo->ArenaTeamRating >= sWorld->getIntConfig(CONFIG_ARENA_QUEUE_ANNOUNCER_MINRATE2C2))
                    || (ginfo->ArenaType == ARENA_TEAM_3v3 && ginfo->ArenaTeamRating >= sWorld->getIntConfig(CONFIG_ARENA_QUEUE_ANNOUNCER_MINRATE3C3))
                    || (ginfo->ArenaType == ARENA_TEAM_5v5 && ginfo->ArenaTeamRating >= sWorld->getIntConfig(CONFIG_ARENA_QUEUE_ANNOUNCER_MINRATE5C5)))
                {
                    if(sWorld->getBoolConfig(CONFIG_ARENA_QUEUE_ANNOUNCER_SHOWTEAMNAMES))
                        sWorld->SendWorldText(LANG_ARENA_QUEUE_ANNOUNCE_WORLD_JOIN, Team->GetName().c_str(), ginfo->ArenaType, ginfo->ArenaType, ginfo->ArenaTeamRating);
                    else
                        sWorld->SendWorldText(LANG_ARENA_QUEUE_ANNOUNCE_JOIN2, ginfo->ArenaType, ginfo->ArenaType, ginfo->ArenaTeamRating);
    ...
    //add GroupInfo to m_QueuedGroups
    {
        ...
        uint32 isGuildWars = guildId > 0 ? BG_QUEUE_GUILD_TRUE : BG_QUEUE_GUILD_FALSE;
        m_QueuedGroups[bracketId][index][isGuildWars].push_back(ginfo);
        ...
        if (!isRated && !isPremade && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE))
        {
            if (Battleground* bg = sBattlegroundMgr->GetBattlegroundTemplate(ginfo->BgTypeId))
            {
                if (sWorld->getBoolConfig(BATTLEGROUND_CROSSFACTION_ENABLED))
                {
                    ...
                    for (GroupsQueueType::const_iterator itr = m_QueuedGroups[bracketId][BG_QUEUE_MIXED][isGuildWars].begin(); itr != m_QueuedGroups[bracketId][BG_QUEUE_MIXED][isGuildWars].end(); ++itr)
                    ...
                    else
                    {
                        ...
                        ss << "|cffff0000[";
                        ss << (isGuildWars ? "GW" : "BG");
                        ss << " Queue]:|r " << bgName << " (" << q_min_level << " - " << q_max_level << ") " << qPlayers << "/" << MinPlayers;
                ...
                else
                {
                    ...
                    uint32 MinPlayers = bg->GetMinPlayersPerTeam();
                    uint32 qHorde = 0;
                    std::map<uint32,uint32> qHordeGuilds;
                    uint32 qAlliance = 0;
                    std::map<uint32,uint32> qAllianceGuilds;
                    uint32 q_min_level = bracketEntry->minLevel;
                    uint32 q_max_level = bracketEntry->maxLevel;
                    GroupsQueueType::const_iterator itr;
                    for (itr = m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_ALLIANCE][isGuildWars].begin(); itr != m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_ALLIANCE][isGuildWars].end(); ++itr)
                        if (!(*itr)->IsInvitedToBGInstanceGUID)
                        {
                            // Si es GuildWars mostraremos el grupo de players mas grande, o su grupo.
                            if (isGuildWars)
                            {
                                if (leader->GetTeam() == ALLIANCE)
                                {
                                    if ((*itr)->GuildId != guildId)
                                        continue;
                                ...
                                {
                                    // Contamos la guild opuesta con mas miembros en cola
                                    if (qAllianceGuilds.find((*itr)->GuildId) != qAllianceGuilds.end())
                                        qAllianceGuilds[(*itr)->GuildId] += (*itr)->Players.size();
                                    else
                                        qAllianceGuilds[(*itr)->GuildId] = (*itr)->Players.size();

                                    if (qAlliance < qAllianceGuilds[(*itr)->GuildId])
                                        qAlliance = qAllianceGuilds[(*itr)->GuildId];
                                }
                            }
                            else
                                qAlliance += (*itr)->Players.size();
                    ...
                    for (itr = m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_HORDE][isGuildWars].begin(); itr != m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_HORDE][isGuildWars].end(); ++itr)
                        if (!(*itr)->IsInvitedToBGInstanceGUID)
                        {
                            // Si es GuildWars mostraremos el grupo de players mas grande, o su grupo.
                            if (isGuildWars)
                            {
                                ...
                                {
                                    // Contamos la guild opuesta con mas miembros en cola
                                    if (qHordeGuilds.find((*itr)->GuildId) != qHordeGuilds.end())
                                        qHordeGuilds[(*itr)->GuildId] += (*itr)->Players.size();
                                    else
                                        qHordeGuilds[(*itr)->GuildId] = (*itr)->Players.size();

                                    if (qHorde < qHordeGuilds[(*itr)->GuildId])
                                        qHorde = qHordeGuilds[(*itr)->GuildId];
                                }
                            }
                            else
                                qHorde += (*itr)->Players.size();
                    ...
                    if (sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_PLAYERONLY))
                    {
                        ChatHandler(leader->GetSession()).PSendSysMessage(LANG_BG_QUEUE_ANNOUNCE_SELF, (isGuildWars ? "GW" : "BG"), bgName, q_min_level, q_max_level,
                            qAlliance, (MinPlayers > qAlliance) ? MinPlayers - qAlliance : (uint32)0, qHorde, (MinPlayers > qHorde) ? MinPlayers - qHorde : (uint32)0);

                        if (sWorld->getIntConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_PLAYERONLY_JOINEDPLAYERS) > 0
                        && sWorld->getIntConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_PLAYERONLY_JOINEDPLAYERS) == joinedPlayers)
                        {
                            // Nuevo sistema de broadcast continuo de players en bg
                            GroupsQueueType::const_iterator gitr;
                            // Grupo Alianza
                            if (!isGuildWars || leader->GetTeam() == ALLIANCE)
                            {
                                for (gitr = m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_ALLIANCE][isGuildWars].begin(); gitr != m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_ALLIANCE][isGuildWars].end(); ++gitr)
                                {
                                    if (!(*gitr)->IsInvitedToBGInstanceGUID)
                                    {
                                        for (std::map<uint64, PlayerQueueInfo*>::iterator itr = (*gitr)->Players.begin(); itr != (*gitr)->Players.end(); ++itr)
                                        {
                                            Player* player = ObjectAccessor::FindPlayer(itr->first);

                                            if (!player || (isGuildWars && (*gitr)->GuildId != guildId))
                                                continue;

                                            ChatHandler(player->GetSession()).PSendSysMessage(LANG_BG_QUEUE_ANNOUNCE_SELF, (isGuildWars ? "GW" : "BG"), bgName, q_min_level, q_max_level,
                                                qAlliance, (MinPlayers > qAlliance) ? MinPlayers - qAlliance : (uint32)0, qHorde, (MinPlayers > qHorde) ? MinPlayers - qHorde : (uint32)0);
                                        }
                                    }
                                }
                            ...
                            // Grupo Horda
                            if (!isGuildWars || leader->GetTeam() == HORDE)
                            {
                                for (gitr = m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_HORDE][isGuildWars].begin(); gitr != m_QueuedGroups[bracketId][BG_QUEUE_NORMAL_HORDE][isGuildWars].end(); ++gitr)
                                {
                                    if (!(*gitr)->IsInvitedToBGInstanceGUID)
                                    {
                                        for (std::map<uint64, PlayerQueueInfo*>::iterator itr = (*gitr)->Players.begin(); itr != (*gitr)->Players.end(); ++itr)
                    ...
                    {
                        sWorld->SendWorldText(LANG_BG_QUEUE_ANNOUNCE_WORLD, (isGuildWars ? "GW" : "BG"), bgName, q_min_level, q_max_level,
                            qAlliance, (MinPlayers > qAlliance) ? MinPlayers - qAlliance : (uint32)0, qHorde, (MinPlayers > qHorde) ? MinPlayers - qHorde : (uint32)0);
                    }
                    // Anunciar el JoinedPlayers customizado en broadcast
                    if (sWorld->getIntConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_JOINEDPLAYERS) > 0
                        && sWorld->getIntConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_JOINEDPLAYERS) == joinedPlayers)
                    {
                        sWorld->SendWorldText(LANG_BG_QUEUE_ANNOUNCE_WORLD, (isGuildWars ? "GW" : "BG"), bgName, q_min_level, q_max_level,
                            qAlliance, (MinPlayers > qAlliance) ? MinPlayers - qAlliance : (uint32)0, qHorde, (MinPlayers > qHorde) ? MinPlayers - qHorde : (uint32)0);
...
void BattlegroundQueue::RemovePlayer(uint64 guid, bool decreaseInvitedCount)
{
    ...
    for (int32 bracket_id_tmp = MAX_BATTLEGROUND_BRACKETS - 1; bracket_id_tmp >= 0 && bracket_id == -1; --bracket_id_tmp)
    {
        ...
        {
            for (uint32 l = 0; l < BG_QUEUE_GUILD_BOOL; ++l)
            {
                GroupsQueueType::iterator k = m_QueuedGroups[bracket_id_tmp][j][l].begin();
                for (; k != m_QueuedGroups[bracket_id_tmp][j][l].end(); ++k)
                {
                    if ((*k) == group)
                    {
    ...
    */
    ...
    {
        uint32 isGuildWars = group->GuildId > 0 ? BG_QUEUE_GUILD_TRUE : BG_QUEUE_GUILD_FALSE;
        m_QueuedGroups[bracket_id][index][isGuildWars].erase(group_itr);
        delete group;
        return;
    }
...
*/
void BattlegroundQueue::FillPlayersToBG(Battleground* bg, BattlegroundBracketId bracket_id, bool GuildWars)
{
    ...
    uint32 isGuildWars = GuildWars ? BG_QUEUE_GUILD_TRUE : BG_QUEUE_GUILD_FALSE;

    //iterator for iterating through bg queue
    GroupsQueueType::const_iterator Ali_itr = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE][isGuildWars].begin();
    for (; Ali_itr != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE][isGuildWars].end(); ++Ali_itr)
    {
        if (!GuildWars || (*Ali_itr)->GuildId == bg->GetGuildId(TEAM_ALLIANCE))
        {
    ...
    GroupsQueueType::const_iterator Horde_itr = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE][isGuildWars].begin();
    for (; Horde_itr != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE][isGuildWars].end(); ++Horde_itr)
    {
        if (!GuildWars || (*Horde_itr)->GuildId == bg->GetGuildId(TEAM_HORDE))
        {
...
bool BattlegroundQueue::CheckPremadeMatch(BattlegroundBracketId bracket_id, uint32 MinPlayersPerTeam, uint32 MaxPlayersPerTeam, bool GuildWars)
{
    uint32 guildId[BG_TEAMS_COUNT];
    guildId[TEAM_ALLIANCE]  = 0;
    guildId[TEAM_HORDE]     = 0;

    uint32 isGuildWars = GuildWars ? BG_QUEUE_GUILD_TRUE : BG_QUEUE_GUILD_FALSE;

    //check match
    if (!m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][isGuildWars].empty() && !m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][isGuildWars].empty())
    {
        //start premade match
        //if groups aren't invited
        GroupsQueueType::const_iterator ali_group, horde_group;
        for (ali_group = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][isGuildWars].begin(); ali_group != m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][isGuildWars].end(); ++ali_group)
            if (!(*ali_group)->IsInvitedToBGInstanceGUID)
            {
                if (GuildWars)
                    guildId[TEAM_ALLIANCE] = (*ali_group)->GuildId;
                break;
            }
        for (horde_group = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][isGuildWars].begin(); horde_group != m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][isGuildWars].end(); ++horde_group)
            if (!(*horde_group)->IsInvitedToBGInstanceGUID)
            {
                if (GuildWars)
                    guildId[TEAM_HORDE] = (*horde_group)->GuildId;
                break;
            }

        if (ali_group != m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][isGuildWars].end() && horde_group != m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][isGuildWars].end())
        {
            ...
            for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
            {
                for (itr = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + i][isGuildWars].begin(); itr != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + i][isGuildWars].end(); ++itr)
                {
                    if (!GuildWars || (*itr)->GuildId == guildId[i])
    ...
    for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
    {
        if (!m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE + i][isGuildWars].empty())
        {
            GroupsQueueType::iterator itr = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE + i][isGuildWars].begin();
            if (!(*itr)->IsInvitedToBGInstanceGUID && ((*itr)->JoinTime < time_before || (*itr)->Players.size() < MinPlayersPerTeam))
            {
                //we must insert group to normal queue and erase pointer from premade queue
                m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + i][isGuildWars].push_front((*itr));
                m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE + i][isGuildWars].erase(itr);
            }
        }
    }
...
bool BattlegroundQueue::CheckNormalMatch(Battleground* bg_template, BattlegroundBracketId bracket_id, uint32 minPlayers, uint32 maxPlayers, bool GuildWars)
{
    ...
    {
        guildId[i] = 0;
        itr_team[i] = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + i][isGuildWars].begin();
        for (; itr_team[i] != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + i][isGuildWars].end(); ++(itr_team[i]))
        {
        ...
        && m_SelectionPools[TEAM_HORDE].GetPlayerCount() >= minPlayers && m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() >= minPlayers)
    {
        ...
        for (; itr_team[j] != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + j][isGuildWars].end(); ++(itr_team[j]))
        {
            if (!(*(itr_team[j]))->IsInvitedToBGInstanceGUID)
                if (!GuildWars || guildId[j] == (*(itr_team[j]))->GuildId)
    ...
    {
        GroupsQueueType::const_iterator itr = m_SelectionPools[m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() ? TEAM_ALLIANCE : TEAM_HORDE].SelectedGroups.begin();
        if (!GuildWars || (*itr)->GuildId > 0)
            return true;
    }
    ...
    if (m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() >= minPlayers && m_SelectionPools[TEAM_HORDE].GetPlayerCount() >= minPlayers)
    {
        ...
        GroupsQueueType::const_iterator horde = m_SelectionPools[TEAM_HORDE].SelectedGroups.begin();
        if (!GuildWars || ((*alliance)->GuildId > 0 && (*horde)->GuildId > 0))
            return true;
    }
...
bool BattlegroundQueue::CheckSkirmishForSameFaction(BattlegroundBracketId bracket_id, uint32 minPlayersPerTeam)
{
    ...
    itr_team = m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + teamIndex][BG_QUEUE_GUILD_FALSE].begin();
    for (; itr_team != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + teamIndex][BG_QUEUE_GUILD_FALSE].end(); ++itr_team)
        if (ginfo == *itr_team)
            break;
    if (itr_team == m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + teamIndex][BG_QUEUE_GUILD_FALSE].end())
        return false;
    GroupsQueueType::iterator itr_team2 = itr_team;
    ++itr_team2;
    //invite players to other selection pool
    for (; itr_team2 != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + teamIndex][BG_QUEUE_GUILD_FALSE].end(); ++itr_team2)
    {
    ...
    for (GroupsQueueType::iterator itr = m_SelectionPools[otherTeam].SelectedGroups.begin(); itr != m_SelectionPools[otherTeam].SelectedGroups.end(); ++itr)
    {
        ...
        m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + otherTeam][BG_QUEUE_GUILD_FALSE].push_front(*itr);
        //remove team from old queue
        GroupsQueueType::iterator itr2 = itr_team;
        ++itr2;
        for (; itr2 != m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + teamIndex][BG_QUEUE_GUILD_FALSE].end(); ++itr2)
        {
            if (*itr2 == *itr)
            {
                m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE + teamIndex][BG_QUEUE_GUILD_FALSE].erase(itr2);
                break;
            }
        }
    }
...
bool BattlegroundQueue::CheckSolo3v3Arena(BattlegroundBracketId bracket_id, bool GuildWars)
{
    ...
    {
        for (GroupsQueueType::iterator itr = m_QueuedGroups[bracket_id][teamId][isGuildWars].begin(); itr != m_QueuedGroups[bracket_id][teamId][isGuildWars].end(); ++itr)
        {
            ...
            for (std::map<uint64, PlayerQueueInfo*>::iterator grpPlrItr = grpPlr->begin(); grpPlrItr != grpPlr->end(); grpPlrItr++)
            {
                    ...
                    || (!filterTalents && m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() != 3))
                {
                    if (m_SelectionPools[TEAM_ALLIANCE].AddGroup((*itr), 3)) // added successfully?
                    {
                        ...
                        {
                            (*itr)->Team = ALLIANCE;
                            m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][isGuildWars].push_back((*itr));
                            itr = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][isGuildWars].erase(itr);
                            return CheckSolo3v3Arena(bracket_id, GuildWars);
                        }
                    }
                }
                else if (filterTalents && soloTeam[TEAM_HORDE][plrCat] == false || !filterTalents) // nope? and in horde team?
                {
                    if (m_SelectionPools[TEAM_HORDE].AddGroup((*itr), 3))
                    {
                        ...
                        {
                            (*itr)->Team = HORDE;
                            m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][isGuildWars].push_back((*itr));
                            itr = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][isGuildWars].erase(itr);
                            return CheckSolo3v3Arena(bracket_id, GuildWars);
                        }
                    }
                }
...
*/
void BattlegroundQueue::BattlegroundQueueUpdate(uint32 /*diff*/, BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating, bool GuildWars)
{
    //if no players in queue - do nothing
    if (m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][BG_QUEUE_GUILD_TRUE].empty() &&
        m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][BG_QUEUE_GUILD_TRUE].empty() &&
        m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE][BG_QUEUE_GUILD_TRUE].empty() &&
        m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE][BG_QUEUE_GUILD_TRUE].empty() &&
        m_QueuedGroups[bracket_id][BG_QUEUE_MIXED][BG_QUEUE_GUILD_TRUE].empty() &&
        m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][BG_QUEUE_GUILD_FALSE].empty() &&
        m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][BG_QUEUE_GUILD_FALSE].empty() &&
        m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_ALLIANCE][BG_QUEUE_GUILD_FALSE].empty() &&
        m_QueuedGroups[bracket_id][BG_QUEUE_NORMAL_HORDE][BG_QUEUE_GUILD_FALSE].empty() &&
        m_QueuedGroups[bracket_id][BG_QUEUE_MIXED][BG_QUEUE_GUILD_FALSE].empty())
        return;
    ...
    for (BGFreeSlotQueueContainer::iterator itr = bgQueues.begin(); itr != bgQueues.end();)
    {
            ...
            bg->GetStatus() > STATUS_WAIT_QUEUE && bg->GetStatus() < STATUS_WAIT_LEAVE)
        {
            ...
            FillPlayersToBG(bg, bracket_id, bg->IsGuildWars());
    ...
    if (bg_template->isBattleground())
    {
        ...
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam, true))
        {
            ...
            for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
            {
                GroupsQueueType::const_iterator citr = m_SelectionPools[TEAM_ALLIANCE + i].SelectedGroups.begin();
                guildId[TEAM_ALLIANCE + i] = (*citr)->GuildId;
            }

            //create new battleground
            Battleground* bg2 = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, 0, false, guildId[TEAM_ALLIANCE], guildId[TEAM_HORDE]);
            if (!bg2)
            {
                TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BattlegroundQueue::Update - Cannot create battleground: %u", bgTypeId);
                return;
    ...
    if (!isRated)
    {
        ...
        if (bg_template->isBattleground() && CheckNormalMatch(bg_template, bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam, true))
        {
            uint32 guildId[BG_TEAMS_COUNT];
            guildId[TEAM_ALLIANCE] = 0;
            guildId[TEAM_HORDE] = 0;
            ...
            // we successfully created a pool
            Battleground* bg2 = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, arenaType, false, guildId[TEAM_ALLIANCE], guildId[TEAM_HORDE]);
            if (!bg2)
            {
                TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BattlegroundQueue::Update - Cannot create battleground: %u", bgTypeId);
                return;
    ...
    else if (bg_template->isArena())
    {
        ...
        if (!arenaRating)
        {
            if (!m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][BG_QUEUE_GUILD_FALSE].empty())
            {
                front1 = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][BG_QUEUE_GUILD_FALSE].front();
                arenaRating = front1->ArenaMatchmakerRating;
                // progressive mmr
                float mmrSteps = static_cast<float>( (getMSTime() - front1->JoinTime) / sWorld->getIntConfig(CONFIG_ARENA_PROGRESSIVE_MMR_TIMER) );
                mmrMaxDiff = mmrSteps * sWorld->getIntConfig(CONFIG_ARENA_PROGRESSIVE_MMR_STEPSIZE);
            }
            if (!m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][BG_QUEUE_GUILD_FALSE].empty())
            {
                front2 = m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][BG_QUEUE_GUILD_FALSE].front();
                arenaRating = front2->ArenaMatchmakerRating;
                // progressive mmr
                float mmrSteps = static_cast<float>( (getMSTime() - front2->JoinTime) / sWorld->getIntConfig(CONFIG_ARENA_PROGRESSIVE_MMR_TIMER) );
                mmrMaxDiff = mmrSteps * sWorld->getIntConfig(CONFIG_ARENA_PROGRESSIVE_MMR_STEPSIZE);
            }
            if (front1 && front2)
            {
        ...
        for (uint8 i = BG_QUEUE_PREMADE_ALLIANCE; i < BG_QUEUE_NORMAL_ALLIANCE; i++)
        {
            ...
            GroupsQueueType::iterator itr2 = m_QueuedGroups[bracket_id][i][BG_QUEUE_GUILD_FALSE].begin();
            for (; itr2 != m_QueuedGroups[bracket_id][i][BG_QUEUE_GUILD_FALSE].end(); ++itr2)
            {
        ...
        if (found == 1)
        {
            for (GroupsQueueType::iterator itr3 = itr_teams[0]; itr3 != m_QueuedGroups[bracket_id][team][BG_QUEUE_GUILD_FALSE].end(); ++itr3)
            {
        ...
        if (found == 2)
        {
            ...
            {
                m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][BG_QUEUE_GUILD_FALSE].push_front(aTeam);
                m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][BG_QUEUE_GUILD_FALSE].erase(itr_teams[TEAM_ALLIANCE]);
            }
            if (hTeam->Team != HORDE)
            {
                m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_HORDE][BG_QUEUE_GUILD_FALSE].push_front(hTeam);
                m_QueuedGroups[bracket_id][BG_QUEUE_PREMADE_ALLIANCE][BG_QUEUE_GUILD_FALSE].erase(itr_teams[TEAM_HORDE]);
            }
