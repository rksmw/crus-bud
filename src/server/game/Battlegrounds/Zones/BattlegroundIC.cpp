...
void BattlegroundIC::PostUpdateImpl(uint32 diff)
{
    ...
    for (uint8 i = NODE_TYPE_REFINERY; i < MAX_NODE_TYPES; ++i)
    {
        ...
        // Update Siege Engine UNIT_FLAG_IMMUNE_TO_PC flag
        for (uint8 Siege_Team = 0; Siege_Team <= TEAM_HORDE; Siege_Team++)
        {
            uint8 siegeEngine =(Siege_Team==TEAM_HORDE ? BG_IC_NPC_SIEGE_ENGINE_H : BG_IC_NPC_SIEGE_ENGINE_A);
            if (Creature* siege = GetBGCreature(siegeEngine))
            {
                siege->RemoveFlag(UNIT_FIELD_FLAGS,UNIT_FLAG_IMMUNE_TO_PC);
            }
        }
...
void BattlegroundIC::EventPlayerClickedOnFlag(Player* player, GameObject* target_obj)
{
    ...
    if (player->HasAura(43680)) // Idle
        player->RemoveAura(43680);
    if (player->HasAura(43681)) // Inactive
        player->RemoveAura(43681);
        ...
        case BG_IC_GO_HANGAR_BANNER:
            ...
            for (uint8 u = 0; u < MAX_HANGAR_TELEPORTER_EFFECTS_SPAWNS; ++u)
            {
                ...
                if (!AddObject(type, (nodePoint->faction == TEAM_ALLIANCE ? GO_ALLIANCE_GUNSHIP_PORTAL : GO_HORDE_GUNSHIP_PORTAL),
                    ...
                    BG_IC_HangarTeleporters[u].GetPositionZ(), BG_IC_HangarTeleporters[u].GetOrientation(),
                    0, 0, 0, 0, RESPAWN_ONE_DAY))
                    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Isle of Conquest: There was an error spawning a gunship portal effects. Type: %u", BG_IC_GO_HANGAR_TELEPORTER_1 + u);
            }
            for (uint8 u = 0; u < MAX_CAPTAIN_SPAWNS_PER_FACTION; ++u)
            {
                uint8 type = BG_IC_NPC_GUNSHIP_CAPTAIN_1 + u;
                ...
                if (type == BG_IC_NPC_GUNSHIP_CAPTAIN_1)
                    if (AddCreature(nodePoint->faction == TEAM_ALLIANCE ? NPC_ALLIANCE_GUNSHIP_CAPTAIN : NPC_HORDE_GUNSHIP_CAPTAIN, type, nodePoint->faction,
                        BG_IC_HangarCaptains[u].GetPositionX(), BG_IC_HangarCaptains[u].GetPositionY(),
                        BG_IC_HangarCaptains[u].GetPositionZ(), BG_IC_HangarCaptains[u].GetOrientation(),
                        RESPAWN_ONE_DAY))
                ...
                if (type == BG_IC_NPC_GUNSHIP_CAPTAIN_2)
                    if (!AddCreature(nodePoint->faction == TEAM_ALLIANCE ? NPC_ALLIANCE_GUNSHIP_CAPTAIN : NPC_HORDE_GUNSHIP_CAPTAIN, type, nodePoint->faction,
                        ...
                        BG_IC_HangarCaptains[u].GetPositionZ(), BG_IC_HangarCaptains[u].GetOrientation(),
                        RESPAWN_ONE_DAY))                        
                        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Isle of Conquest: There was an error spawning a world trigger. Type: %u", BG_IC_NPC_GUNSHIP_CAPTAIN_2);
            }
...
WorldSafeLocsEntry const* BattlegroundIC::GetClosestGraveYard(Player* player)
{
    ...
    for (uint8 i = 0; i < MAX_NODE_TYPES; ++i)
        if (nodePoint[i].faction == player->GetTeamId() && nodePoint[i].nodeState == NODE_STATE_CONTROLLED_A || nodePoint[i].nodeState == NODE_STATE_CONTROLLED_H)
            nodes.push_back(i);
