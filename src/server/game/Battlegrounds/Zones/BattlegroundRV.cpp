...
bool BattlegroundRV::SetupBattleground()
{
    ...
    if (   !AddObject(BG_RV_OBJECT_FENCE_1, BG_RV_OBJECT_TYPE_FENCE_1, 763.432373f, -274.058197f, 28.276695f, 3.141593f, 0, 0, 0, RESPAWN_IMMEDIATELY)
        ...
        // elevators
        || !AddObject(BG_RV_OBJECT_ELEVATOR_1, BG_RV_OBJECT_TYPE_ELEVATOR_1, 763.536377f, -294.535767f, 26.000000f, 3.141593f, 0, 0, 0, RESPAWN_IMMEDIATELY)
        || !AddObject(BG_RV_OBJECT_ELEVATOR_2, BG_RV_OBJECT_TYPE_ELEVATOR_2, 763.506348f, -273.873352f, 26.000000f, 0.000000f, 0, 0, 0, RESPAWN_IMMEDIATELY)
        // buffs
        ...
        // Pilars added to floor height (prevent people falling to textures)
        || !AddObject(BG_RV_OBJECT_PILAR_1_FLOOR, BG_RV_OBJECT_TYPE_FLOOR_PILAR_1, 763.632385f, -306.162384f, 25.913908f, 3.141593f, 0, 0, 0, RESPAWN_IMMEDIATELY) // tiny
        || !AddObject(BG_RV_OBJECT_PILAR_2_FLOOR, BG_RV_OBJECT_TYPE_FLOOR_PILAR_2, 763.611145f, -261.856750f, 25.913908f, 0.000000f, 0, 0, 0, RESPAWN_IMMEDIATELY) // tiny
        || !AddObject(BG_RV_OBJECT_PILAR_3_FLOOR, BG_RV_OBJECT_TYPE_FLOOR_PILAR_3, 723.644287f, -284.493256f, 24.649525f, 3.141593f, 0, 0, 0, RESPAWN_IMMEDIATELY) // big
        || !AddObject(BG_RV_OBJECT_PILAR_4_FLOOR, BG_RV_OBJECT_TYPE_FLOOR_PILAR_4, 802.211609f, -284.493256f, 24.649525f, 0.000000f, 0, 0, 0, RESPAWN_IMMEDIATELY) // big
...
bool BattlegroundRV::IsPlayerUnderMap(Player* player, float x, float y, float z)
{
    ...
    if (z < 26.27f)
        player->TeleportTo(GetMapId(), 763.39f, -285.96f, 30.276f, 2.422f, false);
    return false;
}
...
void BattlegroundRV::TogglePillarCollision()
{
    ...
    for (uint8 i = BG_RV_OBJECT_PILAR_1; i <= BG_RV_OBJECT_PILAR_COLLISION_4; ++i)
    {
        ...
        {
            if (i >= BG_RV_OBJECT_PILAR_1_FLOOR && i <= BG_RV_OBJECT_PILAR_4_FLOOR)
            {
                if (i == BG_RV_OBJECT_PILAR_3_FLOOR || i == BG_RV_OBJECT_PILAR_4_FLOOR)
                {
                    gob->SetPhaseMask(!apply ? 2 : 1, true);
                ...
                {
                    gob->SetPhaseMask(apply ? 1 : 2, true);
                    //gob->SetGoState(gob->GetGOInfo()->door.startOpen ? GO_STATE_ACTIVE : GO_STATE_READY);
                }

                //if (gob->GetGOInfo()->door.startOpen)
                //    gob->EnableCollision(!apply); // Forced collision toggle
            }
            else if (i >= BG_RV_OBJECT_PILAR_COLLISION_1 && i <= BG_RV_OBJECT_PILAR_COLLISION_4)
            {
                uint32 _state = GO_STATE_READY;
                if (gob->GetGOInfo()->door.startOpen)
                    _state = GO_STATE_ACTIVE;
            ...
            for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
            {
                if (Player* player = ObjectAccessor::FindPlayer(MAKE_NEW_GUID(itr->first, 0, HIGHGUID_PLAYER)))
                {
                    ...
                    if (player->GetDistance(723.644287f, -284.493256f, 24.648525f) < 2.80f || // tiny (axe)
                        player->GetDistance(802.211609f, -284.493256f, 24.648525f) < 2.80f || // tiny (lightning)
                        player->GetDistance(763.632385f, -306.162384f, 25.909504f) < 4.90f || // big (arena)
                        player->GetDistance(763.611145f, -261.856750f, 25.909504f) < 4.90f)   // big (ivory)
                        player->KnockbackFrom(player->GetPositionX(), player->GetPositionY(), 1.0f, 0.75f);
                }
            }
        }
    }
...
void BattlegroundRV::UseGameObject(uint32 type)
{
    ...
    {
        if (type == BG_RV_OBJECT_PILAR_COLLISION_1)
        {
            if (getSmallPillar1Status() == BG_RV_PILLAR_STATUS_OFF)
            {
                go->SetGoState(GO_STATE_READY);
                setSmallPillar1Status(BG_RV_PILLAR_STATUS_ON);
            }
            else
            {
                go->SetGoState(GO_STATE_ACTIVE);
                setSmallPillar1Status(BG_RV_PILLAR_STATUS_OFF);
            }
        }
        else if (type == BG_RV_OBJECT_PILAR_COLLISION_3)
        {
            if (getSmallPillar2Status() == BG_RV_PILLAR_STATUS_OFF)
            {
                go->SetGoState(GO_STATE_READY);
                setSmallPillar2Status(BG_RV_PILLAR_STATUS_ON);
            }
            else
            {
                go->SetGoState(GO_STATE_ACTIVE);
                setSmallPillar2Status(BG_RV_PILLAR_STATUS_OFF);
            }
        }
        else if (type == BG_RV_OBJECT_PILAR_COLLISION_2)
        {
            if (getBigPillar1Status() == BG_RV_PILLAR_STATUS_OFF)
            {
                go->SetGoState(GO_STATE_READY);
                setBigPillar1Status(BG_RV_PILLAR_STATUS_ON);
            }
            else
            {
                go->SetGoState(GO_STATE_ACTIVE);
                setBigPillar1Status(BG_RV_PILLAR_STATUS_OFF);
            }
        }
        else if (type == BG_RV_OBJECT_PILAR_COLLISION_4)
        {
            if (getBigPillar2Status() == BG_RV_PILLAR_STATUS_OFF)
            {
                go->SetGoState(GO_STATE_READY);
                setBigPillar2Status(BG_RV_PILLAR_STATUS_ON);
            }
            else
            {
                go->SetGoState(GO_STATE_ACTIVE);
                setBigPillar2Status(BG_RV_PILLAR_STATUS_OFF);
            }
        }
        else
        {
