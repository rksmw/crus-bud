...
void BattlegroundAV::HandleKillUnit(Creature* unit, Player* killer)
{
    ...
    */
    if (entry == BG_AV_CreatureInfo[AV_NPC_A_BOSS][0])
    {
        ...
        RewardReputationToTeam(730, 729, BG_AV_REP_BOSS, killer->GetTeam() == ALLIANCE ? ALLIANCE : HORDE);
    ...
    else if (entry == BG_AV_CreatureInfo[AV_NPC_A_CAPTAIN][0])
    {
        ...
        RewardReputationToTeam(730, 729, BG_AV_REP_CAPTAIN, killer->GetTeam() == ALLIANCE ? ALLIANCE : HORDE);
...
void BattlegroundAV::EndBattleground(uint32 winner)
{
    ...
    for (int i = TEAM_ALLIANCE; i <= TEAM_HORDE; ++i)
    {
        ...
        if (rep[i] != 0)
            RewardReputationToTeam(730, 729, 10, i == TEAM_ALLIANCE ? ALLIANCE : HORDE);
        if (kills[i] != 0)
            RewardHonorToTeam(GetBonusHonor(kills[i]), i == TEAM_ALLIANCE ? ALLIANCE : HORDE);
    }
...
BG_AV_Nodes BattlegroundAV::GetNodeThroughObject(uint32 object)
{
    ...
    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BattlegroundAV: ERROR! GetPlace got a wrong object :(");
    ASSERT(false);
...
{ //this function is the counterpart to GetNodeThroughObject()
    ...
    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "BattlegroundAV: Error! GetPlaceNode couldn't resolve node %i", node);
    ASSERT(false);
    return 0;
}
...
void BattlegroundAV::EventPlayerClickedOnFlag(Player* source, GameObject* target_obj)
{
    ...
    if (source->HasAura(43680)) // Idle
        source->RemoveAura(43680);
    if (source->HasAura(43681)) // Inactive
        source->RemoveAura(43681);
...
bool BattlegroundAV::SetupBattleground()
{
    ...
    //creatures
    TC_LOG_DEBUG(LOG_FILTER_BATTLEGROUND, "BG_AV start poputlating nodes");
    for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i)
    {
        if (m_Nodes[i].Owner)
            PopulateNode(i);
