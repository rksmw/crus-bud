        ...
        if (TotalTime >= 60000)
        {
            ...
            for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
                if (Player* p = ObjectAccessor::FindPlayer(itr->first))
                {
                    ...
                    p->RemoveAurasDueToSpell(SPELL_SOTA_DEMO_SCALING); // Tem fix - when the demolisher teleported the player gets stuck with aura 
                }
        }
        if (TotalTime >= 30000)
        {
...
void BattlegroundSA::DestroyGate(Player* player, GameObject* go)
{
    ...
    if (GameObject* g = GetBGObject(i))
    {
        if (g->GetGOValue()->Building.Health == 0)
        {
            ...
            // Bugs de estado de puertas en SoTA (playa de ancestros)
            g->SetDestructibleState(GO_DESTRUCTIBLE_DESTROYED, NULL, false);
