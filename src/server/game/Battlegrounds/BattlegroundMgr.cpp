...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId originalBgTypeId, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated, uint32 guildIdA, uint32 guildIdH)
{
    ...
    bg->SetGuildId(guildIdA, TEAM_ALLIANCE);
    bg->SetGuildId(guildIdH, TEAM_HORDE);
    ...
    do
    {
        ...
        else
        {
            ...
            if (WorldSafeLocsEntry const* start = sWorldSafeLocsStore.LookupEntry(startId))
            {
                ...
                data.Team1StartLocZ = start->map_id == 618 ? 30.0f : start->z;
                data.Team1StartLocO = fields[6].GetFloat();
            }
            else
            {
            ...
            if (WorldSafeLocsEntry const* start = sWorldSafeLocsStore.LookupEntry(startId))
            {
                ...
                data.Team2StartLocZ = start->map_id == 618 ? 30.0f : start->z;
                data.Team2StartLocO = fields[8].GetFloat();
            }
            else
            {
...
void BattlegroundMgr::SendToBattleground(Player* player, uint32 instanceId, BattlegroundTypeId bgTypeId)
{
    ...
    if (Battleground* bg = GetBattleground(instanceId, bgTypeId))
    {
        ...
        if (Pet* pet = player->GetPet()) // dismiss do the pets in arena & bg
