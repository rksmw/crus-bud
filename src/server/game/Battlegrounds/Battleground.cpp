...
Battleground::Battleground()
{
    ...
    // GuildWars
    // ////////////////////////////////
    m_guildId[TEAM_ALLIANCE] = 0;
    m_guildId[TEAM_HORDE] = 0;
}
...
inline void Battleground::_ProcessJoin(uint32 diff)
{
    ...
    if (!(m_Events & BG_STARTING_EVENT_1))
    {
        ...
        if (Player* player = ObjectAccessor::FindPlayer(GetPlayers().begin()->first))
            if (player->InArena())
                player->SendChatMessage("BattleCore: Recuerda que al activar los Cristales de arena estos reduciran el tiempo de inicio de la arena en 30 segundos ");
...
inline Player* Battleground::_GetPlayer(uint64 guid, bool offlineRemove, char const* context) const
{
    ...
    if (!offlineRemove)
    {
        ...
        if (!player)
            TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::%s: player (GUID: %u) not found for BG (map: %u, instance id: %u)!",
...
bool Battleground::CheckCheatArena(uint32 winTeam)
{
    ...
    float factorLooser = dmgLoser * 100 / maxHealthWinner;
...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    if (isArena() && isRated())
    {
        ...
        if (sWorld->getBoolConfig(CONFIG_ARENA_CHECK_CHEATERS) && CheckCheatArena(GetArenaTeamIdForTeam(winner)))
        {
            ...
            TC_LOG_DEBUG(LOG_FILTER_ARENAS, "Arena cheated, score is 0 for teams IDs %u and %u", winnerArenaTeam->GetId(),loserArenaTeam->GetId());
        }
        else
        {
            if (winnerArenaTeam && loserArenaTeam && winnerArenaTeam != loserArenaTeam)
            {
                if (winner != WINNER_NONE)
                {
                    ...
                    TC_LOG_DEBUG(LOG_FILTER_ARENAS, "match Type: %u --- Winner: old rating: %u, rating gain: %d, old MMR: %u, MMR gain: %d --- Loser: old rating: %u, rating loss: %d, old MMR: %u, MMR loss: %d ---", m_ArenaType, winnerTeamRating, winnerChange, winnerMatchmakerRating,
                    ...
                    if (sWorld->getBoolConfig(CONFIG_ARENA_LOG_EXTENDED_INFO))
                        for (Battleground::BattlegroundScoreMap::const_iterator itr = GetPlayerScoresBegin(); itr != GetPlayerScoresEnd(); ++itr)
                            if (Player* player = ObjectAccessor::FindPlayer(itr->first))
                            {
                                TC_LOG_DEBUG(LOG_FILTER_ARENAS, "Statistics match Type: %u for %s (GUID: " UI64FMTD ", Team: %d, IP: %s): %u damage, %u healing, %u killing blows",
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        player->RemoveAura(30070); //ID - 30070 Blood Frenzy (Rank 2)
    ...
    if (IsGuildWars())
    {
        uint32 winnerGuildId = GetGuildId(winner == ALLIANCE ? TEAM_ALLIANCE : TEAM_HORDE);
        uint32 loserGuildId = GetGuildId(winner == ALLIANCE ? TEAM_HORDE : TEAM_ALLIANCE);

        PreparedStatement* stmt;
        PreparedQueryResult result;

        // Guild ganadora
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUILDWARS);
        stmt->setUInt32(0, winnerGuildId);
        result = CharacterDatabase.Query(stmt);
        if (result)
            stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILDWARS_WIN);
        else
        {
            stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_GUILDWARS);
            stmt->setUInt32(1, 1);
        }
        stmt->setUInt32(0, winnerGuildId);
        ...
        // Guild perdedora
        stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUILDWARS);
        stmt->setUInt32(0, loserGuildId);
        result = CharacterDatabase.Query(stmt);
        if (result)
            stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_GUILDWARS_LOSE);
        else
        {
            stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_GUILDWARS);
            stmt->setUInt32(1, 0);
...
void Battleground::StartBattleground()
{
    ...
    if (m_IsRated)
        TC_LOG_DEBUG(LOG_FILTER_ARENAS, "Arena match type: %u for Team1Id: %u - Team2Id: %u started.", m_ArenaType, m_ArenaTeamIds[TEAM_ALLIANCE], m_ArenaTeamIds[TEAM_HORDE]);
}
...
void Battleground::AddPlayer(Player* player)
{
    ...
    {
        WorldPacket data;
        sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, this, 0, STATUS_IN_PROGRESS, 0, GetStartTime(), GetArenaType(), 2);
        player->SendDirectMessage(&data);
        return;
    }
    ...
    if (isArena())
    {
        // Removing pet's buffs and debuffs
        ...
        if (GetStatus() == STATUS_WAIT_JOIN)                 // not started yet
        {
            ...
            switch (player->getClass())
            {
                case CLASS_HUNTER:
                    ...
                    break;
                case CLASS_MAGE:
                    player->CastSpell(player, 58661, true, NULL, NULL, player->GetGUID()); // Conjure Refreshment Table
                    break;
                case CLASS_WARLOCK:
                    player->CastSpell(player, 58888, true, NULL, NULL, player->GetGUID()); // Ritual of Souls
                    break;
                default:
                    break;
            }
...
void Battleground::EventPlayerLoggedOut(Player* player)
{
    ...
    if (player->IsSpectator()) // Fix reconnect in bg after lost connection
    {
        player->TeleportToBGEntryPoint();
...
bool Battleground::AddObject(uint32 type, uint32 entry, float x, float y, float z, float o, float rotation0, float rotation1, float rotation2, float rotation3, uint32 /*respawnTime*/, GOState goState)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_SQL, "Battleground::AddObject: cannot create gameobject (entry: %u) for BG (map: %u, instance id: %u)!",
                entry, m_MapId, m_InstanceID);
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::AddObject: cannot create gameobject (entry: %u) for BG (map: %u, instance id: %u)!",
                entry, m_MapId, m_InstanceID);
        delete go;
        return false;
    }
...
void Battleground::DoorClose(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type]))
    {
        ...
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::DoorClose: door gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!",
...
void Battleground::DoorOpen(uint32 type)
{
    if (GameObject* obj = GetBgMap()->GetGameObject(BgObjects[type]))
    {
        ...
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::DoorOpen: door gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!",
            type, GUID_LOPART(BgObjects[type]), m_MapId, m_InstanceID);
}
...
GameObject* Battleground::GetBGObject(uint32 type)
{
    ...
    if (!obj && BgObjects[type])
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::GetBGObject: gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!",
            type, GUID_LOPART(BgObjects[type]), m_MapId, m_InstanceID);
    return obj;
}
...
Creature* Battleground::GetBGCreature(uint32 type)
{
    ...
    if (!creature && BgCreatures[type])
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::GetBGCreature: creature (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!",
...
Creature* Battleground::AddCreature(uint32 entry, uint32 type, uint32 teamval, float x, float y, float z, float o, uint32 respawntime, Transport* transport)
{
    ...
    {
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::AddCreature: cannot create creature (entry: %u) for BG (map: %u, instance id: %u)!",
            entry, m_MapId, m_InstanceID);
        delete creature;
        return NULL;
    }
    ...
    if (!cinfo)
    {
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::AddCreature: creature template (entry: %u) does not exist for BG (map: %u, instance id: %u)!",
            entry, m_MapId, m_InstanceID);
        delete creature;
        return NULL;
    }
...
bool Battleground::DelCreature(uint32 type)
{
    ...
    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::DelCreature: creature (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!",
        type, GUID_LOPART(BgCreatures[type]), m_MapId, m_InstanceID);
    BgCreatures[type] = 0;
    return false;
}
...
bool Battleground::DelObject(uint32 type)
{
    ...
    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::DelObject: gameobject (type: %u, GUID: %u) not found for BG (map: %u, instance id: %u)!",
        type, GUID_LOPART(BgObjects[type]), m_MapId, m_InstanceID);
    BgObjects[type] = 0;
    return false;
}
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z, float o, uint32 team)
{
    ...
    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::AddSpiritGuide: cannot create spirit guide (type: %u, entry: %u) for BG (map: %u, instance id: %u)!",
...
void Battleground::HandleTriggerBuff(uint64 go_guid)
{
    ...
    if (index < 0)
    {
        TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::HandleTriggerBuff: cannot find buff gameobject (GUID: %u, entry: %u, type: %u) in internal data for BG (map: %u, instance id: %u)!",
...
int32 Battleground::GetObjectType(uint64 guid)
{
    for (uint32 i = 0; i < BgObjects.size(); ++i)
        if (BgObjects[i] == guid)
            return i;
    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "Battleground::GetObjectType: player used gameobject (GUID: %u) which is not in internal data for BG (map: %u, instance id: %u), cheating?",
...
uint32 Battleground::GetTeamScore(uint32 teamId) const
{
    ...
    TC_LOG_ERROR(LOG_FILTER_BATTLEGROUND, "GetTeamScore with wrong Team %u for BG %u", teamId, GetTypeID());
    return 0;
}
...
bool Battleground::IsGuildWars()
{
    return m_guildId[TEAM_ALLIANCE] > 0 || m_guildId[TEAM_HORDE] > 0;
}

void Battleground::ArenaReadyCheck() const
{
