...
void HouseMgr::Load()
{
        ...
        do
        {
            ...
            if (result2)
            {
                do
                {
                    ...
                    TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::Load] House Id: %u Rooom(GuildId: %u), cargada", houseId, guildId);
            ...
            TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::Load] House Id: %u, cargada", houseId);
    ...
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded 0 houses. DB table `houses` is empty!");
        return;
    }

    sLog->outInfo(LOG_FILTER_SERVER_LOADING, ">> Loaded %u houses in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void HouseMgr::Register(House* house)
{
    if (!house)
    {
        sLog->outError(LOG_FILTER_HOUSE, "Se trato de registrar una House invalida");
        return;
    }
    ...
    TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::Register] House (Id: %u) registrada", house->GetHouseId());
}
...
void HouseMgr::Unregister(House* house)
{
    if (!house)
    {
        sLog->outError(LOG_FILTER_HOUSE, "Se trato de eliminar del registro una House invalida");
        return;
    }
    ...
    for (HouseSet::iterator itr = _Houses.begin(); itr != _Houses.end(); itr++)
        ...
        {
            TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::Load] House (Id: %u) eliminada del registro", house->GetHouseId());
            _Houses.erase(itr);
            return;
        }

    sLog->outError(LOG_FILTER_HOUSE, "Se trato de eliminar del registro una House (Id: %u) que no se encuentra registrada", house->GetHouseId());
}
...
House* HouseMgr::GetHouseBy(uint32 id)
{
    for (HouseSet::iterator itr = _Houses.begin(); itr != _Houses.end(); itr++)
    {
        ...
        {
            TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::GetHouseBy] House de ID: %u, encontrada", id);
            return (*itr);
        }
    }

    TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::GetHouseBy] No se pudo encontrar la House de ID: %u", id);
...
House* HouseMgr::GetHouseBy(Guild* guild)
{
    if (guild)
    {
        for (HouseSet::iterator itr = _Houses.begin(); itr != _Houses.end(); itr++)
        {
            ...
            {
                TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::GetHouseBy] House ID: %u, encontrada para la guild ID: %u", (*itr)->GetHouseId(), guild->GetId());
                return (*itr);
            }
        }

        TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::GetHouseBy] No se pudo encontrar una House para la guild ID: %u", guild->GetId());
    }
    else
        sLog->outError(LOG_FILTER_HOUSE, "Se trato de obtener una House con una guild invalida");
...
House* HouseMgr::GetHouseBy(Player* player)
{
    if (player)
    {
        for (HouseSet::iterator itr = _Houses.begin(); itr != _Houses.end(); itr++)
        {
            ...
            {
                TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::GetHouseBy] House ID: %u, encontrada para el jugador GUID: %u", (*itr)->GetHouseId(), player->GetGUIDLow());
                return (*itr);
            }
        }

        TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::GetHouseBy] No se pudo encontrar una House para el jugador GUID: %u", player->GetGUIDLow());
    }
    else
        sLog->outError(LOG_FILTER_HOUSE, "Se trato de obtener una House con un jugador invalido");
...
House* HouseMgr::GetHouseBy(Creature* creature)
{
    if (creature)
    {
        for (HouseSet::iterator itr = _Houses.begin(); itr != _Houses.end(); itr++)
        {
            ...
            TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::GetHouseBy] Buscando criatura GUID: %u en House ID: %u", creature->GetGUIDLow(), (*itr)->GetHouseId());
            if ((*itr)->IsWarden(creature->GetGUID()))
            {
                TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::GetHouseBy] House ID: %u, encontrada para la criatura GUID: %u", (*itr)->GetHouseId(), creature->GetGUIDLow());
                return (*itr);
            }
        }

        TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[HouseMgr::GetHouseBy] No se pudo encontrar una House para la criatura GUID: %u", creature->GetGUIDLow());
    }
    else
        sLog->outError(LOG_FILTER_HOUSE, "Se trato de obtener una House con una criatura invalida");
