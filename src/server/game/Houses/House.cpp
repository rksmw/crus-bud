...
bool House::AddWarden(uint64 guid)
{
    ...
    {
        TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[House::AddWarden] Guid: %u, descartado", uint32(GUID_LOPART(guid)));
        return false;
    }
...
bool House::DelWarden(uint64 guid)
{
    ...
    {
        TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[House::DelWarden] Guid: %u, descartado", uint32(GUID_LOPART(guid)));
        return false;
    }
...
void House::AddRoom(uint32 guildId, uint32 startTime, uint32 endTime, uint32 phaseMask)
{
    ...
    if (room)
    {
        TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[House::AddRoom] GuildId: %u, ya existe", guildId);
        return;
    }

    if (IsInUse())
    {
        TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[House::AddRoom] House no tiene disponibilidad");
        return;
    }
    ...
    TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[House::AddRoom] Se ha añadido Room para GuildId: %u, endTime: %u, startTime: %u, phaseMask: %u", guildId, endTime, startTime, phaseMask);
...
HouseRoom* House::GetRoom(uint32 guildId)
{
    for (RoomSet::iterator itr = _rooms.begin(); itr != _rooms.end(); itr++)
        ...
        {
            TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[House::GetRoom] Se ha encontrado Room para GuildId: %u", guildId);
            return (*itr);
        }
        else
            TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[House::GetRoom] Se ha saltado Room de GuiildId: %u, IsInUse: %u.", (*itr)->GetGuildId(), (*itr)->IsInUse() ? 1 : 0);

    TC_LOG_DEBUG(LOG_FILTER_HOUSE, "[House::GetRoom] No se ha encontrado Room para GuildId: %u", guildId);
