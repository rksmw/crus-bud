...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_int_configs[CONFIG_PLAYER_SAVE_PER_SECOND] = sConfigMgr->GetIntDefault("PlayerSavePerSecond", 1);
    if (m_int_configs[CONFIG_PLAYER_SAVE_PER_SECOND] < 0)
    {
        TC_LOG_ERROR(LOG_FILTER_SERVER_LOADING, "PlayerSavePerSecond (%i) must be greater or equal than 0. Using default (1).", m_int_configs[CONFIG_PLAYER_SAVE_PER_SECOND]);
        m_int_configs[CONFIG_PLAYER_SAVE_PER_SECOND] = 1;
    }
    ...
    m_float_configs[CONFIG_SOLO_3V3_POINTS] = sConfigMgr->GetFloatDefault("Solo.3v3.Points", 0.3f);
    ...
    m_bool_configs[CONFIG_SOLO_3V3_ENABLE] = sConfigMgr->GetBoolDefault("Solo.3v3.Enable", true);
    ...
    m_int_configs[CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_JOINEDPLAYERS] = sConfigMgr->GetIntDefault("Battleground.QueueAnnouncer.JoinedPlayers", 0);
    m_int_configs[CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_PLAYERONLY_JOINEDPLAYERS] = sConfigMgr->GetIntDefault("Battleground.QueueAnnouncer.PlayerOnly.JoinedPlayers", 0);
    ...
    m_int_configs[CONFIG_ARENA_QUEUE_ANNOUNCER_MINRATE2C2]           = sConfigMgr->GetIntDefault ("Arena.QueueAnnouncer.MinRate2c2", 1700);
    m_int_configs[CONFIG_ARENA_QUEUE_ANNOUNCER_MINRATE3C3]           = sConfigMgr->GetIntDefault ("Arena.QueueAnnouncer.MinRate3c3", 1700);
    m_int_configs[CONFIG_ARENA_QUEUE_ANNOUNCER_MINRATE5C5]           = sConfigMgr->GetIntDefault ("Arena.QueueAnnouncer.MinRate5c5", 1700);
    m_bool_configs[CONFIG_ARENA_QUEUE_ANNOUNCER_SHOWTEAMNAMES]       = sConfigMgr->GetBoolDefault("Arena.QueueAnnouncer.ShowTeamNames", true);
    ...
    m_float_configs[CONFIG_ANTICHEAT_MAX_SPEED_DIFF_ALLOWED] = sConfigMgr->GetFloatDefault("Anticheat.MaxSpeedDif", 0.0f);

    // Wintergrasp battlefield
    m_bool_configs[CONFIG_WINTERGRASP_ENABLE] = sConfigMgr->GetBoolDefault("Wintergrasp.Enable", false);
    m_int_configs[CONFIG_WINTERGRASP_PLAYER_MAX] = sConfigMgr->GetIntDefault("Wintergrasp.PlayerMax", 120);
    m_int_configs[CONFIG_WINTERGRASP_PLAYER_MIN] = sConfigMgr->GetIntDefault("Wintergrasp.PlayerMin", 0);
    m_int_configs[CONFIG_WINTERGRASP_PLAYER_MIN_LVL] = sConfigMgr->GetIntDefault("Wintergrasp.PlayerMinLvl", 77);
    m_int_configs[CONFIG_WINTERGRASP_TIME_IN_BATTLE] = sConfigMgr->GetIntDefault("Wintergrasp.TimeInBattle", 30);
    m_int_configs[CONFIG_WINTERGRASP_TIME_FOR_BATTLE] = sConfigMgr->GetIntDefault("Wintergrasp.TimeForBattle", 150);
    m_int_configs[CONFIG_WINTERGRASP_TIME_ON_STARTUP] = sConfigMgr->GetIntDefault("Wintergrasp.TimeOnStartup", 15);

    // Individual loot rates
    int sec = sConfigMgr->GetIntDefault("Player.LootRateSecurity", 0);
    if (sec < SEC_PLAYER || sec > SEC_ADMINISTRATOR)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Player.LootRateSecurity has invalid security `%i`, must be between `%i and `%i`, defaulting to 0 ...",
    ...
    int maxXpRate = sConfigMgr->GetIntDefault("Player.MaximumLootRate", 1);
    if (maxXpRate < 1)
    {
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Player.MaximumLootRate has too low value `%i`, defaulting to 1 ...", maxXpRate);
    ...
    // Custom - LANG_UNIVERSAL in Dalaran PVP zone (Circle of Wills)
    m_bool_configs[CONFIG_DALARAN_UNIVERSAL_LANG] = sConfigMgr->GetBoolDefault("Custom.DalaranUniversalLanguage", true);

    m_int_configs[CONFIG_PACKET_SPOOF_BANDURATION] = sConfigMgr->GetIntDefault("PacketSpoof.BanDuration", 86400);

    m_bool_configs[CONFIG_LOG_COMMANDS_TO_DB]     = sConfigMgr->GetBoolDefault("Log.CommandToDb", true);

    m_float_configs[CONFIG_ARENA_2V2_MULTIPLIER] = sConfigMgr->GetFloatDefault("Arenas.2VS2.Multiplier", 1.0f);
    m_float_configs[CONFIG_ARENA_3V3_MULTIPLIER] = sConfigMgr->GetFloatDefault("Arenas.3VS3.Multiplier", 1.0f);
    m_float_configs[CONFIG_ARENA_5V5_MULTIPLIER] = sConfigMgr->GetFloatDefault("Arenas.5VS5.Multiplier", 1.0f);

    m_bool_configs[CHAR_DELETE_ITEMS] = sConfigMgr->GetBoolDefault("CharDelete.Items", true);

    SetCountryRestrict(sConfigMgr->GetStringDefault("CountryRestrict", "all"));

    // Guild Leveling System
    LoadGuildLevelingBonus();
...
void World::SetInitialWorldSettings()
{
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading Broadcast texts...");
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading Censure Words...");
    sWordFilterMgr->LoadBadWords();

    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading Account Roles and Permissions...");
    sAccountMgr->LoadRBAC();

    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading Account Access...");
    ...
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading Skill Perfection Data Table...");
    ...
    TC_LOG_INFO(LOG_FILTER_GENERAL, "Loading Custom Sanctuaries..." );
    sObjectMgr->LoadCustomSanctuary();

    TC_LOG_INFO(LOG_FILTER_GENERAL, "Loading Custom FFA Zones..." );
    ...
    // [Custom][Channel]Envia la informacion PVP
    ...
    ///- Initialize House
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading Houses...");
    sHouseMgr->Load();

    ///- Initialize Event
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading Events...");
    sEventMgr->Load();
    ...
    ///- Initialize ipcountry
    TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading ipcountry...");
    sIpCountryMgr->Load();
...
void World::Update(uint32 diff)
{
    ...
    // Update Anticheat Reports
    sAnticheatMgr->Update(diff);

    // Update houses
    sHouseMgr->Update(diff);
    RecordTimeDiff("HouseMgr");
...
void World::ProcessCliCommands()
{
    ...
    while (cliCmdQueue.next(command))
    {
        ...
        CliHandler handler(callbackArg, zprint, command->_accountId);
...
bool World::hasWorldState(uint32 index) const
{
    WorldStatesMap::const_iterator it = m_worldstates.find(index);
    return it != m_worldstates.end() ? true : false;
}
...
void World::SetCountryRestrict(const std::string& country)
{
    m_contryRestrict = country;
}

const char* World::GetCountryRestrict() const
{
    return m_contryRestrict.c_str();
}
...
void World::SendCustomPvpInformationUpdate()
{
    ...
    for (uint32 battlegroundQueue = BATTLEGROUND_QUEUE_NONE; battlegroundQueue < MAX_BATTLEGROUND_QUEUE_TYPES; ++battlegroundQueue)
    {
        ...
        for (uint32 bracket = BG_BRACKET_ID_FIRST; bracket < MAX_BATTLEGROUND_BRACKETS; ++bracket)
        {
            ...
            {
                for (BattlegroundQueue::GroupsQueueType::const_iterator itr = bgQueue.m_QueuedGroups[bracket][queueType][BG_QUEUE_GUILD_FALSE].begin(); itr != bgQueue.m_QueuedGroups[bracket][queueType][BG_QUEUE_GUILD_FALSE].end(); ++itr)
                {
                    if ((*itr))
                    {
                        // Cuando estamos en las listas de la arena y y la pelea esta sin "rated", te esquiva xD
                    ...
                    if (battlegroundQueue == BATTLEGROUND_QUEUE_2v2 || battlegroundQueue == BATTLEGROUND_QUEUE_3v3 || battlegroundQueue == BATTLEGROUND_QUEUE_5v5)
                    {
                        switch (battlegroundQueue)
                        {
                            case BATTLEGROUND_QUEUE_2v2:
                                messageQueue << "Bracket - 2vs2 : " << allianceCount + hordeCount << " Team(s)";
                                break;
                            case BATTLEGROUND_QUEUE_3v3:
                                messageQueue << "Bracket - 3vs3 : " << allianceCount + hordeCount << " Team(s)";
                                break;
                            case BATTLEGROUND_QUEUE_5v5:
                                messageQueue << "Bracket - 5vs5 : " << allianceCount + hordeCount << " Team(s)";
                                break;
                            default:
                                break;
                        }
    ...
    for (uint32 i = BATTLEGROUND_TYPE_NONE; i < MAX_BATTLEGROUND_TYPE_ID; i++)
    {
        ...
        if (!storedBattlegroundCounts.empty())
        {
            if (!activeGameFound)
            {
                ...
                SendMessageToAllPlayersInChannel("pvp", "========== Active BG's ==========");
            }
            ...
            if (BattlemasterListEntry const* bl = sBattlemasterListStore.LookupEntry(i))
            {
                for (std::map<uint32, std::pair<uint32, uint32> >::const_iterator itr = storedBattlegroundCounts.begin(); itr != storedBattlegroundCounts.end(); ++itr)
                {
                    ...
                    {
                        messageActive << GetBattlegroundAbbreviation((BattlegroundTypeId)i) << " (" << (*itr).first << "-" << (*itr).second.first << "): " << (*itr).second.second << " Battleground's";
    ...
    if (arenaCount2vs2 || arenaCount3vs3 || arenaCount5vs5)
    {
        ...
        if (arenaCount2vs2)
        {
            messageActive << "Arena (Rated) - 2vs2 : " << arenaCount2vs2;
            SendMessageToAllPlayersInChannel("pvp", messageActive.str());
            messageActive.str("");
        }
        ...
        if (arenaCount3vs3)
        {
            messageActive << "Arena (Rated) - 3vs3 : " << arenaCount3vs3;
            SendMessageToAllPlayersInChannel("pvp", messageActive.str());
            messageActive.str("");
        }
        ...
        if (arenaCount5vs5)
        {
            messageActive << "Arena (Rated) - 5vs5 : " << arenaCount5vs5;
            SendMessageToAllPlayersInChannel("pvp", messageActive.str());
            messageActive.str("");
        }
    }
...
void World::LoadGuildLevelingBonus()
{
    m_guild_leveling_bonus.clear();

    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUILD_LEVELING_BONUS);
    ...
    if (result)
    {
        ...
        {
            Field* fields = result->Fetch();
            m_guild_leveling_bonus[fields[0].GetUInt16()] = fields[1].GetUInt8();
        }
        while (result->NextRow());
    }
}

uint8 World::GetReqGuildLevelForBonus(uint16 GuildLevelingBonus)
{
    GuildLevelingBonusMap::iterator itr = m_guild_leveling_bonus.find(GuildLevelingBonus);

    if (itr != m_guild_leveling_bonus.end())
        return itr->second;
    return 0;
}
