...
void AchievementMgr::SetCompletedAchievement(AchievementEntry const* achievement)
{
    // disable for gamemasters with GM-mode enabled
    if (m_player->IsGameMaster())
        return;
    ...
    TC_LOG_INFO(LOG_FILTER_ACHIEVEMENTSYS, "AchievementMgr::SetCompletedAchievement(%u). Player: %s (%u)",
...
void AchievementMgr::DelCompletedAchievement(AchievementEntry const* achievement)
{
    // disable for gamemasters with GM-mode enabled
    if (m_player->IsGameMaster())
        return;
    ...
    TC_LOG_INFO(LOG_FILTER_ACHIEVEMENTSYS, "AchievementMgr::DelCompletedAchievement(%u). Player: %s (%u)",
        achievement->ID, m_player->GetName().c_str(), m_player->GetGUIDLow());
    ...
    /// @todo where do set this instead?
    if (!(achievement->flags & ACHIEVEMENT_FLAG_REALM_FIRST_KILL))
        sAchievementMgr->DelRealmCompleted(achievement);

    // @TODO No existe metodo para revertir los criterios asociados
    //UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_COMPLETE_ACHIEVEMENT);
    //UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_EARN_ACHIEVEMENT_POINTS, achievement->points);
    ...
    // mail
    if (reward->sender)
        GetPlayer()->DestroyItemCount(reward->itemId, 1, true, false);
}
