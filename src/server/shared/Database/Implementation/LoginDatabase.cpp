...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_IP_BANNED, "SELECT bannedby, bandate, unbandate FROM ip_banned WHERE ip = ? ORDER BY bandate DESC LIMIT 1", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_INS_IP_AUTO_BANNED, "INSERT INTO ip_banned (ip, bandate, unbandate, bannedby, banreason) VALUES (?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()+?, 'Login Server', 'Demasiados intentos fallidos de conexion')", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_SEL_ACCOUNT_BANNED, "SELECT bannedby, bandate, unbandate FROM account_banned WHERE id = ? AND active = 1 ORDER BY bandate DESC LIMIT 1", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_INS_ACCOUNT_AUTO_BANNED, "INSERT INTO account_banned VALUES (?, UNIX_TIMESTAMP(), UNIX_TIMESTAMP()+?, 'Login Server', 'Demasiados intentos fallidos de conexion', 1)", CONNECTION_SYNCH);
    ...
    PrepareStatement(LOGIN_SEL_LOGONCHALLENGE, "SELECT a.sha_pass_hash, a.id, aa.gmlevel, a.v, a.s, a.token_key, a.last_ip FROM account a LEFT JOIN account_access aa ON (a.id = aa.id) WHERE a.username = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_UPD_FAILEDLOGINS, "UPDATE account SET failed_logins = failed_logins + 1 WHERE username = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_FAILEDLOGINS, "SELECT id, failed_logins FROM account WHERE username = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_ACCOUNT_ID_BY_NAME, "SELECT id FROM account WHERE username = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_ACCOUNT_LIST_BY_NAME, "SELECT id, username FROM account WHERE username = ?", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_ACCOUNT_INFO_BY_NAME, "SELECT id, sessionkey, last_ip, locked, expansion, mutetime, locale, recruiter, os FROM account WHERE username = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_SEL_PINFO, "SELECT a.username, aa.gmlevel, a.email, a.reg_mail, a.last_ip, DATE_FORMAT(a.last_login, '%Y-%m-%d %T'), a.mutetime, a.mutereason, a.muteby, a.failed_logins, a.OS FROM account a LEFT JOIN account_access aa ON (a.id = aa.id AND (aa.RealmID = ? OR aa.RealmID = -1)) WHERE a.id = ?", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_SEL_PINFO_BANS, "SELECT unbandate, bandate = unbandate, bannedby, banreason FROM account_banned WHERE id = ? AND active ORDER BY bandate ASC LIMIT 1", CONNECTION_ASYNC);
    ...
    PrepareStatement(LOGIN_SEL_ACCOUNTS, "SELECT id, username FROM account WHERE last_ip = (SELECT last_ip FROM account WHERE id = ?)", CONNECTION_ASYNC);

    PrepareStatement(LOGIN_INS_QUESTCOMPLETER, "REPLACE INTO quest_completer (`id`) VALUES (?)", CONNECTION_ASYNC);
