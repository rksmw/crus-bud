...
{
public:
    PlayerInfoCommand(uint32 accId, uint32 lowguid, const char* eMail, const char* regMail, uint32 latency, int64 muteTime, uint8 raceid, uint8 classid, uint8 gender, uint32 totalPlayerTime, uint8 level, const char* alive, uint32 money, uint32 mapId, uint32 areaId, uint32 phase, WorldSession* session, Player* target, const char* targetName)
    : _accId(accId), _lowguid(lowguid), _eMail(eMail), _regMail(regMail), _latency(latency), _muteTime(muteTime), _raceid(raceid), _classid(classid), _gender(gender), _totalPlayerTime(totalPlayerTime), _level(level), _alive(alive), _money(money), _mapId(mapId), _areaId(areaId), _phase(phase), _session(session), _target(target), _targetName(targetName), _security(0), _failedLogins(0), _OS("None"), _muteReason("unknown"), _muteBy("unknown")
    {
