...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "spectate",       RBAC_PERM_COMMAND_SPECTATE_PLAYER,      false, &HandleSpectateCommand,          "", NULL },
            { "watch",          RBAC_PERM_COMMAND_SPECTATE_VIEW,        false, &HandleSpectateFromCommand,      "", NULL },
            { "reset",          RBAC_PERM_COMMAND_SPECTATE_RESET,       false, &HandleSpectateResetCommand,     "", NULL },
            { "leave",          RBAC_PERM_COMMAND_SPECTATE_LEAVE,       false, &HandleSpectatorLeaveCommand,    "", NULL },
            { "version",        RBAC_PERM_COMMAND_SPECTATE_VERSION,     false, &HandleSpectatorVersionCommand,  "", NULL },
            { NULL,             0,                                      false, NULL,                            "", NULL }
        };

        static ChatCommand commandTable[] =
        {
            { "spectator",      RBAC_PERM_COMMAND_SPECTATE,             false,  NULL, "",           spectateCommandTable },
            { NULL,             0,                                      false,  NULL, "",                           NULL }
        };
        return commandTable;
    }
    ...
    static bool HandleSpectateCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (Player* player = handler->GetSession()->GetPlayer())
        {
            ...
            {
                handler->SendSysMessage("No se ha encontrado al jugador.");
            ...
            {
                handler->SendSysMessage("Solo puedes mirar a jugadores.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            {
                handler->SendSysMessage("No puedes mirarte a ti mismo.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            {
                handler->SendSysMessage("Estas en combate.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            if (!target)
            {
                handler->SendSysMessage("No se ha encontrado al jugador que quieres mirar.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            if (player->GetPet())
            {
                handler->SendSysMessage("Para entrar a mirar una arena, debes guardar tu pet/mascota.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            {
                handler->SendSysMessage("Ya estas mirando una arena.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            {
                handler->SendSysMessage("El jugador que quieres mirar no esta en bg/arena.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            {
                handler->SendSysMessage("No puedes hacer esto mientras estas en bg/arena.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            {
                handler->SendSysMessage("El jugador que intentas mirar no esta jugando una arena.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            {
                handler->SendSysMessage("Espera a que la arena empiece para poder mirar a este jugador (aun esta en preparacion).");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            {
                handler->SendSysMessage("Para poder mirar una arena, quita la montura.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            {
                handler->SendSysMessage("No puedes mirar una arena mientras estas en cola para bg o arena.");
                handler->SetSentErrorMessage(true);
                return false;
            }
            ...
            if (bGround->isRated())
            {
                ...
                if (firstTeamID > 0 && secondTeamID > 0 && secondTeamMember)
                {
                    ...
                    {
                        handler->PSendSysMessage("Has entrado a la Arena.");
            ...
            bGround->AddSpectator(player->GetGUID());

            return true;
        }
        else
        {
            handler->SendSysMessage("No se encuentra al jugador.");
            handler->SetSentErrorMessage(true);
            return false;
        }
    }

    static bool HandleSpectatorLeaveCommand(ChatHandler* handler, const char* /*args*/)
    {
        if (WorldSession* session = handler->GetSession())
        {
            if (Player* player = session->GetPlayer())
            {
                ...
                {
                    handler->SendSysMessage("No estas mirando una arena.");
                    handler->SetSentErrorMessage(true);
                    return false;
                }

                if (Unit* spectated = player->GetSpectateFrom())
                    player->SetViewpoint(spectated, false);
    ...
    static bool HandleSpectateFromCommand(ChatHandler* handler, const char *args)
    {
        ...
        if (Player* player = handler->GetSession()->GetPlayer())
        {
            if (!player || !target || target->GetTypeId() != TYPEID_PLAYER)
            {
                handler->PSendSysMessage("No se pudo encontrar al jugador.");
                return false;
            }

            if (!player->IsSpectator())
            {
                handler->PSendSysMessage("No estas espectando.");
                return false;
            }

            if (target->IsSpectator())
            {
                handler->PSendSysMessage("No puedes hacer eso, es un espectador.");
                return false;
            }

            if (player->GetMap() != target->GetMap())
            {
                handler->PSendSysMessage("No puedes cambiar de arena");
                return false;
            }

            if (target->HasAuraType(SPELL_AURA_MOD_STEALTH))
            {
                handler->PSendSysMessage("No puedes espectar personajes invisibles.");
                return false;
            }

            if (target && (target->HasAura(32728) || target->HasAura(32727)))
            {
                handler->SendSysMessage("La arena aun no ha empezado.");
                handler->SetSentErrorMessage(true);
                return false;
            }

            if (player->GetSpectateFrom() == target)
    ...
    static bool HandleSpectateResetCommand(ChatHandler* handler, const char *args)
    {
        ...
        for (Battleground::BattlegroundPlayerMap::const_iterator itr = bGround->GetPlayers().begin(); itr != bGround->GetPlayers().end(); ++itr)
            if (Player* tmpPlayer = ObjectAccessor::FindPlayer(itr->first))
            {
                if (tmpPlayer->IsSpectator() || tmpPlayer->IsGameMaster())
                    continue;
                ...
                msg.SetGuid(tmpPlayer->GetGUID());
                msg.SetPlayer(pName);

                if (Player* target = tmpPlayer->GetSelectedPlayer())
                    msg.SetTarget(target->GetGUID());
                else
                    msg.SetTarget(0);
    ...
    static bool HandleSpectatorVersionCommand(ChatHandler* handler, const char *args)
    {
        if (!*args)
            return true;
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        Gossip2s << "Arenas 2vs2 (Jugando: " << arenasQueueTotal[0] << ")"/* << arenasQueuePlaying[0] << ")"*/;
        std::stringstream Gossip3s;
        Gossip3s << "Arenas 3v3 (Jugando: " << arenasQueueTotal[1] << ")"/* << arenasQueuePlaying[1] << ")"*/;
        std::stringstream Gossip3ss;
        Gossip3ss << "Arenas 3vs3 SoloQueue (Jugando:" << arenasQueueTotal[2] << ")"/* << arenasQueuePlaying[2] << ")"*/;
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        ...
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Actualizar", GOSSIP_SENDER_MAIN, NPC_SPECTATOR_ACTION_3V3S_GAMES);
    ...
    void ShowPage(Player* player, uint16 page, bool IsTop)
    {
        ...
        for (uint8 i = 0; i <= MAX_BATTLEGROUND_TYPE_ID; ++i)
        {
            ...
            for (BattlegroundContainer::const_iterator itr = arenas->m_Battlegrounds.begin(); itr != arenas->m_Battlegrounds.end(); ++itr)
            {
                ...
                else if (IsTop && arena->GetArenaType() == ARENA_TYPE_3v3_SOLO)
                {
                    TypeSolo++;
