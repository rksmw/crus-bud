    ...
    void PlayerInfoCallback(PreparedQueryResult result, PlayerInfoCommand* playerInfoCommand)
    {
        switch (sWorldCallbackScript->_pinfoCallback.GetStage())
        {
            case 0:
            {
                ...
                if (result)
                {
                        ...
                        playerInfoCommand->_session->GetSecurity() >= AccountTypes(playerInfoCommand->_security))
                    {
                        ...
                        playerInfoCommand->_regMail   = fields[3].GetString();
                        playerInfoCommand->_lastIp    = fields[4].GetString();
                        playerInfoCommand->_lastLogin = fields[5].GetString();
                    ...
                    playerInfoCommand->_muteTime      = fields[6].GetUInt64();
                    playerInfoCommand->_muteReason    = fields[7].GetString();
                    playerInfoCommand->_muteBy        = fields[8].GetString();
                    playerInfoCommand->_failedLogins  = fields[9].GetUInt32();
                    playerInfoCommand->_OS            = fields[10].GetString();
                }
            ...
            case 1:
            {
                ...
                if (banTime >= 0)
                    ChatHandler(playerInfoCommand->_session).PSendSysMessage(LANG_PINFO_BANNED, banType.c_str(), banTime > 0 ? secsToTimeString(banTime - time(NULL), true).c_str() : "permanently", banReason.c_str(), bannedBy.c_str());
                ...
                ChatHandler(playerInfoCommand->_session).PSendSysMessage(LANG_PINFO_ACC_LASTLOGIN, playerInfoCommand->_lastLogin.c_str(), playerInfoCommand->_failedLogins);

                // Output VII. LANG_PINFO_ACC_OS
                ChatHandler(playerInfoCommand->_session).PSendSysMessage(LANG_PINFO_ACC_OS, playerInfoCommand->_OS.c_str(), playerInfoCommand->_latency);

                // Output VIII. LANG_PINFO_ACC_REGMAILS
                ChatHandler(playerInfoCommand->_session).PSendSysMessage(LANG_PINFO_ACC_REGMAILS, playerInfoCommand->_regMail.c_str(), playerInfoCommand->_eMail.c_str());
                ...
                ChatHandler(playerInfoCommand->_session).PSendSysMessage(LANG_PINFO_CHR_PLAYEDTIME, (secsToTimeString(playerInfoCommand->_totalPlayerTime, true, true)).c_str());
