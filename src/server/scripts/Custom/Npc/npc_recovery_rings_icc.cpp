...
/*##################
# CUSTOM npc para devolver anillos de ICC
###################*/
...
public:
 ...
 bool OnGossipHello(Player* player, Creature* creature)
 {
     switch (player->GetReputationRank(ASHEN_VERDICT))
     {
         ...
         case REP_UNFRIENDLY:
         case REP_NEUTRAL:
             player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,"Usted no es aliado con Ashen Veredict",GOSSIP_SENDER_MAIN,GOSSIP_ACTION_INFO_DEF+21);
             break;
         case REP_FRIENDLY:
                 ...
                 !player->HasItemCount(T_F,1,true) && player->GetQuestStatus(QUEST_CHOOSE_YOUR_PATH) == QUEST_STATUS_COMPLETE)
             {
                ...
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,"Usted ya posee un anillo de Ashen Veredict",GOSSIP_SENDER_MAIN,GOSSIP_ACTION_INFO_DEF+21);
             break;
         case REP_HONORED:
             if (!player->HasItemCount(C_F,1,true) && !player->HasItemCount(H_F,1,true) && !player->HasItemCount(MS_F,1,true) && !player->HasItemCount(MA_F,1,true) &&
