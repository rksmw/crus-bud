...
public:
    ...
    static bool HandlePhaseGoCommand(ChatHandler * handler, const char * args)
    {
        ...
        if (objectInfo->displayId && !sGameObjectDisplayInfoStore.LookupEntry(objectInfo->displayId))
        {
            ...
            TC_LOG_ERROR(LOG_FILTER_PHASE_SYSTEM, "Gameobject (Entry %u GoType: %u) have invalid displayId (%u), not spawned.", objectId, objectInfo->type, objectInfo->displayId);
    ...
    static bool HandlePhaseGoTargetCommand(ChatHandler * handler, const char * args)
    {
        ...
        if (!result)
        {
            handler->SendSysMessage("Target Note FOund");
            return true;
        }
        ...
        if (!found)
        {
            handler->PSendSysMessage("GameoBject Not Exist", id);
            return false;
        }
        ...
        handler->PSendSysMessage("GameObject Retail", lowguid, goI->name.c_str(), lowguid, id, x, y, z, mapid, o, phase);
        ...
        if (target)
        {
            ...
            handler->PSendSysMessage("RAWPAWNTIMES", defRespawnDelayStr.c_str(), curRespawnDelayStr.c_str());
        }
        return true;
    };
    ...
    static bool HandlePhaseGoDeleteCommand(ChatHandler * handler, const char * args)
    {
        ...
        if (owner_guid)
        {
            ...
            {
                handler->PSendSysMessage("obj Deleted", GUID_LOPART(owner_guid), obj->GetGUIDLow());
                handler->SetSentErrorMessage(true);
                return false;
            }
