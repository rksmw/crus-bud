...
public:
    ...
    ChatCommand* GetCommands() const
    {
        static ChatCommand houseCommandTable[] =
        {
            ...
            { "phaseable",              RBAC_PERM_COMMAND_BC_HOUSE_PHASEABLE,   false, &HandleHousePhaseableCommand,    "", NULL},
        ...
        static ChatCommand eventCommandTable[] =
        {
            ...
            { "phasemask",              RBAC_PERM_COMMAND_BC_EVENT_PHASEMASK,   false, &HandleEventPhaseMaskCommand,    "", NULL},
    ...
    static bool HandleAntiCheatHandleCommand(ChatHandler* handler, const char* args)
    {
        ...
        else if (strCommand.compare("off") == 0)
        {
            ...
            handler->PSendSysMessage("The Anticheat System is: %s", sWorld->getBoolConfig(CONFIG_ANTICHEAT_ENABLE) ? "Enabled" : "Disabled");

        return true;
    }

    static bool HandleAntiCheatListenCommand(ChatHandler* handler, const char* args)
    {
        /* Filtres
        * PLAYER_FLAGS_UNK29 Speed
        * PLAYER_FLAGS_UNK28 Fly
        * PLAYER_FLAGS_UNK26 Tele
        * PLAYER_FLAGS_UNK27 Damage
        * PLAYER_FLAGS_UNK Jump
        * PLAYER_FLAGS_UNK WalkOnWater
        */
        ...
        Player* player = handler->GetSession()->GetPlayer();
        if (strCommand.compare("teleport") == 0)
        {
            if (!player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK26))
            {
                player->SetFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK26);
                handler->SendSysMessage("[AntiCheat] Teleport ON");
            }
            else
            {
                player->RemoveFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK26);
                handler->SendSysMessage("[AntiCheat] Teleport OFF");
            }
        }
        else if (strCommand.compare("fly") == 0)
        {
            if (!player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK28))
            {
                player->SetFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK28);
                handler->SendSysMessage("[AntiCheat] Fly ON");
            }
            else
            {
                player->RemoveFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK28);
                handler->SendSysMessage("[AntiCheat] Fly ON");
            }
        }
        else if (strCommand.compare("speed") == 0)
        {
            if(!player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK29))
            {
                player->SetFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK29);
                handler->SendSysMessage("[AntiCheat] Speed ON");
            }
            else
            {
                player->RemoveFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK29);
                handler->SendSysMessage("[AntiCheat] Speed OFF");
            }
        }
        else if (strCommand.compare("status") == 0)
        {
            handler->SendSysMessage(player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK28) ? "[AntiCheat] Fly ON" : "[AntiCheat] Fly OFF");
            handler->SendSysMessage(player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK26) ? "[AntiCheat] Teleport ON" : "[AntiCheat] Teleport OFF");
            handler->SendSysMessage(player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_UNK29) ? "[AntiCheat] Speed ON" : "[AntiCheat] Speed OFF");
        }
        else
        {
            handler->SendSysMessage("Tipo no encontrado");
            handler->SetSentErrorMessage(true);
            return false;
        }
        return true;
    ...
    {
        TC_LOG_INFO(LOG_FILTER_GENERAL, "Re-Loading ipcountry...");
    ...
    {
        TC_LOG_INFO(LOG_FILTER_GENERAL, "Re-Loading Account Access...");
