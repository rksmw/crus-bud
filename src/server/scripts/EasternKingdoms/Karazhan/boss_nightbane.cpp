...
public:
    ...
    {
        boss_nightbaneAI(Creature* creature) : BossAI(creature, TYPE_NIGHTBANE), summons(me)
        {
            instance = creature->GetInstanceScript();
            instance->SetData64(BOSS_NIGHTBANE, creature->GetGUID());
            Intro   = true;
            Started = false;
        }
...
/*######
## go_blackened_urn
## Will start combat with Nightbane
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* /*go*/)
    {
        if (_instance = player->GetInstanceScript())
        {
            ...
            {
                if (Creature* nightbane = ObjectAccessor::GetCreature(*player, _instance->GetData64(BOSS_NIGHTBANE)))
                    nightbane->AI()->AttackStart(player);
            }
        }
        return false;
    }
