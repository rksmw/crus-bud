...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action) OVERRIDE
    {
        ...
        switch (action)
        {
            case GOSSIP_ACTION_INFO_DEF+1:
            {
                ...
                Creature* temp = creature->SummonCreature(NPC_HIGHLORD_DARION_MOGRAINE, creature->GetPositionX(), creature->GetPositionY(), creature->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, DAY * IN_MILLISECONDS);
                temp->SetEventVisibleFor(player, true);
                creature->SetEventVisibleFor(player, false);

                if (Creature* pKoltira = GetClosestCreatureWithEntry(creature, NPC_KOLTIRA_DEATHWEAVER, 50.0f))
                {
                    temp->SummonCreature(NPC_KOLTIRA_DEATHWEAVER, pKoltira->GetPositionX(), pKoltira->GetPositionY(), pKoltira->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, DAY * IN_MILLISECONDS);
                    pKoltira->SetEventVisibleFor(player, false);
                }
                if (Creature* pOrbaz = GetClosestCreatureWithEntry(creature, NPC_ORBAZ_BLOODBANE, 50.0f))
                {
                    temp->SummonCreature(NPC_ORBAZ_BLOODBANE, pOrbaz->GetPositionX(), pOrbaz->GetPositionY(), pOrbaz->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, DAY * IN_MILLISECONDS);
                    pOrbaz->SetEventVisibleFor(player, false);
                }
                if (Creature* pThassarian = GetClosestCreatureWithEntry(creature, NPC_THASSARIAN, 50.0f))
                {
                    temp->SummonCreature(NPC_THASSARIAN, pThassarian->GetPositionX(), pThassarian->GetPositionY(), pThassarian->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, DAY * IN_MILLISECONDS);
                    pThassarian->SetEventVisibleFor(player, false);
                }

                CAST_AI(npc_highlord_darion_mograine::npc_highlord_darion_mograineAI, temp->AI())->uiStep = 1;
                CAST_AI(npc_highlord_darion_mograine::npc_highlord_darion_mograineAI, temp->AI())->Start(true, false, player->GetGUID());
                break;
            }
        }
        return true;
    ...
    struct npc_highlord_darion_mograineAI : public npc_escortAI
    {
        ...
        {
            if (me->IsSummon() || me->GetEntry() != NPC_HIGHLORD_DARION_MOGRAINE)
                return;

            for (WorldObject::EventVisible::const_iterator itr = creature->_EventVisibilityPlayers.begin(); itr != creature->_EventVisibilityPlayers.end(); itr++)
            {
                me->DelEventVisibleFor((*itr).first);

                if (Creature* pKoltira = GetClosestCreatureWithEntry(me, NPC_KOLTIRA_DEATHWEAVER, 50.0f))
                    pKoltira->DelEventVisibleFor((*itr).first);
                if (Creature* pOrbaz = GetClosestCreatureWithEntry(me, NPC_ORBAZ_BLOODBANE, 50.0f))
                    pOrbaz->DelEventVisibleFor((*itr).first);
                if (Creature* pThassarian = GetClosestCreatureWithEntry(me, NPC_THASSARIAN, 50.0f))
                    pThassarian->DelEventVisibleFor((*itr).first);
            }
        }
                ...
                if (uiPhase_timer <= diff)
                {
                    ...
                    switch (uiStep)
                    {
                        ...
                        case 71:
                            ...
                            //    go->SetPhaseMask(0, true);
                            {
                                ...
                                if (!PlayerList.isEmpty())
                                {
                                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                                        if (i->GetSource()->IsAlive() && me->IsWithinDistInMap(i->GetSource(), 50) && me->IsEventVisibleFor(i->GetSource()))
                            ...
                            me->SummonCreature(NPC_HIGHLORD_DARION_MOGRAINE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, DAY * IN_MILLISECONDS);
                            JumpToNextStep(1000);
                            break;
