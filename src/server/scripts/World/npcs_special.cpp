...
/*######
## npc_torch_tossing_target_controller
######*/

enum eTorchTossing
{
    NPC_TORCH_TOSSING_BUNNY      = 25535,
    SPELL_TARGET_INDICATOR       = 43313
};

class npc_torch_tossing_target_controller : public CreatureScript
{
public:
    npc_torch_tossing_target_controller() : CreatureScript("npc_torch_tossing_target_controller") { }

    CreatureAI* GetAI(Creature* creature) const OVERRIDE
    {
        return new npc_torch_tossing_target_controllerAI(creature);
    }

    struct npc_torch_tossing_target_controllerAI : ScriptedAI
    {
        npc_torch_tossing_target_controllerAI(Creature* creature) : ScriptedAI(creature) {}

        uint32 ChangeTimer;
        uint64 LastBunnyGUID;
        uint64 NewBunnyGUID;
        ...
        void ChangeTarget()
        {
            // Delete the old bunny brand
            if (Creature* targetold = Creature::GetCreature(*me, LastBunnyGUID))
                targetold->RemoveAura(SPELL_TARGET_INDICATOR);

            // Looking for a new bunny
            std::list<Creature*> targets;
            me->GetCreatureListWithEntryInGrid(targets, NPC_TORCH_TOSSING_BUNNY, 60.0f);
            if (!targets.empty())        
                while (NewBunnyGUID == 0 || NewBunnyGUID == LastBunnyGUID)
                {
                    Creature* newtarget = Trinity::Containers::SelectRandomContainerElement(targets);
                    NewBunnyGUID = newtarget->GetGUID();
                }

                // Putting the brand new bunny
                if (Creature* targetnew = Creature::GetCreature(*me, NewBunnyGUID))
                    targetnew->CastSpell(targetnew,SPELL_TARGET_INDICATOR,true);

                LastBunnyGUID = NewBunnyGUID;
                ChangeTimer = 3000;
        }
...
enum eRibbonPole
{
    NPC_RIBBON_POLE_FIRE_SPIRAL  = 25303,
    SPELL_CHANNEL_1              = 29705,
    SPELL_CHANNEL_2              = 29726,
    SPELL_CHANNEL_3              = 29727,
    SPELL_RIBBON_POLE_DANCER     = 45406,
    SPELL_HAS_CLOTH              = 58933,
    SPELL_GIVE_ACHIEVEMENT       = 58934
};

class npc_ribbon_pole_debug_target : public CreatureScript
{
public:
    npc_ribbon_pole_debug_target() : CreatureScript("npc_ribbon_pole_debug_target") { }

    CreatureAI* GetAI(Creature* creature) const OVERRIDE
    {
        return new npc_ribbon_pole_debug_targetAI(creature);
    }

    struct npc_ribbon_pole_debug_targetAI : ScriptedAI
    {
        npc_ribbon_pole_debug_targetAI(Creature* creature) : ScriptedAI(creature) {}
        ...
        void IsSummonedBy(Unit* owner)
        {
            ...
            {
            case 1:
                owner->CastSpell(me,SPELL_CHANNEL_1);
                break;
            case 2:
                owner->CastSpell(me,SPELL_CHANNEL_2);
                break;
            case 3:
                owner->CastSpell(me,SPELL_CHANNEL_3);
                break;
            default:
                break;
            }
        ...
        void UpdateAI(uint32 diff) OVERRIDE
        {
            if (Timer <= diff)
            {
                if (!SpinStarted)
                {
                    if (Player* owner = Player::GetPlayer(*me, PlayerGUID))
                        owner->CastSpell(owner,SPELL_RIBBON_POLE_DANCER,true);
                    SpinStarted = true;
                    Timer = 57000;
                }
                ...
                {
                    if (Player* owner = Player::GetPlayer(*me, PlayerGUID))
                        if (owner->HasAura(SPELL_HAS_CLOTH))
                            owner->CastSpell(owner,SPELL_GIVE_ACHIEVEMENT,true);
                }

            }
            else Timer -= diff;

            if (SpinStarted && !me->HasAura(SPELL_CHANNEL_1) && !me->HasAura(SPELL_CHANNEL_2) && !me->HasAura(SPELL_CHANNEL_3))
            {
                if(Creature* bunny = me->FindNearestCreature(NPC_RIBBON_POLE_FIRE_SPIRAL,30.0f))
...
void AddSC_npcs_special()
{
    ...
    new npc_risen_ally();
    new npc_torch_tossing_target_controller();
    new npc_ribbon_pole_debug_target();
    //new npc_ghoul();
}
