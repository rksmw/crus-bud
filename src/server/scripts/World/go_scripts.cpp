...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) OVERRIDE
    {
        ...
        {
            if(Creature* creature = go->SummonCreature(25318, 3485.089844f, 6115.7422188f, 70.966812f, 0, TEMPSUMMON_TIMED_DESPAWN, 7000))
                if(creature)
                    creature->AI()->Talk(SAY_CHAU, player);
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) OVERRIDE
    {
        ...
        {
            go->UseDoorOrButton();
            std::list<Creature*> tadpoleList;
            GetCreatureListWithEntryInGrid(tadpoleList, go, NPC_WINTERFIN_TADPOLE, INTERACTION_DISTANCE);
            for (std::list<Creature*>::const_iterator itr = tadpoleList.begin(); itr != tadpoleList.end(); ++itr)
            {
                (*itr)->DespawnOrUnsummon(0);
                player->KilledMonsterCredit(NPC_WINTERFIN_TADPOLE, (*itr)->GetGUID());
            }
        }
        return true;
    }
...
enum EvanorsP
{
    NPC_ARCHMAGE_EVANOR = 25784,
    QUEST_RESCUING_EVANOR = 11681,
    SPELL_TELEPORT = 46018,
};
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) OVERRIDE
    {
            if (player->GetQuestStatus(QUEST_RESCUING_EVANOR) == QUEST_STATUS_COMPLETE)
            {
                go->UseDoorOrButton(10);
                if (Creature* creature = player->SummonCreature(NPC_ARCHMAGE_EVANOR, 3212.01f,6091.77f,138.00f, 3.920693f, TEMPSUMMON_DEAD_DESPAWN, 60000))
                {
                    creature->AI()->Talk(0);
...
enum RibbonPole
{
    NPC_RIBBON_TRIGGER  = 17066,
    NPC_RIBBON_BUNNY    = 25303,
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* go) OVERRIDE
    {
        if (!player->HasAura(SPELL_CHANNEL_1) && !player->HasAura(SPELL_CHANNEL_2) && !player->HasAura(SPELL_CHANNEL_3))
        {
            player->SummonCreature(NPC_RIBBON_TRIGGER, go->GetPositionX(), go->GetPositionY(), go->GetPositionZ() + 6.0f, TEMPSUMMON_MANUAL_DESPAWN);

            if (!player->FindNearestCreature(NPC_RIBBON_BUNNY,20.0f))
                player->SummonCreature(NPC_RIBBON_BUNNY, go->GetPositionX(), go->GetPositionY(), go->GetPositionZ(), TEMPSUMMON_MANUAL_DESPAWN);
        }
