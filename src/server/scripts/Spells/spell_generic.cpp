...
class spell_gen_rocket_blast_trigger : public SpellScriptLoader
{
public:
    spell_gen_rocket_blast_trigger() : SpellScriptLoader("spell_rocket_blast_trigger") { }

    class spell_gen_rocket_blast_trigger_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_gen_rocket_blast_trigger_SpellScript);

        void RecalculateDamage()
        {
            Unit* target = GetHitUnit();

            const uint32 newdamage = GetSpellInfo()->Effects[EFFECT_0].BasePoints;

            if (!target)
                return;

            if (target->GetCreatureType() != CREATURE_TYPE_MECHANICAL)
            {
                SetHitDamage(newdamage);// 3999
            }
        }

        void Register()
        {
            OnHit += SpellHitFn(spell_gen_rocket_blast_trigger_SpellScript::RecalculateDamage);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_gen_rocket_blast_trigger_SpellScript();
    }
};

enum SpellListeningMusic
{
    SPELL_LISTENING_MUSIC = 50493
};

class spell_gen_listening_music_disco : public SpellScriptLoader
{
public:
    spell_gen_listening_music_disco() : SpellScriptLoader("spell_gen_listening_music_disco") {}

    class spell_gen_listening_music_disco_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_gen_listening_music_disco_AuraScript);
        ...
        void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
        {
            ...
            Unit* caster = GetCaster();
            if (!caster || target->isMoving() || target->isTurning() || target->IsFlying() || target->IsFalling())
                target->RemoveAurasDueToSpell(SPELL_LISTENING_MUSIC);
            else
        ...
        void Register() OVERRIDE
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_gen_listening_music_disco_AuraScript::HandleEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
        }
    };

    AuraScript* GetAuraScript() const OVERRIDE
    {
        return new spell_gen_listening_music_disco_AuraScript();
    }
};

enum SpellTauntFlagTargeting
{
...
public:
    ...
    class spell_gen_taunt_flag_targeting_SpellScript : public SpellScript
    {
        ...
        SpellCastResult CheckCast()
        {
            if (!GetExplTargetUnit() || GetExplTargetUnit()->IsAlive())
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            Corpse* corpse = target->ToPlayer()->GetCorpse();
            caster->CastSpell(corpse->GetPositionX(), corpse->GetPositionY(), corpse->GetPositionZ(), SPELL_TAUNT_FLAG, true);
        }

        void Register() OVERRIDE
        {
            OnCheckCast += SpellCheckCastFn(spell_gen_taunt_flag_targeting_SpellScript::CheckCast);
...
class spell_vanity_pet_focus : public SpellScriptLoader
{
public:
    spell_vanity_pet_focus(const char* name, uint32 _passiveId = 0) : SpellScriptLoader(name), passiveId(_passiveId) { }

    class spell_vanity_pet_focus_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_vanity_pet_focus_SpellScript);

    public:
        spell_vanity_pet_focus_SpellScript(int32 _passiveId) : SpellScript(), passiveId(_passiveId) { }
        ...
        void Register() OVERRIDE
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_vanity_pet_focus_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
            OnEffectHitTarget += SpellEffectFn(spell_vanity_pet_focus_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    ...
    SpellScript* GetSpellScript() const OVERRIDE
    {
        return new spell_vanity_pet_focus_SpellScript(passiveId);
    }
            ...
            if (GetCaster()->HasAura(SPELL_MIXOLOGY) && GetCaster()->HasSpell(GetSpellInfo()->Effects[EFFECT_0].TriggerSpell))
            {
                switch (GetId())
                {
                    ...
                    case SPELL_ELIXIR_OF_GIANTH_GROWTH:
                        ...
                        break;
                    default:
                        TC_LOG_ERROR(LOG_FILTER_TSCR, "SpellId %d couldn't be processed in spell_gen_mixology_bonus", GetId());
                        break;
                }
                amount += bonus;
            }
...
void AddSC_generic_spell_scripts()
{
    ...
    new spell_gen_rocket_blast_trigger();
    new spell_gen_listening_music_disco();
    ...
    new spell_vanity_pet_focus("spell_lil_kt_focus");
    new spell_vanity_pet_focus("spell_willy_focus");
