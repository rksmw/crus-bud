...
// hhttp://www.wowhead.com/item=31279 Enchanted Illidari Tabard
// 38224 Illidari Agent Disguise
enum IllidariDisguise
{
    SPELL_ILLIDARI_DISGUISE_MALE     = 38225,
    SPELL_ILLIDARI_DISGUISE_FEMALE   = 38227,
};

class spell_item_illidari_agent_illusion : public SpellScriptLoader
{
public:
    spell_item_illidari_agent_illusion() : SpellScriptLoader("spell_item_illidari_agent_illusion") {}

    class spell_item_illidari_agent_illusion_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_item_illidari_agent_illusion_AuraScript);

        void HandleOnEffectApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            Unit* caster = GetCaster();
            uint32 spellId = 0;
            spellId = (caster->getGender() == GENDER_MALE ? SPELL_ILLIDARI_DISGUISE_MALE : SPELL_ILLIDARI_DISGUISE_FEMALE);
            caster->CastSpell(caster, spellId, true, NULL);
        }
        ...
        {
            Unit* target = GetTarget();
            target->RemoveAura(SPELL_ILLIDARI_DISGUISE_MALE);
            target->RemoveAura(SPELL_ILLIDARI_DISGUISE_FEMALE);
        }

        void Register() OVERRIDE
        {
            OnEffectApply += AuraEffectApplyFn(spell_item_illidari_agent_illusion_AuraScript::HandleOnEffectApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(spell_item_illidari_agent_illusion_AuraScript::HandleOnEffectRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const OVERRIDE
    {
        return new spell_item_illidari_agent_illusion_AuraScript();
    }
};

// http://www.wowhead.com/item=40946 Anuniaq's Net
// 21014 Anuniaq's Net - Quest - The Way to His Heart...
enum AnuniaqNet
{
...
class spell_item_anuniaq_net : public SpellScriptLoader
{
public:
    spell_item_anuniaq_net(): SpellScriptLoader("spell_item_anuniaq_net") { }

    class spell_item_anuniaq_net_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_item_anuniaq_net_SpellScript);
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
                if (GameObject* go = caster->FindNearestGameObject(GO_TASTY_REEF_FISH, 10.0f))
                    if (!go->IsInvisibleDueToDespawn())
                    {
                        go->SetGoType(GAMEOBJECT_TYPE_GOOBER);
        ...
        void Register() OVERRIDE
        {
            OnEffectHit += SpellEffectFn(spell_item_anuniaq_net_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const OVERRIDE
    {
        return new spell_item_anuniaq_net_SpellScript();
    }
};
...
void AddSC_item_spell_scripts()
{
    ...
    new spell_item_illidari_agent_illusion();
    new spell_item_anuniaq_net();
}
