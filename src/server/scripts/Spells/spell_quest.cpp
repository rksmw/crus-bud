...
// Item - Toalu'u's Spiritual Incense   - http://www.wowhead.com/item=35907
// Quest - Spiritual Insight            - http://www.wowhead.com/quest=12028
enum ItemToalu
{
    SPELL_TOALUUS_SPIRITUAL_INCENSE = 47190,
    SPELL_SPIRITUAL_INSIGHT         = 47189,
};

class spell_q12028_toalu_spiritual_incense : public SpellScriptLoader
{
    public:
        spell_q12028_toalu_spiritual_incense() : SpellScriptLoader("spell_q12028_toalu_spiritual_incense") { }

        class spell_q12028_toalu_spiritual_incense_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_q12028_toalu_spiritual_incense_AuraScript);

            bool Validate(SpellInfo const* /*spellInfo*/) OVERRIDE
            {
                if (!sSpellMgr->GetSpellInfo(SPELL_TOALUUS_SPIRITUAL_INCENSE))
                    return false;
                return true;
            }
            ...
            {
                if (Unit* target = GetTarget())
                    target->CastSpell(target, SPELL_SPIRITUAL_INSIGHT);
            }
            
            void Register() OVERRIDE
            {
                OnEffectApply += AuraEffectApplyFn(spell_q12028_toalu_spiritual_incense_AuraScript::HandleEffectApply, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };

        AuraScript* GetAuraScript() const OVERRIDE
        {
            return new spell_q12028_toalu_spiritual_incense_AuraScript();
        }
};

// http://www.wowhead.com/quest=11421 It Goes to 11
// 44214 - Fiery Harpoon
enum eDragonflayer
{
    NPC_DRAGONFLAYER_LONGHOUSE = 24538,
    NPC_DRAGONFLAYER_DOCKHOUSE = 24646,
    NPC_DRAGONFLAYER_STORG_FAC = 24647,
    NPC_DRAGONFLAYER_DEFENDER  = 24533,
    SAY_DESTROYED              = 0
};

class spell_q11421_fiery_harpoon : public SpellScriptLoader
{
public:
    spell_q11421_fiery_harpoon() : SpellScriptLoader("spell_q11421_fiery_harpoon") { }

    class spell_q11421_fiery_harpoon_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_q11421_fiery_harpoon_SpellScript);

        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (GetHitCreature()->GetEntry() == NPC_DRAGONFLAYER_LONGHOUSE || GetHitCreature()->GetEntry() == NPC_DRAGONFLAYER_DOCKHOUSE || GetHitCreature()->GetEntry() == NPC_DRAGONFLAYER_STORG_FAC)
            {
                Creature* house = GetHitCreature(); 
                if (Unit* caster = GetOriginalCaster())
                    if (Vehicle* vehicle = caster->GetVehicleKit())
                        if (Unit* passenger = vehicle->GetPassenger(0))
                            if (Player* player = passenger->ToPlayer())
                            {
                                player->KilledMonsterCredit(house->GetEntry(), 0);
                                house->AI()->Talk(SAY_DESTROYED, player);
                                for (uint8 i = 1; i < 4; ++i)
                                {
                                    TempSummon* defender = house->SummonCreature(NPC_DRAGONFLAYER_DEFENDER,1027.988f,-5209.707f,195.625f,0.0f,TEMPSUMMON_CORPSE_TIMED_DESPAWN,4000);
                                    defender->AI()->SetData(0,i);
                                }
                            }
            }
        }
        ...
        void Register() OVERRIDE
        {
            OnEffectHitTarget += SpellEffectFn(spell_q11421_fiery_harpoon_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const OVERRIDE
    {
        return new spell_q11421_fiery_harpoon_SpellScript();
    }
};
...
void AddSC_quest_spell_scripts()
{
    ...
    new spell_q12028_toalu_spiritual_incense();
    new spell_q11421_fiery_harpoon();
