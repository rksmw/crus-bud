...
enum DeathKnightSpells
{
    ...
    SPELL_DK_RAISE_ALLY_TARGET                  = 46619
};
    ...
    public:
        ...
        class spell_dk_death_grip_SpellScript : public SpellScript
        {
            ...
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                if (Unit* target = GetHitUnit())
                {
                    if (target->HasAuraType(SPELL_AURA_REFLECT_SPELLS)) // Prevenir que se aplique si tiene reflejar hechizos
                        return;

                    if (!target->HasAuraType(SPELL_AURA_DEFLECT_SPELLS)) // Deterrence
                        target->CastSpell(pos->GetPositionX(), pos->GetPositionY(), target->GetMapId() == 618 && GetCaster() ? GetCaster()->GetPositionZ() : pos->GetPositionZ(), damage, true);
                }
            }
...
// 56222 Dark Command
// Hack para que al usar Dark Command tenga % de proc de que salte los encantamientos de runas del DK
enum DarkCommand
{
    SPELL_UNHOLY_STRENGTH           = 53365,
    SPELL_CINDERGLACIER             = 53386,
    ENCHANT_RUNE_OTFC               = 3368,
    ENCHANT_RUNE_OF_CINDERGLACIER   = 3369,
};

class spell_dk_dark_command : public SpellScriptLoader
{
public:
    spell_dk_dark_command() : SpellScriptLoader("spell_dk_dark_command") { }

    class spell_dk_dark_command_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dk_dark_command_SpellScript);

            bool Validate(SpellInfo const* /*spellEntry*/)
        {
            if (!sSpellMgr->GetSpellInfo(DK_SPELL_DARK_COMMAND))
                return false;
            return true;
        }
        ...
        {
            // Le pongo el chance de 5% mismo, no es muy alto el % de que salte, asi que lo veo bien asi.
            int32 chance = 5;
            ...
            if (item)
            {
                ...
                if (SpellItemEnchantmentEntry const* enchant = sSpellItemEnchantmentStore.LookupEntry(item->GetEnchantmentId(slot) == ENCHANT_RUNE_OTFC))
                {
                    if (roll_chance_i(chance))
                        player->CastSpell(player, SPELL_UNHOLY_STRENGTH, true);
                }
                // Rune of Cinderglacier Main hand
                else if (SpellItemEnchantmentEntry const* enchant = sSpellItemEnchantmentStore.LookupEntry(item->GetEnchantmentId(slot) == ENCHANT_RUNE_OF_CINDERGLACIER))
                {
                    if (roll_chance_i(chance))
                        player->CastSpell(player, SPELL_CINDERGLACIER, true);
                }
            }
            if (offitem)
            {
                // Rune of the Fallen Crusader Off-hand
                if (SpellItemEnchantmentEntry const* enchant = sSpellItemEnchantmentStore.LookupEntry(offitem->GetEnchantmentId(slot) == ENCHANT_RUNE_OTFC))
                {
                    if (roll_chance_i(chance))
                        player->CastSpell(player, SPELL_UNHOLY_STRENGTH, true);
                }
                // Rune of Cinderglacier Off-hand
                else if (SpellItemEnchantmentEntry const* enchant = sSpellItemEnchantmentStore.LookupEntry(offitem->GetEnchantmentId(slot) == ENCHANT_RUNE_OF_CINDERGLACIER))
                {
                    if (roll_chance_i(chance))
                        player->CastSpell(player, SPELL_CINDERGLACIER, true);
                }
        ...
        void Register()
        {
            OnEffectHit += SpellEffectFn(spell_dk_dark_command_SpellScript::HandleEffect, EFFECT_1, SPELL_EFFECT_APPLY_AURA);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_dk_dark_command_SpellScript();
    }
};

class spell_dk_death_grip_trigger : public SpellScriptLoader
{
public:
    spell_dk_death_grip_trigger() : SpellScriptLoader("spell_dk_death_grip_trigger") { }

    class spell_dk_death_grip_trigger_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dk_death_grip_trigger_SpellScript);

        SpellCastResult CheckCast()
        {
            Unit* caster = GetCaster();
            // Check para que no pueda usar death grip si el DK esta saltando o cayendo
            if (caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING) || caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING_FAR))
        ...
        void Register()
        {
            OnCheckCast += SpellCheckCastFn(spell_dk_death_grip_trigger_SpellScript::CheckCast);
        }
    ...
    SpellScript* GetSpellScript() const
    {
        return new spell_dk_death_grip_trigger_SpellScript();
    }
};
...
enum RaiseDead
{
    ...
    SPELL_RAISE_DEAD_REMOVE_ITEM    = 48289, // spell handling Corpse Dust removal
};
    ...
    class spell_dk_raise_dead_SpellScript : public SpellScript
    {
        ...
        SpellCastResult CheckIfCorpseNear()
        {
            ...
            if (!unitTarget)
            {
                ...
                else if (((Player*)caster)->HasItemCount(ITEM_CORPSE_DUST, 1))
                {
                    caster->CastSpell(caster, SPELL_RAISE_DEAD_REMOVE_ITEM);
                    caster->CastSpell(caster->GetPositionX(), caster->GetPositionY(), caster->GetPositionZ(), triggered_spell_id, true);
                    return SPELL_CAST_OK;
                }
                else
...
public:
    ...
    class spell_dk_raise_ally_SpellScript : public SpellScript
    {
        ...
        {
            if (Unit* target = GetHitUnit())
                target->CastSpell(target, SPELL_DK_RAISE_ALLY_TARGET, true);
        }
        ...
        SpellCastResult CheckCast()
        {
            ...
            {
                if (!player->IsInPartyWith(caster) && !player->IsInRaidWith(caster))
        ...
        void Register() OVERRIDE
        {
            OnCheckCast += SpellCheckCastFn(spell_dk_raise_ally_SpellScript::CheckCast);
            ...
            void CalculateAvoidanceAmount(AuraEffect const* /* aurEff */, int32& amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* pet = GetUnitOwner())
            {
                ...
                if (!owner)
                    return;
                // Night of the dead
                if (owner->HasAura(55623)) //RANK2
                    amount = -90.0f;
                else if (owner->HasAura(55620)) //RANK1
                    amount = -45.0f;
            }
        }
    ...
    class spell_dk_pet_scaling_01_AuraScript : public AuraScript
    {
            ...
            if (Unit* pet = GetUnitOwner())
            {
                if (pet->IsGuardian())
                {
                    if (Unit* owner = pet->GetOwner())
                    {
                        ...
                        if (aurEff)
                        {
                            if (SpellInfo const* info = aurEff->GetSpellInfo()) // Then get the SpellProto and add the dummy effect value
                                mod += CalculatePct(mod, aurEff->GetSpellInfo()->Effects[EFFECT_1].CalcValue());
                        }

                        // Glyph of the Ghoul
                        aurEff = owner->GetAuraEffect(SPELL_DEATH_KNIGHT_GLYPH_OF_GHOUL, 0);
                        if (aurEff)
        ...
        void CalculateDamageDoneAmount(AuraEffect const* /* aurEff */, int32& amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* pet = GetUnitOwner())
            {
                ...
                float mod = 1.0f;
                //bonus damage moved from statsystem some auras SPELL_AURA_MOD_DAMAGE_DONE?
                ((TempSummon*)pet)->SetBonusDamage(int32(owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.1287f * mod));
        ...
        void Register() OVERRIDE
        {
            ...
            {
            case SPELL_DEATH_KNIGHT_PET_SCALING_01:
    ...
    class spell_dk_pet_scaling_02_AuraScript : public AuraScript
    {
            ...
            void CalculateBonusDamagePct(AuraEffect const* /* aurEff */, int32& amount, bool& /*canBeRecalculated*/)
        {
            ...
            if (Player* owner = GetCaster()->GetOwner()->ToPlayer())
            {
                ...
                for (Unit::AuraEffectList::const_iterator itr = auraDamagePctList.begin(); itr != auraDamagePctList.end(); ++itr)
                    if ((*itr)->GetMiscValue() & SPELL_SCHOOL_MASK_ALL)
        ...
        void CalculateAmountMeleeHaste(AuraEffect const* /* aurEff */, int32& amount, bool& /*canBeRecalculated*/)
        {
            ...
            if (Player* owner = GetCaster()->GetOwner()->ToPlayer())
            {
                ...
                // Increase from owner haste or attackspeed  
        ...
        void Register() OVERRIDE
        {
            switch (m_scriptSpellId)
            {
            case SPELL_DEATH_KNIGHT_PET_SCALING_02:
...
public:
    ...
    class spell_dk_rune_weapon_scaling_02_AuraScript : public AuraScript
    {
        ...
        void Register() OVERRIDE
        {
            ...
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_dk_rune_weapon_scaling_02_AuraScript::CalculateBonusDamagePct, EFFECT_2, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE);
        }
    };
    AuraScript* GetAuraScript() const OVERRIDE
    {
...
void AddSC_deathknight_spell_scripts()
{
    ...
    new spell_dk_dark_command();
    new spell_dk_death_grip_trigger();
