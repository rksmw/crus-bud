...
enum MageSpells
{
    ...
    SPELL_MAGE_DEEP_FREEZE_TRIGGERED             = 71757,
};
...
class spell_mage_cold_snap : public SpellScriptLoader
{
    public:
        ...
        class spell_mage_cold_snap_SpellScript : public SpellScript
        {
            ...
            void Register() OVERRIDE
            {
                OnEffectHitTarget += SpellEffectFn(spell_mage_cold_snap_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
            }
        };
...
class spell_mage_focus_magic : public SpellScriptLoader
{
    public:
        ...
        class spell_mage_focus_magic_AuraScript : public AuraScript
        {
            ...
            {
                PreventDefaultAction();
                if (Player* player = _procTarget->ToPlayer())
                {
                    if (player->HasTalent(SPELL_MAGE_FOCUS_MAGIC, player->GetActiveSpec()))
                        GetTarget()->CastSpell(_procTarget, SPELL_MAGE_FOCUS_MAGIC_PROC, true, NULL, aurEff);
                    else
                        GetTarget()->RemoveAura(SPELL_MAGE_FOCUS_MAGIC);
                }
                else
                    GetTarget()->RemoveAura(SPELL_MAGE_FOCUS_MAGIC);
            }
...
public:
    ...
    class spell_mage_deep_freeze_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellInfo const * /*spellInfo*/) OVERRIDE
        {
            if (!sSpellStore.LookupEntry(SPELL_MAGE_DEEP_FREEZE_TRIGGERED))
                return false;
            return true;
        }

        SpellCastResult ImmunityDamage()
        {
            ...
            if (target->ToCreature()->GetCreatureTemplate()->MechanicImmuneMask & (1 << (MECHANIC_STUN - 1)))
            {
                GetCaster()->CastSpell(target, SPELL_MAGE_DEEP_FREEZE_TRIGGERED, true);

                // Apply a small cooldown to prevent triggered spell from hitting twice
                if (GetCaster()->GetTypeId() == TYPEID_PLAYER)
                    GetCaster()->ToPlayer()->AddSpellCooldown(SPELL_MAGE_DEEP_FREEZE_TRIGGERED, 0, time(NULL) + 1);
            }
            return SPELL_CAST_OK;
        }
        ...
        void Register() OVERRIDE
        {
            OnCheckCast += SpellCheckCastFn(spell_mage_deep_freeze_SpellScript::ImmunityDamage);
        }
    };
