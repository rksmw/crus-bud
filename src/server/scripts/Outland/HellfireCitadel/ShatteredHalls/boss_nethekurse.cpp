...
enum Spells
{
    ...
    SPELL_KARGATH_EXECUTIONER   = 39288     // Quest Buff 'Trial_of_the_Naaru: Mercy'
                ...
                if (IsHeroic())
                {
                    ...
                    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                    {
                        if (Player* player = itr->GetSource())
                            if (player->GetQuestStatus(10884) == QUEST_STATUS_INCOMPLETE)
                                me->CastSpell(player, SPELL_KARGATH_EXECUTIONER);
                    }
                }
