...
class npc_chief_archaeologist : public CreatureScript
{
public:
    npc_chief_archaeologist() : CreatureScript("npc_chief_archaeologist") { }

    struct npc_chief_archaeologistAI : public npc_escortAI
    {
        npc_chief_archaeologistAI(Creature* creature) : npc_escortAI(creature) {}

        void Hablar(uint8 SAY,uint32 tempGUID)
       {
           std::list<Creature*> tempcreature;
           tempcreature.clear();
           me->GetCreatureListWithEntryInGrid(tempcreature, 22464, 20.0f);

           if (!tempcreature.empty())
           {
               for (std::list<Creature*>::iterator itr = tempcreature.begin(); itr != tempcreature.end(); itr++)
               {
                   if(GUID_LOPART((*itr)->GetGUID()) == tempGUID)
                   {
                       (*itr)->AI()->Talk(SAY);
                   }
               }
       ...
       void Despawn()
       {
        ...
        void WaypointReached(uint32 waypointId)
        {
            ...
            switch (waypointId)
            {
                ...
                case 23:
                    ...
                    break;
                case 24:
                    me->SummonGameObject(202882, me->GetPositionX()+1,me->GetPositionY()+1, me->GetPositionZ(), 0, 0, 0, 0, 0, 1000);
                    Hablar(SAY_COMP1,78837);
                    break;
                case 25:
                    Hablar(SAY_COMP2,78837);
                    break;
                case 26:
                    Hablar(SAY_COMP3,78839);
                    break;
                case 27:
                    Talk(SAY_EVENT07);
                    ...
                    break;
                case 28:
                    Hablar(SAY_COMP4,78838);
                    break;
                case 29:
                    Hablar(SAY_COMP5,78839);
                    break;
                case 30:
                    Hablar(SAY_COMP6,78840);
                    break;
                case 31:
                    Hablar(SAY_COMP7,78838);
                    break;
                case 32:
                    Talk(SAY_EVENT08);
                    me->HandleEmoteCommand(EMOTE_ONESHOT_EXCLAMATION);
                ...
                case 36:
                    Talk(SAY_EVENT11);
                    if (Creature* creature = me->SummonCreature(22466, me->GetPositionX()+4, me->GetPositionY(), me->GetPositionZ(),0 , TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 20000))
                    {
                        if(creature)
                            creature->AI()->AttackStart(me);
                    }

                    if(Creature *attack = me->FindNearestCreature(22466,10.0f,true))
                    {
                        if (attack)
                            me->AI()->AttackStart(attack);
                    }
                    break;
                case 37:
                    if (player->GetQuestStatus(10922) == QUEST_STATUS_INCOMPLETE)
                        player->GroupEventHappens(10922, me);

                    Talk(SAY_EVENT12);
                    Talk(SAY_EVENT13, player);
                    if(GameObject *temp = me->FindNearestGameObject(202882,10.0f))
                    {
                        if(temp)
                            temp->DestroyForNearbyPlayers();
                    }
                    me->HandleEmoteCommand(EMOTE_ONESHOT_TALK);
                    me->DespawnOrUnsummon(8500);
                    Despawn();
                    break;
            }
        }
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            {
                if (player->GetQuestStatus(10922))
                    player->FailQuest(10922);
    ...
    {
        return new npc_chief_archaeologistAI(creature);
    }
};
...
class bone_sifter : public CreatureScript
{
public:
    bone_sifter() : CreatureScript("bone_sifter") { }

    CreatureAI* GetAI(Creature* creature) const
    {
        return new bone_sifterAI (creature);
    }

    struct bone_sifterAI : public ScriptedAI
    {
        bone_sifterAI(Creature* creature) : ScriptedAI(creature) {}

        uint32 uiBoreTimer;
        uint32 uiPoisonTimer;

        void Reset()
        {
            uiBoreTimer = urand(5*IN_MILLISECONDS,15*IN_MILLISECONDS);
            uiPoisonTimer = urand(7*IN_MILLISECONDS,15*IN_MILLISECONDS);
        }
        ...
        void UpdateAI(uint32 uiDiff)
        {
            ...
            if(uiBoreTimer <= uiDiff)
            {
                if(Unit *target = SelectTarget(SELECT_TARGET_RANDOM,0,50.0f,true))
                {
                    DoCast(target,SPELL_BORE);
                    uiBoreTimer = urand(6*IN_MILLISECONDS,15*IN_MILLISECONDS);
                }
            }
            else
                uiBoreTimer -= uiDiff;

            if(uiPoisonTimer <= uiDiff)
            {
                DoCast(me->GetVictim(),SPELL_POISON);
                uiPoisonTimer = urand(5*IN_MILLISECONDS,9*IN_MILLISECONDS);
            }
            else
                uiPoisonTimer -= uiDiff;
