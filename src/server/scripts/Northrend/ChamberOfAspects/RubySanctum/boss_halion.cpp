...
enum Events
{
    ...
    EVENT_SHADOW_PULSARS_WARN   = 16,
    EVENT_TRIGGER_BERSERK       = 17,
    EVENT_TWILIGHT_MENDING      = 18,
    EVENT_RECHECK_INSTANCE      = 19
};
    ...
    public:
        ...
        struct boss_twilight_halionAI : public generic_halionAI
        {
            ...
            void EnterCombat(Unit* who) OVERRIDE
            {
                ...
                me->SetHealth(me->GetHealth() * 0.75);
                generic_halionAI::EnterCombat(who);
    ...
    public:
        ...
        struct npc_halion_controllerAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit* /*who*/) OVERRIDE
            {
                ...
                _events.ScheduleEvent(EVENT_RECHECK_INSTANCE, 15 * IN_MILLISECONDS);
            }
                ...
                switch (action)
                {
                    ...
                    case ACTION_MONITOR_CORPOREALITY:
                    {
                        for (uint8 itr = DATA_HALION; itr <= DATA_TWILIGHT_HALION; itr++)
                        {
                            ...
                            Creature* twilight = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_TWILIGHT_HALION));
                            if (!halion)
                                continue;
                            ...
                            twilight->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                            halion->AddAura(SPELL_COPY_DAMAGE, twilight);
                            twilight->AddAura(SPELL_COPY_DAMAGE, halion);
                        }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_TRIGGER_BERSERK:
                            ...
                            _events.ScheduleEvent(EVENT_TRIGGER_BERSERK, 5 * MINUTE * IN_MILLISECONDS);
                            break;
                        case EVENT_SHADOW_PULSARS_WARN:
                            ...
                            _events.ScheduleEvent(EVENT_SHADOW_PULSARS_SHOOT, 3000);
                            break;
                        case EVENT_SHADOW_PULSARS_SHOOT:
                            ...
                            _events.ScheduleEvent(EVENT_SHADOW_PULSARS_WARN, 29000);
                            break;
                        case EVENT_CHECK_CORPOREALITY:
                            UpdateCorporeality();
                            _events.ScheduleEvent(EVENT_CHECK_CORPOREALITY, 5000);
                            break;
                        case EVENT_RECHECK_INSTANCE:
                            {
                                if (!_instance)
                                    break;
                                Creature* halion = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_HALION));
                                if (!halion)
                                    break;
                                ...
                                for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                                {
                                    ...
                                    if (!player->IsInDist2d(3156.0371f, 533.2656f, 51.0f))
                                        halion->Kill(player);
                                    else
                                    {
                                        someoneAlive = true;
                                ...
                                if (someoneAlive)
                                    _events.ScheduleEvent(EVENT_RECHECK_INSTANCE, 40 * IN_MILLISECONDS);
                                else
        ...
        private:
            ...
            void UpdateCorporeality()
            {
                ...
                if (_twilightDamageTaken == 0 || _materialDamageTaken == 0)
                {
                    ...
                    _twilightDamageTaken = 0;
                    _materialDamageTaken = 0;
                    _twilightDamageTakenOld = 0;
                    _materialDamageTakenOld = 0;
                    return;
                }


                CorporealityEvent action = CORPOREALITY_NONE;
                if (_materialDamageTaken < 0.98f * _twilightDamageTaken)            // [0   , 0.98[: Corporeality goes up (twiligth damage > materialdamage)
                    action = CORPOREALITY_INCREASE;
                else if ( _twilightDamageTaken * 1.02f < _materialDamageTaken)      // ]1.02, +oo [: Corporeality goes down (material damage > twilight damage)
                    action = CORPOREALITY_DECREASE;
                if (_twilightDamageTakenOld == _twilightDamageTaken || _materialDamageTakenOld == _materialDamageTaken)
                    action = CORPOREALITY_TWILIGHT_MENDING;

                _twilightDamageTakenOld = _twilightDamageTaken;
                _materialDamageTakenOld = _materialDamageTaken;
            ...
            uint32 _muertos;
            uint32 _fueraDeRango;

            uint32 _twilightDamageTaken;
            uint32 _materialDamageTaken;
            uint32 _twilightDamageTakenOld;
            uint32 _materialDamageTakenOld;
        ...
        struct npc_orb_carrierAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 /*diff*/) OVERRIDE
            {
                ...
                if (IsHeroic() && !_check)
                {
                    ...
                    vehicle->InstallAccessory(40468, 2, 1, 6, 30000);
                    vehicle->InstallAccessory(40469, 3, 1, 6, 30000);
            ...
            void DoAction(int32 action) OVERRIDE
            {
                switch (action)
                {
                    case ACTION_METEOR_STRIKE_AOE:
                        ...
                        {
                            _numSpawms = urand(8, 10);
                            for (uint32 i = _numSpawms; i != 0; --i)
                            {
                                me->SummonCreature(LIVING_EMBER, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ());
                            }
                        }
    ...
    public:
        ...
        struct npc_living_emberAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 diff) OVERRIDE
            {
                ...
                if (_awakenTimer <= diff)
                {
                    if (me->FindNearestCreature(NPC_LIVING_INFERNO, 15.0f))
                        me->AddAura(SPELL_AWAKEN_FLAMES, me);

                    _awakenTimer = 2000;
                }
                else _awakenTimer -= diff;
