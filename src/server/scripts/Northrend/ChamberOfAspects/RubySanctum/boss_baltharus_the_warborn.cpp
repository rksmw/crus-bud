        ...
        class spell_baltharus_enervating_brand_AuraScript : public AuraScript
        {
            ...
            void HandlePeriodic(AuraEffect const* aurEff)
            {
                if (Unit* target = GetCaster())
                {
                    ...
                    {
                        if (owner->IsWithinDist(target, 45.0f) && owner->IsWithinLOS(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ()))
                            owner->CastSpell(target, SPELL_SIPHONED_MIGHT, true);
                    }
                }
            }
            ...
            {
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_baltharus_enervating_brand_AuraScript::HandlePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
            }
        };
