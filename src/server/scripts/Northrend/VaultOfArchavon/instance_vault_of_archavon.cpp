    ...
    public:
        ...
        struct instance_vault_of_archavon_InstanceMapScript : public InstanceScript
        {
            ...
            void OnPlayerEnter(Player* player)
            {
                ...
                if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldByBattleId(BATTLEFIELD_WG))
                    if (!player->IsGameMaster() && player->GetTeamId() != bf->GetDefenderTeam())
                    {
                        player->SendTransferAborted(624, TRANSFER_ABORT_MAP_NOT_ALLOWED);
                        player->CastSpell(player, 58622, true);
                    }
            }
