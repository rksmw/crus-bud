...
class npc_wg_queue : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) OVERRIDE
        {
            ...
            Battlefield* wintergrasp = sBattlefieldMgr->GetBattlefieldByBattleId(BATTLEFIELD_WG);
            if (!wintergrasp)
                return true;

            if (wintergrasp->IsBattleTime())
            {
            ...
            {
                uint32 timer = wintergrasp->GetTimer() / IN_MILLISECONDS;
        ...
        bool OnGossipSelect(Player* player, Creature* /*creature*/, uint32 /*sender*/, uint32 /*action*/) OVERRIDE
        {
            ...
            wintergrasp->InvitePlayer(player);
            return true;
        }
};
...
public:
    ...
    {
        if (target)
            if (target->IsMounted()) // Dificil que se de el caso que muerto este montado
                if (Vehicle* vehicle = target->GetVehicle())
                    if (vehicle->GetCreatureEntry() == WG_NPC_CANNON)
                            return true;
...
class achievement_wg_destruction_derby : public AchievementCriteriaScript
{
public:
    achievement_wg_destruction_derby() : AchievementCriteriaScript("achievement_wg_destruction_derby") { }
...
public:
    ...
    bool OnCheck(Player* source, Unit* target)
    {
        if (Battlefield* wintergrasp = sBattlefieldMgr->GetBattlefieldByBattleId(BATTLEFIELD_WG))
            if (wintergrasp->GetAttackerTeam() == source->GetTeamId() && wintergrasp->GetTimer() > 20 * MINUTE * IN_MILLISECONDS)
                return true;
...
public:
    ...
    class spell_wintergrasp_defender_teleport_SpellScript : public SpellScript
    {
        ...
        {
            if (Battlefield* wg = sBattlefieldMgr->GetBattlefieldByBattleId(BATTLEFIELD_WG))
...
void AddSC_wintergrasp()
{
    ...
    new achievement_wg_destruction_derby();
