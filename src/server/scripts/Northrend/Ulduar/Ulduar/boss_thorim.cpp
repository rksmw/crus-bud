...
enum Events
{
    ...
    EVENT_SAY_EVENT_3,
    EVENT_BEGIN_COMBAT,
    EVENT_HARD_MODE_FAILED,
    EVENT_START_PHASE_2,
    EVENT_SAY_NORMAL_OUT_1,
    EVENT_SAY_NORMAL_OUT_2,
    EVENT_SAY_NORMAL_OUT_3,
    EVENT_SAY_HARD_OUT_1,
    EVENT_SAY_HARD_OUT_2,
    EVENT_SAY_HARD_OUT_3,
    EVENT_END,
    CAST_TOUCH_OF_DOMINION,
    EVENT_REMOVE_SIF,
    SIF_SAY_EVENT_03,
    SIF_HARD_MODE,
    SIF_DESPAWN_NORMAL_1,
    SIF_DESPAWN_NORMAL_2,
    ACTION_CAST_FIELD,

    // Thorim's controller
    EVENT_CHECK_PLAYER_IN_RANGE,   // used also by Pre-phase adds
    EVENT_CHECK_WIPE,
...
{
    //Thorim                
    SAY_THORIM_AGGRO_1      = 0,
    SAY_THORIM_AGGRO_2      = 15,
    SAY_THORIM_SPECIAL_1    = 1,
    SAY_THORIM_SPECIAL_2    = 2,
    SAY_THORIM_SPECIAL_3    = 3,
    SAY_THORIM_JUMPDOWN     = 4,
    SAY_THORIM_SLAY         = 5,
    SAY_THORIM_BERSERK      = 6,
    SAY_THORIM_WIPE         = 7,
    SAY_THORIM_DEATH        = 8,
    SAY_THORIM_END_NORMAL_1 = 9,
    SAY_THORIM_END_NORMAL_2 = 10,
    SAY_THORIM_END_NORMAL_3 = 11,
    SAY_THORIM_END_HARD_1   = 12,
    SAY_THORIM_END_HARD_2   = 13,
    SAY_THORIM_END_HARD_3   = 14,
...
enum Creatures
{
    NPC_INFLUENCE_TENTACLE_SIF      = 34202,
    NPC_SIF                         = 33196,
    ...
    NPC_THORIM_TRAP_A               = 33054, // First trap
    NPC_THORIM_TRAP_B               = 33725  // Second trap
};
...
enum TrapSpells
{
    PARALYTIC_FIELD_A       = 62241,
    PARALYTIC_FIELD_B       = 63540
};
...
{
    SPELL_BLINK                                 = 62578,
    SPELL_TOUCH_OF_DOMINION_CHANNEL             = 62507,
    ...
    public:
        ...
        struct npc_thorim_controllerAI : public ScriptedAI
        {
            npc_thorim_controllerAI(Creature* creature) : ScriptedAI(creature), _summons(me)
            {
                ...
                _events.ScheduleEvent(EVENT_CHECK_PLAYER_IN_RANGE, 10000);
            }
            ...
            void UpdateAI(uint32 diff) OVERRIDE
            {
                ...
                if (refreshTimer > 2500)
                {
                    DoZoneInCombat(me, 85.0f);
                    refreshTimer = 0;
                    if (!_gotActivated)
                    {
                        ...
                        for (Map::PlayerList::const_iterator itr = playerList.begin(); itr != playerList.end(); ++itr)
                        {
                            ...
                            {
                                if (me->IsWithinDistInMap(player, 85.0f))
                                {
...
public:
    ...
    CreatureAI* GetAI(Creature* creature) const OVERRIDE
    {
        return new npc_thorim_trap_bunnyAI(creature);
    }
    ...
    struct npc_thorim_trap_bunnyAI : public ScriptedAI
    {
        ...
        void DoAction(int32 action)
        {
            switch (action)
            {
                case ACTION_CAST_FIELD:
                    if (me->GetEntry() == NPC_THORIM_TRAP_A)
                        DoCastAOE(PARALYTIC_FIELD_A);
                    else
                        if (me->GetEntry() == NPC_THORIM_TRAP_B)
                            DoCastAOE(PARALYTIC_FIELD_B);
                    activateTimer = 25000;
                    break;
                default:
                    break;
        ...
        void UpdateAI(uint32 diff) OVERRIDE
        {
            ...
            {
                Map::PlayerList const& _playerList = me->GetMap()->GetPlayers();
                ...
                for (Map::PlayerList::const_iterator itr = _playerList.begin(); itr != _playerList.end(); ++itr)
                {
                    if (Player* player = itr->GetSource())
                    {
                        ...
                        if (me->IsWithinDistInMap(player, 12.0f))
                        {
                            me->AI()->DoAction(ACTION_CAST_FIELD);
                            break;
                        }
                    }
                }
