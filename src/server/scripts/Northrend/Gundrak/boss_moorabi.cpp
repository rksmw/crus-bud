...
public:
    ...
    struct boss_moorabiAI : public ScriptedAI
    {
        boss_moorabiAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            // Realizamos que solo la spell Transformation pueda ser cortada durante el combate
            SpellEntry *spell;
            spell = (SpellEntry*)sSpellStore.LookupEntry(SPELL_TRANSFORMATION);
            if (spell)
                spell->InterruptFlags = 13;
        }
