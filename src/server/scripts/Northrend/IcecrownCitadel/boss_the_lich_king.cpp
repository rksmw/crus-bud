...
enum Events
{
    ...
    EVENT_COMBAT_PULSE              = 64,

    // Disable LoS for Shards
    EVENT_DISABLE_LOS_SHARDS        = 65,
};
...
enum EncounterActions
{
    ...
    //Raging Spirit (Actions when players are sent in frostmorune)
        ...
        bool operator()(GameObject* go)
        {
            switch (go->GetEntry())
            {
                ...
                case GO_DOODAD_ICESHARD_STANDING04:
                    ...
                    go->EnableCollision(true); // On reset - Enable collision
                    break;
                default:
                    break;
            }
...
class StartMovementEvent : public BasicEvent
{
    public:
        ...
        bool Execute(uint64 /*time*/, uint32 /*diff*/)
        {
            ...
            _owner->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
    ...
    public:
        ...
        struct boss_the_lich_kingAI : public BossAI
        {
            ...
            void EnterCombat(Unit* target) OVERRIDE
            {
                ...
                events.ScheduleEvent(EVENT_COMBAT_PULSE, 1000);
                if (IsHeroic())
            ...
            void MovementInform(uint32 type, uint32 pointId) OVERRIDE
            {
                ...
                switch (pointId)
                {
                    ...
                    case POINT_CENTER_1:
                        ...
                        events.ScheduleEvent(EVENT_DISABLE_LOS_SHARDS, 5000, 0, PHASE_TRANSITION);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_DEFILE:
                            ...
                            /*if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, SpellTargetSelector(me, SPELL_DEFILE)))*/ // Testing Defile on Tank
                            {
                        ...
                        case EVENT_QUAKE_2:
                            ...
                            Talk(SAY_LK_QUAKE);
                            break;
                        case EVENT_DISABLE_LOS_SHARDS:
                        {
                            // Disable LoS for shards on destruction right after Remorseless Winter is cast
                            GameObject* gob1 = me->FindNearestGameObject(GO_DOODAD_ICESHARD_STANDING01, 333.0f);
                            GameObject* gob2 = me->FindNearestGameObject(GO_DOODAD_ICESHARD_STANDING02, 333.0f);
                            GameObject* gob3 = me->FindNearestGameObject(GO_DOODAD_ICESHARD_STANDING03, 333.0f);
                            GameObject* gob4 = me->FindNearestGameObject(GO_DOODAD_ICESHARD_STANDING04, 333.0f);

                            if (gob1 && gob2 && gob3 && gob4)
                            {
                                gob1->EnableCollision(false);
                                gob2->EnableCollision(false);
                                gob3->EnableCollision(false);
                                gob4->EnableCollision(false);
                            }
                            break;
                        }
            ...
            void MovementInform(uint32 type, uint32 id) OVERRIDE
            {
                ...
                switch (id)
                {
                    case POINT_DROP_PLAYER:
                        ...
                        if (!me->GetDistance(_dropPoint) && !IsHeroic())
                        {
                            DoCastAOE(SPELL_EJECT_ALL_PASSENGERS);
    ...
    public:
        ...
        struct npc_strangulate_vehicleAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* summoner) OVERRIDE
            {
                ...
                summoner->ClearUnitState(UNIT_STATE_UNATTACKABLE); // HACK: Target needs to be healable
                _events.Reset();
...
class npc_wicked_spirit : public CreatureScript
{
    public:
        ...
        {
            return new npc_wicked_spiritAI(creature);
        }
};
