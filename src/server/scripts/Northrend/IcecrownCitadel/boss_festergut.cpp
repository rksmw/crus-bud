    ...
    public:
        ...
        struct boss_festergutAI : public BossAI
        {
            ...
            {    
                ...
                instance->DoRemoveAurasDueToSpellOnPlayers(RAID_MODE(69291, 72101, 72102, 72103));
                instance->DoRemoveAurasDueToSpellOnPlayers(69279);
                RemoveBlight();
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_VILE_GAS:
                        {
                            ...
                            else if (Is25ManRaid() && minDist == 0.0f)
                            {
                                for (uint8 i = 0; i < 3; ++i)
    ...
    public:
        ...
        class spell_festergut_blighted_spores_AuraScript : public AuraScript
        {
            ...
            void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (GetTarget()->HasAura(SPELL_INOCULATED))
                    MaxStacks = GetTarget()->GetAura(SPELL_INOCULATED)->GetStackAmount();
            ...
            void Register() OVERRIDE
            {
                AfterEffectRemove += AuraEffectRemoveFn(spell_festergut_blighted_spores_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE, AURA_EFFECT_HANDLE_REAL);
            }
        };
