...
struct StalkerPerGroup
{
    StalkerPerGroup() {}

    bool operator() (Creature* stalker) const
    {
        switch (stalker->GetDBTableGUIDLow())
        {
            case STALKER_ONE:
            case STALKER_TWO:
            case STALKER_THREE:
    ...
    public:
        ...
        struct boss_professor_putricideAI : public BossAI
        {
            ...
            void JustDied(Unit* /*killer*/) OVERRIDE
            {
                ...
                instance->DoRemoveAurasDueToSpellOnPlayers(72873);
                ...
                switch (action)
                {
                    ...
                    case ACTION_ROTFACE_COMBAT:
                    {
                        ...
                        if (Creature* rotface = Unit::GetCreature(*me, instance->GetData64(DATA_ROTFACE)))
                        {
                            ...
                            list.remove_if(StalkerPerGroup());
                    ...
                    case ACTION_ROTFACE_OOZE:
                        ...
                        else
                        {
                            ...
                            {
                                i = urand(0, 3);
                            } while (_oozeFloodStageUsed[i]);
                            _oozeFloodStage = i;
                        }
                        setOozeFloodUsed(_oozeFloodStage);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_FESTERGUT_GOO:
                            ...
                            events.ScheduleEvent(EVENT_FESTERGUT_GOO, (Is25ManRaid() ? 10000 : 15000) + urand(0, 5000), 0, PHASE_FESTERGUT);
                            break;
        ...
        private:
            ...
            {
                for (uint8 i = 0; i < 4; i++)
                    _oozeFloodStageUsed[i] = false;
            }

            bool comproveFloodUsed(uint8 i)
            {
                if (_oozeFloodStageUsed[i])
                    return true;
                return false;
            }

            bool comproveAllUsed()
            {
                for (uint8 i = 0; i < 4; i++)
                    if (!_oozeFloodStageUsed[i])
                        return false;
            ...
            void setOozeFloodUsed(uint8 i)
            {
                _oozeFloodStageUsed[i] = true;
            }

            uint64 _oozeFloodDummyGUIDs[4];
            bool _oozeFloodStageUsed[4];
...
const uint32 ForbiddenAuras[14] =
{
    // Hand of Protection
    1022,
    5599,
...
class npc_putricide_oozeAI : public ScriptedAI
{
    public:
        ...
        void UpdateAI(uint32 diff) OVERRIDE
        {
            ...
            if (i->GetBossState(DATA_PROFESSOR_PUTRICIDE) != IN_PROGRESS)
                me->DespawnOrUnsummon();

            if (Unit* player = me->GetVictim())
                for (uint8 i = 0; i < 14; ++i)
                    if (player->HasAura(ForbiddenAuras[i]))
                        player->RemoveAura(ForbiddenAuras[i]);
...
class MutatedTargetSelector
{
public:
    MutatedTargetSelector() { }

    bool operator()(WorldObject* object) const
    {
        if (Unit* unit = object->ToUnit())
            return unit->HasAura(46598);
        return false;
    }
};
...
{
public:
    BloatedTargetSelector(Unit* caster) : _caster(caster) { }

    bool operator()(WorldObject* object) const
    {
        if (Unit* unit = object->ToUnit())
            return unit->HasAura(sSpellMgr->GetSpellIdForDifficulty(SPELL_GASEOUS_BLOAT, _caster));
        return false;
    }
private:
    Unit* _caster;
...
class AdhesivedTargetSelector
{
public:
    AdhesivedTargetSelector(Unit* caster) : _caster(caster) { }

    bool operator()(WorldObject* object) const
    {
        if (Unit* unit = object->ToUnit())
            return unit->HasAura(sSpellMgr->GetSpellIdForDifficulty(SPELL_VOLATILE_OOZE_ADHESIVE, _caster));
        return false;
    }
private:
    Unit* _caster;
...
class nonTankTargetSelector
{
public:
    nonTankTargetSelector(Unit* professor) : _professor(professor) { }

    bool operator()(WorldObject* object) const
    {
        if (Unit* target = object->ToUnit())
            if (Unit* tank = _professor->GetVictim())
                if (tank->GetTypeId() == TYPEID_PLAYER && target == tank)
                    return true;
        return false;
    }
private:
...
class spell_putricide_ooze_channel : public SpellScriptLoader
{
    public:
        ...
        class spell_putricide_ooze_channel_SpellScript : public SpellScript
        {
            ...
            {
                targets.remove_if(MutatedTargetSelector());
                targets.remove_if(BloatedTargetSelector(GetCaster()));
                targets.remove_if(AdhesivedTargetSelector(GetCaster()));
                targets.remove_if(AliveTargetSelector());

                if (Creature* professor = ObjectAccessor::GetCreature(*GetCaster(), GetCaster()->GetInstanceScript()->GetData64(DATA_PROFESSOR_PUTRICIDE)))
                    targets.remove_if(nonTankTargetSelector(professor));
