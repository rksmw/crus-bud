...
namespace lady
{
    ...
    public:
        ...
        bool Execute(uint64 /*time*/, uint32 /*diff*/)
        {
            ...
            if (Unit* target = _summoner->AI()->SelectTarget(SELECT_TARGET_NEAREST, 0, NonTankTargetSelector(_summoner)))
        ...
        struct boss_lady_deathwhisperAI : public BossAI
        {
            ...
            void JustSummoned(Creature* summon) OVERRIDE
            {
                ...
                if (summon->GetEntry() == NPC_VENGEFUL_SHADE)
                    summon->m_Events.AddEvent(new lady::StartMovementEvent(me, summon), summon->m_Events.CalculateTime(3000));
                else
                {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_P2_SUMMON_SHADE:
                            if (Unit* shadeTarget1 = SelectTarget(SELECT_TARGET_RANDOM, 1))
                                DoCast(shadeTarget1, SPELL_SUMMON_SHADE);

                            if (Is25ManRaid())
                            {
                                if (Unit* shadeTarget2 = SelectTarget(SELECT_TARGET_RANDOM, 1))
                                    DoCast(shadeTarget2, SPELL_SUMMON_SHADE);

                                if (Unit* shadeTarget3 = SelectTarget(SELECT_TARGET_RANDOM, 1))
                                    DoCast(shadeTarget3, SPELL_SUMMON_SHADE);
                            }
