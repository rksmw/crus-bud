        ...
        struct boss_valithria_dreamwalkerAI : public ScriptedAI
        {
            ...
            void HealReceived(Unit* healer, uint32& heal) OVERRIDE
            {
                ...
                if (me->HealthAbovePctHealed(100, heal) && !_done)
                {
                    ...
                    _instance->GuildSystemInstanceGiveXp();
                    _instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, me);
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_BERSERK:
                            ...
                            if (Creature* lichKing = ObjectAccessor::GetCreature(*me, _instance->GetData64(DATA_VALITHRIA_LICH_KING)))
                                lichKing->AI()->DoAction(EVENT_BERSERK);
                            break;
...
class npc_the_lich_king_controller : public CreatureScript
{
    public:
        ...
        struct npc_the_lich_king_controllerAI : public ScriptedAI
        {
            ...
            {
                if (type == EVENT_BERSERK)
                    return _enraged ? DONE : FAIL;
            ...
            {
                if (action != EVENT_BERSERK)
                    return;

                _enraged = true;

                for (uint8 i = 0; i < (GetDifficulty() == RAID_DIFFICULTY_25MAN_HEROIC ? 6 : 3); i++)
                {
...
class npc_gluttonous_abomination : public CreatureScript
{
    public:
        ...
        struct npc_gluttonous_abominationAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* killer) OVERRIDE
            {
                if (!killer->GetEntry()) // Spawn worms only if killer isnt a npc
...
class spell_dreamwalker_summoner : public SpellScriptLoader
{
    public:
        ...
        class spell_dreamwalker_summoner_SpellScript : public SpellScript
        {
            ...
            {
                if (Creature* lichKing = ObjectAccessor::GetCreature(*GetCaster(), GetCaster()->GetInstanceScript()->GetData64(DATA_VALITHRIA_LICH_KING)))
                    if (lichKing->AI()->GetData(EVENT_BERSERK) != DONE)
    ...
    public:
        ...
        class spell_dreamwalker_summon_suppresser_AuraScript : public AuraScript
        {
            ...
            void PeriodicTick(AuraEffect const* /*aurEff*/)
            {
                ...
                if (Creature* lichKing = ObjectAccessor::GetCreature(*caster, caster->GetInstanceScript()->GetData64(DATA_VALITHRIA_LICH_KING)))
                    if (lichKing->AI()->GetData(EVENT_BERSERK) != DONE)
