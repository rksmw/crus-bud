...
enum Events
{
    ...
    EVENT_STICKY_OOZE       = 7,
    EVENT_ACTIVATE_BIG_OOZE = 8,
    EVENT_ACTIVATE_ZOMBIE   = 9
};
    ...
    public:
        ...
        struct npc_big_oozeAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* /*summoner*/) OVERRIDE
            {
                ...
                DoCast(me, SPELL_GREEN_ABOMINATION_HITTIN__YA_PROC, true);
                events.ScheduleEvent(EVENT_ACTIVATE_BIG_OOZE, 2000);
                events.ScheduleEvent(EVENT_STICKY_OOZE, 5000);
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_DECIMATE:
                            ...
                            _events.ScheduleEvent(EVENT_DECIMATE, urand(32500, 34000));
                            break;
...
public:
    ...
    struct npc_plagued_zombieAI : public ScriptedAI
    {
        npc_plagued_zombieAI(Creature* creature) : ScriptedAI(creature)
        {
            me->SetReactState(REACT_PASSIVE);
            DoCast(SPELL_INFECTED_WOUND);
            _events.ScheduleEvent(EVENT_ACTIVATE_ZOMBIE, 3000);
        }
