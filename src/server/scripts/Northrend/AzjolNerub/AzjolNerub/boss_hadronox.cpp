...
enum Movements
{
        POINT_FINALIZE_WPS                                                      = 0, // Point ID used by each member of crusher reinforcement group to finalize wps
...
class spell_trigger_large_aoi_summon_anubar_champion_periodic : public SpellScriptLoader
{
    public:
        ...
        class spell_trigger_large_aoi_summon_anubar_champion_periodic_AuraScript : public AuraScript
        {
            ...
            {
                if (_instance->GetData(DATA_KRIKTHIR_THE_GATEWATCHER_EVENT) != DONE || _instance->GetData(DATA_HADRONOX_EVENT) == DONE)
                                        return;
