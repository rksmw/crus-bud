...
public:
    ...
    static bool HandleGMTicketAssignToCommand(ChatHandler* handler, char const* args)
    {
        ...
        ticket->SetAssignedTo(targetGuid, AccountMgr::IsAdminAccount(sAccountMgr->GetSecurity(accountId, realmID)));
    ...
    static bool HandleGMTicketUnAssignCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string msg = ticket->FormatMessageString(*handler, NULL, assignedTo.c_str(), handler->GetSession() ? handler->GetSession()->GetPlayer()->GetName().c_str() : "Console", NULL, NULL);
        handler->SendGlobalGMSysMessage(msg.c_str());
