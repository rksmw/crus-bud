...
public:
    ...
    static bool HandleNpcAddCommand(ChatHandler* handler, char const* args)
    {
        ...
        Event* e = NULL;
        bool isDev = chr->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DEVELOPER);
        if (!isDev)
        {
            if (map->Instanceable())
            {
            ...
            e = sEventMgr->GetEventByConstructorGUID(chr->GetGUID());
            if (!e)
            {
                handler->SendSysMessage("No estas en modo de construccion.");
                handler->SetSentErrorMessage(true);
                return false;
            }

            const CreatureTemplate* creatureInfo = sObjectMgr->GetCreatureTemplate(id);
            if (!sEventMgr->IsValidCreature(creatureInfo->rank, creatureInfo->Entry))
            {
                handler->SendSysMessage("Esa criatura se encuentra bloqueada por seguridad.");
                handler->SetSentErrorMessage(true);
                return false;
            }
        }
        ...
        if (!creature->Create(sObjectMgr->GenerateRecycledLowGuid(HIGHGUID_UNIT), map, chr->GetPhaseMaskForSpawn(), id, 0, (uint32)teamval, x, y, z, o))
        {
            delete creature;
            return false;
        }
        ...
        if (e)
            e->InsertCreature(creature->GetGUID(), chr->GetSession()->GetAccountId()); // Despues de ser aniadido al Grid
    ...
    static bool HandleNpcDeleteCommand(ChatHandler* handler, char const* args)
    {
        ...
        bool isDev = handler->GetSession()->GetPlayer()->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DEVELOPER);
        if (!isDev && unit->GetMap()->Instanceable())
        {
            handler->SendSysMessage("Esta accion se encuentra bloqueada en Instancias, Arenas y Battlegrounds.");
            handler->SetSentErrorMessage(true);
            return false;
        ...
        if (!isDev)
        {
            ...
            else if (!e->IsEventCreature(unit))
            {
                handler->SendSysMessage("No se ha encontrado ningun evento para esta criatura.");
                handler->SetSentErrorMessage(true);
                return false;
            }
        }
    ...
    static bool HandleNpcInfoCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        if (Event* e = sEventMgr->GetEventByCreatureGUID(target->GetGUID()))
        {
            std::string eventAccountName = "...";
            std::string eventVisible = e->IsVisible() ? "si" : "no";
            uint32 eventAccountId = e->GetAccountOfCreature(target->GetGUID());
            AccountMgr::GetName(eventAccountId, eventAccountName);
            ...
            handler->PSendSysMessage("├─ Cuenta: %s (Id: %u)", eventAccountName.c_str(), eventAccountId);
        }

        if (House* house = sHouseMgr->GetHouseBy(target))
        {
            uint8 inUse = 0;
    ...
    */
    ...
    static bool HandleNpcAddTempSpawnCommand(ChatHandler* handler, char const* args)
    {
        ...
        bool isDev = chr->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DEVELOPER);
        if (!isDev && chr->GetMap()->Instanceable())
        {
            handler->SendSysMessage("Esta accion se encuentra bloqueada en Instancias, Arenas y Battlegrounds.");
            handler->SetSentErrorMessage(true);
            return false;
        ...
        if (!creatureInfo)
        {
            handler->SendSysMessage("Esa criatura no existe.");
            handler->SetSentErrorMessage(true);
            return false;
        }

        if (!isDev && !sEventMgr->IsValidCreature(creatureInfo->rank, creatureInfo->Entry))
        {
            handler->SendSysMessage("Esa criatura se encuentra bloqueada por seguridad.");
            handler->SetSentErrorMessage(true);
            return false;
