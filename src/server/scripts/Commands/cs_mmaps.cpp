...
public:
    ...
    static bool HandleMmapPathCommand(ChatHandler* handler, const char* args)
    {
        ...
        {
            Creature* waypoint = player->SummonCreature(VISUAL_WAYPOINT, pointPath[i].x, pointPath[i].y, pointPath[i].z, 0, TEMPSUMMON_TIMED_DESPAWN, 10 * IN_MILLISECONDS);
            waypoint->SetLevel(i + 1);
        }
