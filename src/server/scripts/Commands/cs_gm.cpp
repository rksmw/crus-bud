...
public:
    ...
    static bool HandleGMFixDualSpecCommand(ChatHandler* handler, const char* args)
    {
        Player* target = handler->getSelectedPlayer();
        if (!target)
            target = handler->GetSession()->GetPlayer();

        if (!target->HasAchieved(2716))
        {
            handler->PSendSysMessage("Este jugador no tiene el logro de la doble especializacion.");
            handler->SetSentErrorMessage(true);
            return false;
        }

        target->removeSpell(63645, false, false);  // Unlearn Activate Primary Spec
        target->learnSpell(63645, false);          // Learn Activate Primary Spec
        target->removeSpell(63644, false, false);  // Unlearn Activate Secondary Spec
        target->learnSpell(63644, false);          // Learn Activate Secondary Spec
        handler->PSendSysMessage("Arreglada doble especializacion en %s.", handler->GetNameLink(target).c_str());
        return true;
    }
};
