...
public:
    ...
    static bool HandleGameObjectAddCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!isDev)
        {
            ...
            e = sEventMgr->GetEventByConstructorGUID(player->GetGUID());
            if (!e)
            {
            ...
            if (!sEventMgr->IsValidGameObject(objectInfo->type, objectInfo->entry))
            {
                handler->SendSysMessage("Este objeto se encuentra bloqueado por seguridad.");
                handler->SetSentErrorMessage(true);
                return false;
            }
        }
        ...
        if (e)
            e->InsertGameObject(object->GetGUID(), player->GetSession()->GetAccountId()); // Despues de ser aniadido al Grid
    ...
    static bool HandleGameObjectAddTempCommand(ChatHandler* handler, char const* args)
    {
        ...
        bool isDev = player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_DEVELOPER);
        if (!isDev && player->GetMap()->Instanceable())
        {
            handler->SendSysMessage("Esta accion se encuentra bloqueada en Instancias, Arenas y Battlegrounds.");
            handler->SetSentErrorMessage(true);
            return false;
        ...
        if (!isDev && !sEventMgr->IsValidGameObject(objectInfo->type, objectInfo->entry))
        {
            handler->SendSysMessage("Este objeto se encuentra bloqueado por seguridad.");
            handler->SetSentErrorMessage(true);
            return false;
    ...
    static bool HandleGameObjectTargetCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (target)
        {
            ...
            if (Event* e = sEventMgr->GetEventByGameObjectGUID(target->GetGUID()))
            {
    ...
    static bool HandleGameObjectDeleteCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!isDev)
        {
            if (!isDev && object->GetMap()->Instanceable())
            {
                handler->SendSysMessage("Esta accion se encuentra bloqueada en Instancias, Arenas y Battlegrounds.");
                handler->SetSentErrorMessage(true);
                return false;
            ...
            e = sEventMgr->GetEventByConstructorGUID(handler->GetSession()->GetPlayer()->GetGUID());
            if (!e)
            {
                handler->SendSysMessage("No estas en modo de construccion.");
                handler->SetSentErrorMessage(true);
            ...
            else if (!e->IsEventGameObject(object))
            {
                handler->SendSysMessage("No se ha encontrado ningun evento para este objeto.");
                handler->SetSentErrorMessage(true);
                return false;
            }
        }
