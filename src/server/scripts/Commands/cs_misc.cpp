...
#include "../Custom/world_query_callback.h"
...
public:
    ...
    {
        static ChatCommand FilterCommandTable[] =
        {
            { "add",              RBAC_PERM_COMMAND_FILTER_ADD,        true, &HandleFilterAddCommand,        "", NULL },
            { "remove",           RBAC_PERM_COMMAND_FILTER_REMOVE,     true, &HandleFilterRemoveCommand,     "", NULL },
            { "list",             RBAC_PERM_COMMAND_FILTER_LIST,       true, &HandleFilterListCommand,       "", NULL },
            { "mod",              RBAC_PERM_COMMAND_FILTER_MOD,        true, &HandleFilterModCommand,        "", NULL },
        ...
        static ChatCommand commandTable[] =
        {
            ...
            { "filter",           RBAC_PERM_COMMAND_FILTER,           false, NULL,                           "", FilterCommandTable },
            { NULL,               0,                                  false, NULL,                           "", NULL }
        };
        return commandTable;
    }
    ...
    **/
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (sWorldCallbackScript->_pinfoCallback.GetParam())
        {
            handler->SendSysMessage("Alguien mas puso pinfo, y esta en prcoeso. Solo podemos procesar uno a la vez.");
            handler->SetSentErrorMessage(true);
            return false;
        }

        sWorldCallbackScript->_pinfoCallback.SetParam(new PlayerInfoCommand(accId, lowguid, eMail.c_str(), regMail.c_str(), latency, muteTime, raceid, classid, gender, totalPlayerTime, level, alive.c_str(), money, mapId, areaId, phase, handler->GetSession(), target, targetName.c_str()));
    ...
    */
    ...
    static bool HandleFilterAddCommand(ChatHandler* handler, char const* args)
	{
		...
		handler->PSendSysMessage("The word '%s' is added to 'custom_censure_word'.", badWord.c_str());
		return true;
	}

	static bool HandleFilterRemoveCommand(ChatHandler* handler, char const* args)
	{
		std::string badWord = args;
		...
		handler->PSendSysMessage("The word '%s' is removed from 'custom_censure_word'.", badWord.c_str());
		return true;
	}

	static bool HandleFilterListCommand(ChatHandler* handler, char const* args)
	{
		...
		for (WordFilterMgr::BadWordMap::const_iterator it = badWords.begin(); it != badWords.end(); ++it)
		{
			...
			{
				handler->PSendSysMessage("Censure Words: %s", strBadWordList.c_str());
	...
	static bool HandleFilterModCommand(ChatHandler* handler, char const* args)
	{
		std::string argstr = (char*)args;
		if (argstr == "on")
		{
