            ...
            void UpdateAI(uint32 diff) OVERRIDE
            {
                ...
                if (me->GetCharmerOrOwnerGUID() && me->GetVictim())
                {
                    hasCC = me->GetVictim()->HasAuraType(SPELL_AURA_MOD_CONFUSE) ||
                        me->GetVictim()->HasAuraWithMechanic(MECHANIC_KNOCKOUT) ||
                        me->GetVictim()->HasAuraWithMechanic(MECHANIC_SAPPED) ||
                        me->GetVictim()->HasAuraWithMechanic(MECHANIC_SLEEP) ? true : false;
                }
                if (hasCC)
                {
