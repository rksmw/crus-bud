...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    if (result)
    {
        ...
        PreparedQueryResult banresult;

        bool isLastIP = fields[6].GetString().compare(socket().getRemoteAddress()) == 0;
        ...
        stmt = LoginDatabase.GetPreparedStatement(LOGIN_SEL_IP_BANNED);
        stmt->setString(0, ip_address);
        banresult = LoginDatabase.Query(stmt);
        if (banresult)
        {
            if (isLastIP && (*banresult)[0].GetString().compare("Login Server") == 0)
                banresult = NULL;
            else if ((*banresult)[1].GetUInt32() == (*banresult)[2].GetUInt32())
            {
                pkt << uint8(WOW_FAIL_BANNED);
                TC_LOG_DEBUG(LOG_FILTER_AUTHSERVER, "'%s:%d' [AuthChallenge] Banned ip tried to login!", socket().getRemoteAddress().c_str(), socket().getRemotePort());
            }
            else
            {
                pkt << uint8(WOW_FAIL_SUSPENDED);
                TC_LOG_DEBUG(LOG_FILTER_AUTHSERVER, "'%s:%d' [AuthChallenge] Temporarily banned ip tried to login!", socket().getRemoteAddress().c_str(), socket().getRemotePort());
            }
        }
        ...
        if (!banresult)
        {
            ...
            std::string databaseV = fields[3].GetString();
            std::string databaseS = fields[4].GetString();
        ...
        if (MaxWrongPassCount > 0)
        {
            ...
            if (PreparedQueryResult loginfail = LoginDatabase.Query(stmt))
            {
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    uint32 WrongPassBanType = sConfigMgr->GetIntDefault("WrongPass.BanType", 0);
