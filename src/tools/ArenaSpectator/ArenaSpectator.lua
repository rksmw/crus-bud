	...
	for index=1, #aurastack do
			if aurastack[index].icon then
				aurastack[index].icon:SetScript("OnUpdate", function(self, elapsed)
					self:SetAlpha(self:GetAlpha() - .004)
...
local function Execute(target, prefix, ...)
    ...
    elseif (prefix == "SCA") then
        InterruptSpell(target, tonumber(value))
        CancelSpell(target, tonumber(value))
    elseif (prefix == "ARE") then
...
local function ParseCommands(data)
    ...
    if data:find(';AUR=') then
        ...
        local aRemove, aStack, aExpire, aDuration, aSpellId, aType, aIsDebuff, aCaster = strsplit(",", data2)
        Execute(tar, "AUR", tonumber(aRemove), tonumber(aStack), tonumber(aExpire), tonumber(aDuration), tonumber(aSpellId), tonumber(aDebyffType), tonumber(aIsDebuff), aCaster)
        return
    end
